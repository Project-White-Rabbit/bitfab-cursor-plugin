...
| `fix` | `/bitfab-assistant fix [<key>] <trace-id>` | **Fix one specific bug, end to end.** Resolve the failing trace, capture it into a dataset as a **validated failing test** (so there's a regression test to turn green), then edit the code and replay only that trace until it passes. The targeted replay gets an experiment group for before/after inspection, but Studio opens only after the replay verdict is persisted and the user asks to inspect it. Then ask whether to run the full dataset in Studio as an experiment, keep iterating on the target trace, or stop. Use when the user points at a specific bug or **failing Bitfab trace** in a traced function and wants it *fixed* and locked in as a test. **Requires a trace anchor** (a trace ID, or a failure the user is clearly pointing at in a traced function): a bare "fix this bug" about ordinary code with no trace involved is NOT this mode (see the hard guard in argument routing). `<key>` is optional (inferred from the trace). Also triggered by natural language ("fix this trace", "fix trace abc123", "this trace is wrong, make it pass") |
...
Reached only from `fix` mode. The user has a specific bug, a failing trace, and wants it fixed and locked in as a regression test. This phase resolves THIS trace and its function, captures it into a dataset as a **validated failing label** (so the regression guard has a real red), then tails into the Phase 5 loop, where the fix itself is made and replayed against only the captured trace first. Diagnosis is not a separate step: it happens inside the fix (`make-change`), where you read the captured trace and the code together before editing. Studio does not open during capture; the single-trace replay is still tagged with an experiment group so Studio can later show the before/after if the user asks.
...
1. **Studio activity:** If `studioMode` is true, run `node "${CURSOR_PLUGIN_ROOT:-${CLAUDE_PLUGIN_ROOT}}/dist/commands/pushActivity.js" started "Resolving the bug"`.
   ...
   **Resolve what to fix.** The user invoked `/bitfab-assistant fix [<key>] <trace-id>` or phrased it in natural language ("fix this bug", "fix trace abc123", "this trace is wrong, make it pass"). A UUID is the **trace ID**; a non-UUID slug is the function key.
   ...
   Hold the trace ID, function key, code path, and the trace's failure details in working context.
2. **Studio activity:** If `studioMode` is true, run `node "${CURSOR_PLUGIN_ROOT:-${CLAUDE_PLUGIN_ROOT}}/dist/commands/pushActivity.js" started "Capturing the scenario"`.

   **Lock in the failing scenario before fixing anything.** Do the following, in order:

   1. **Label the trace as a validated fail.** Decide the verdict from the trace + code you read in `resolve` (the output is wrong). Write a one-or-two-sentence `annotation` describing the bug, what the output got wrong, and what correct behavior is, then call `mcp__Bitfab__set_human_labels` once with `[{ traceId, label: false, annotation }]`. This records a **validated human verdict** (not an unapproved agent suggestion), so the trace counts as a real regression test the instant it's written, with no Studio approval step. (Use `label: false` because a captured bug is a failing scenario; on the rare occasion the user is capturing an already-*passing* trace to guard against future regressions, use `label: true`.)
   2. **Pick or create a dataset.** If the user named a dataset ID, use it (confirm it's scoped to this function via `mcp__Bitfab__list_datasets`). Otherwise call `mcp__Bitfab__list_datasets` with the function key: if none exist, silently call `mcp__Bitfab__create_dataset` with `traceFunctionKey: <key>` and `name: <key>`; if one or more exist, present them via `AskUserQuestion` (one option per dataset: name · id · count, plus "Create new"), recommend the most recently used dataset that has traces. Hold the `datasetId`.
   3. **Attach the trace.** Call `mcp__Bitfab__add_traces_to_dataset` with the `datasetId` and `[traceId]` (idempotent, a safe no-op if already present).
   4. **If you picked a pre-existing dataset that already held other traces, load their labels now.** `fix` may later run the full dataset after the target trace passes, so the dataset's sibling labels need to be ready for that optional experiment. Run `node "${CURSOR_PLUGIN_ROOT:-${CLAUDE_PLUGIN_ROOT}}/dist/commands/readTracesBatched.js" <trace-id...> --scope full` over the dataset's trace IDs and `Read` the `outputFile` it prints, so every sibling's validated label + annotation is in context. **Skip this** when you just created the dataset or it held only this trace (no siblings to load).
   5. **Set the replay scope.** Hold `fixReplayScope = "single-trace"` in working context. The first replay must use only the captured trace ID, not the full dataset, and Studio must remain closed.

   Tell the user in one line: the scenario is captured as a failing test in dataset `<name>` (`<datasetId>`), and you'll now fix the code and replay only trace `<traceId>` to confirm. Then continue into the replay loop, where `make-change` is the fix.
...
In `experiment` mode this is an iterative improvement loop (each iteration makes a change and replays). In `fix` mode the first pass replays only the captured trace and tags that replay with an experiment group; after that replay's labels are persisted, the user can choose to inspect the before/after in Studio, open Studio and run the full dataset as an experiment, keep iterating, or stop. In `benchmark` mode it is a single replay of the current code followed by a terminal scorecard, no changes, no iteration.
...
3. **Run only when mode is `wizard`, `dataset`, `experiment`, `cost-optimize`, `investigate`, `benchmark` or `fix`.**
   ...
   **Fix-mode single-trace pass:** if mode is `fix` and `fixReplayScope = "single-trace"`, generate the `experimentGroupId` but do **not** open Studio yet. Hold this UUID as `fixSingleTraceExperimentGroupId` as well as the current `experimentGroupId`, then continue to `make-change`. This first pass is a targeted replay of the captured trace, and the group exists only so the user can inspect the before/after in Studio after labels are persisted. If the user later chooses the full-dataset run, set `fixReplayScope = "dataset"` and return here; then generate a fresh `experimentGroupId` for the dataset experiment and open Studio to that experiment page before replay.
   ...
   **Open the experiments page.** Pick exactly one case (they are mutually exclusive):
   ...
   - **`fix` mode with `fixReplayScope = "single-trace"`:** do NOT run any `openStudio` navigation yet. Keep Studio closed until the user explicitly chooses the post-replay "Open before/after in Studio" option. Continue to `make-change`.
...
4. **Run only when mode is `wizard`, `dataset`, `experiment`, `cost-optimize`, `investigate` or `fix`.**
   ...
   **Make the change.**
   ...
   - **In `fix` mode, this is THE fix, and diagnosis happens here (it is not a separate step).** Re-read the captured failing trace (held from Phase Fix `resolve`) and the function code together, pinpoint why the output is wrong, and make the change that addresses it. The captured trace's annotation is the acceptance criterion the first replay will check. If this step is being reached after the user chose to run the full dataset with the already-made fix (`fixReplayScope = "dataset"`, `fixSkipMakeChange = true`, and a previous single-trace fix already produced snapshots), do not edit again or ask for another change; reuse the last change description and before/after snapshots, then continue to replay the dataset.
...
5. **Run only when mode is `wizard`, `dataset`, `experiment`, `cost-optimize`, `investigate`, `benchmark` or `fix`.**
   ...
   **Choose trace selection.**
   ...
   - **Initial `fix` pass (`fixReplayScope = "single-trace"`)**: pass `--trace-ids <capturedTraceId>` and do NOT pass `--dataset-id`. This is the user's requested targeted replay. It proves the captured bug turned green before spending time on the rest of the dataset.
   ...
   **Run the replay through `node "${CURSOR_PLUGIN_ROOT:-${CLAUDE_PLUGIN_ROOT}}/dist/commands/replayProgress.js"` in the background, then relay the progress lines it prints.** Before starting, choose a unique run directory for this replay, for example `.bitfab/replays/<experimentN>-<timestamp>-<short-random>`, pass it with `--run-dir`, and hold that path in context; do not use a shared `--events-log` path. A foreground run blocks you for the whole replay and the user just sees "a shell is running" with no detail. `node "${CURSOR_PLUGIN_ROOT:-${CLAUDE_PLUGIN_ROOT}}/dist/commands/replayProgress.js"` runs the replay, turns the SDK's per-trace `@@bitfab:progress` events into one self-contained line per trace on its stdout (a header, then for each trace as it settles a pass/fail glyph, the running `n/total`, and that trace's duration, with the error reason inline on failure, plus a liveness heartbeat line when a slow trace goes quiet so the run never looks frozen, then a final summary with total + average time), and writes a small JSONL event log at `<run-dir>/events.jsonl`: `type: "progress"` rows as items settle, then a final `type: "complete"` row. Large per-item payloads are written atomically under `<run-dir>/items/` and referenced by `item.itemPath` / `items[].itemPath`, so outputs are not duplicated in the event log. It also tees every human line to `<run-dir>/progress.log`, so the user can `tail -f` it in a separate terminal for a live view that never collapses the way a tool card does. The wrapper writes `<run-dir>/run.json`; once the server test run ID is known it also writes `<parent-of-run-dir>/by-test-run/<testRunId>/run.json` pointing back to the run directory. If the replay settles no traces at all (an `@bitfab/sdk` too old to carry the progress reporter, or a script that never wired `onProgress`), it closes with a `⚠ done · replay finished but reported no progress` line instead of ending silently, so a missing stream of per-trace lines never reads as a hang, fix it via `upgrade-replay-script` (or an SDK upgrade) and re-run. You do NOT decide what or when to print: it does the formatting; you just show each new line.
   ...
   # Fix initial pass: replay only the captured target trace, no dataset-id, but tag it with an experiment group
   cd <project-dir> && node "${CURSOR_PLUGIN_ROOT:-${CLAUDE_PLUGIN_ROOT}}/dist/commands/replayProgress.js" --label <pipeline-name> -- npx tsx scripts/replay.ts <pipeline-name> --trace-ids <capturedTraceId> --name "<experimentName>" --code-change /tmp/bitfab-code-change-<experimentN>.json --experiment-group-id <experimentGroupId>
...
10. **Run only when mode is `wizard`, `dataset`, `experiment`, `cost-optimize`, `investigate`, `benchmark` or `fix`.**
   ...
   **Finalize the live evaluation loop; do not start from scratch.** If `replay-against-dataset` produced a non-empty run `events.jsonl`, read it before judging and reuse any prep, unreplayable buckets, verdicts, and persisted server replay trace IDs already produced during the running replay. Each progress event's `item.traceId` is the source/original trace id; use it to identify which dataset item settled. Enriched progress rows carry `item.replayTraceId` and `item.itemPath`; the item file has the full input, replay output, original output, and metadata needed to judge during replay. Progress-only rows are only a trigger/prep signal. If the file is missing or empty, continue from the final complete event; older scripts remain valid and simply do not get incremental evaluation. The final verdict still comes from the completed replay item files / experiment trace data, and persistence is still keyed by the **server replay** trace id.
   ...
   For each completed (non-errored) replay item, derive a verdict by comparing the replay's new output against the original trace's label and annotation (from Phase 3 in `wizard`/`dataset` modes, loaded by `pick-dataset` in Phase 5 Setup in `experiment` and `benchmark` modes, or loaded by `fix/capture` in `fix` mode, the captured trace plus any pre-existing dataset siblings):
...
13. **Run only when mode is `fix`.**
   ...
   **Run only in `fix` mode.** This step is reached after replay verdicts were evaluated for a fix run.
   ...
   - **`fixReplayScope = "dataset"` (the user already chose the full-dataset experiment)**: continue directly to share-results → step 17
   - **the targeted single-trace replay verdict is PASS (`label: true`)**: the captured trace went green. Ask whether to inspect the before/after, run the full dataset, keep iterating, or stop → step 14
   - **the targeted single-trace replay verdict is FAIL (`label: false`), skipped, unreplayable, shape-incompatible, or could not be persisted because replay trace IDs were unavailable**: the fix did not prove the target trace green. Ask whether to inspect the before/after, keep iterating, stop, or run the full dataset anyway → step 15
14. **Run only when mode is `fix`.**

   **Run only in `fix` mode when `fixReplayScope = "single-trace"` and the targeted replay passed.**

   Report the target trace result first. Lead with that the captured trace went green; this initial pass intentionally did not run the rest of the dataset and did not open Studio. The replay labels have been persisted when trace IDs were available, so this fix is queryable on the replay trace.

   Then ask what to do next. Include all four choices so the user can inspect the targeted before/after, decide whether to spend time on broader regression coverage now, keep iterating, or stop:

   > A) **Open before/after**: open Studio to the targeted single-trace experiment using `fixSingleTraceExperimentGroupId` (or `testRunIds` fallback), then ask this question again → step 16
   > B) **Run full dataset**: set `fixReplayScope = "dataset"`, set `fixSkipMakeChange = true`, open Studio, and run the same code change as a dataset experiment → step 3
...
15. **Run only when mode is `fix`.**
   ...
   **Run only in `fix` mode when `fixReplayScope = "single-trace"` and the targeted replay did not pass.**
   ...
   Report the failed target result first. Say whether the replay produced a real FAIL verdict, was skipped as ambiguous, was unreplayable / shape-incompatible, or could not persist labels because replay trace IDs were unavailable. Be explicit that the full dataset has not run yet and that the original bug is not proven fixed.

   Then ask what to do next. The default shape here is different from the passed path: do not steer toward the full dataset while the target is still red. Offer inspection and another fix attempt first, with full-dataset as an "anyway" escape hatch:

   > A) **Open before/after**: open Studio to the targeted single-trace experiment using `fixSingleTraceExperimentGroupId` (or `testRunIds` fallback), then ask the failed-result question again → step 16
   > B) **Keep iterating**: set `fixReplayScope = "single-trace"`, set `fixSkipMakeChange = false`, and make another change against the target trace *(recommended)* → step 3
   > C) **Stop and wrap up**: leave the captured dataset guard in place and report that the target is still not green → step 1 of the Phase 6: Validate & Wrap Up phase
   > D) **Run dataset anyway**: set `fixReplayScope = "dataset"`, set `fixSkipMakeChange = true`, open Studio, and run the same code change across the dataset despite the target still failing → step 3
16. **Run only when mode is `fix`.**

   **Run only when the user chose "Open before/after" from a post-target-replay fix prompt.** Open Studio to the single-trace replay's experiment view so the user can inspect the original failing trace, the replay trace, the verdict, and the code-change before/after.

   Use the experiment group from the targeted replay whenever possible:

   ```bash
   node "${CURSOR_PLUGIN_ROOT:-${CLAUDE_PLUGIN_ROOT}}/dist/commands/openStudioTo.js" "/studio/experiments?experimentGroupId=<fixSingleTraceExperimentGroupId>"
   ```

   If `supportsExperimentGroups` was false but you captured `testRunId`, use the fallback instead:

   ```bash
   node "${CURSOR_PLUGIN_ROOT:-${CLAUDE_PLUGIN_ROOT}}/dist/commands/openStudioTo.js" "/studio/experiments?testRunIds=<testRunId>"
   ```

   If neither an experiment group nor a test run ID is available, say Studio cannot show this before/after run yet and continue.
...
17. **Run only when mode is `wizard`, `dataset`, `experiment`, `cost-optimize`, `investigate` or `fix`.**
   ...
   **In `fix` mode with `fixReplayScope = "dataset"`, this is the optional full-dataset experiment after the target trace already replayed.** Frame the result around whether the captured trace stayed green and what the full dataset revealed: regressions, still-failing sibling traces, and unreplayable items. If the captured trace now passes and other dataset traces are still red, do NOT auto-loop through them: frame "Keep iterating" as an explicit offer, name how many traces are still failing and ask whether to keep going or stop here. If the captured trace regressed during the dataset run, "Keep iterating" means another fix attempt on it. Before continuing from a full-dataset fix result, always reset the fix loop state: set `fixReplayScope = "single-trace"` and `fixSkipMakeChange = false` so the next pass makes a fresh targeted change instead of replaying the dataset again with the previous code. The dataset now permanently guards this scenario regardless of what they pick.
