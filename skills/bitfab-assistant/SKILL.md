...
6. **Run only when mode is `wizard`, `dataset`, `experiment`, `cost-optimize`, `investigate`, `benchmark` or `fix`.**
   ...
   **Instead: tell the user what's wrong and offer exactly two workarounds.** Use use `AskUserQuestion` to surface a clear summary first, the failing trace ID(s), the error pattern, the function and span where it happens, then present the two options below. Pick a representative failing trace and call `mcp__Bitfab__get_traces` with `scope: "summary"` to read its `environment` field (production / staging / development), so option B can name the source environment concretely.
   ...
   - **healthy or mixed run (at least one completed item, infra errors at most half of total)**: proceed. Carry `infraErrored` forward, surface as its own bucket in the final report (the share-results step, or the benchmark scorecard's Unreplayable row in `benchmark` mode), never folded into pass/fail → step 9
...
16. **Run only when mode is `fix`.**
   ...
   **Run only in `fix` mode when `fixReplayScope = "single-trace"` and the targeted replay passed.** `fix-add-to-dataset` has just resolved where the target trace goes: either it was added to dataset `<name>` (`<datasetId>`) with a validated failing label, or the user chose to continue without saving it (on a re-entry, that choice was made earlier).
   ...
   > A) **Re-run the entire dataset**: **Only include this option in the question when `datasetHasSiblings` is true; omit it entirely when the added trace is the dataset's only trace** (re-running a one-trace dataset just replays the trace you already proved green). When included, continue to `fix-rerun-dataset-mode`, which asks whether to run it in Studio or terminal-only before replaying the same code change across the full dataset → step 17
   > B) **Show in Studio**: open Studio to the targeted single-trace before/after experiment using `fixSingleTraceExperimentGroupId` (or `testRunIds` fallback), then ask this question again → step 20
   ...
   > D) **Stop and wrap up**: stop here and leave the dataset as it is: the saved trace stays if one was added (`fixAddedToDataset`), and nothing is saved if the user continued without adding (`fixDatasetSkipped`) → step 1 of the Phase 6: Validate & Wrap Up phase
...
18. **Run only when mode is `fix`.**
   ...
   **Distinguish "the fix failed" from "couldn't even test it," because it changes the recommended next step:**
   ...
   > A) **Show in Studio**: open Studio to the targeted single-trace before/after experiment using `fixSingleTraceExperimentGroupId` (or `testRunIds` fallback), then ask the failed-result question again → step 20
