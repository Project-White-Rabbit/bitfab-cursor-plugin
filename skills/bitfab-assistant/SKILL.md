...
| `fix` | `/bitfab-assistant fix [<key>] <trace-id>` | **Fix one specific bug, end to end.** Resolve the failing trace, capture it into a dataset as a **validated failing test** (so there's a regression test to turn green), then edit the code and replay the dataset until that trace passes, flagging any regressions. Use when the user points at a specific bug or **failing Bitfab trace** in a traced function and wants it *fixed* and locked in as a test. **Requires a trace anchor** (a trace ID, or a failure the user is clearly pointing at in a traced function): a bare "fix this bug" about ordinary code with no trace involved is NOT this mode (see the hard guard in argument routing). After it passes you'll be asked whether to fix the other still-failing traces too. `<key>` is optional (inferred from the trace). Also triggered by natural language ("fix this trace", "fix trace abc123", "this trace is wrong, make it pass") |
...
**Run only when mode is `wizard`, `dataset`, `experiment`, `cost-optimize`, `investigate`, `benchmark` or `fix`.**
...
1. **In `benchmark` mode, first check the Studio opt-in flag** (set during argument routing when the `studio` keyword was passed). If benchmark did NOT opt in, skip this entire step without running any command and continue to the `pick-dataset` step (Phase 5 Setup). In all other modes, and in `benchmark` with the `studio` flag, proceed.
   ...
   **The path MUST start with `/studio`.** Never pass `/`, a bare URL, or any path outside the `/studio/` route tree.
   ...
   - **`fix [<key>] <trace-id>` mode:** pass `/studio` (Phase Fix's `capture` step navigates to the dataset review page once the datasetId is held)
...
Reached only from `fix` mode. The user has a specific bug, a failing trace, and wants it fixed and locked in as a regression test. This phase resolves THIS trace and its function, captures it into a dataset as a **validated failing label** (so the regression guard has a real red), then tails into the Phase 5 experiment loop, where the fix itself is made and replayed against the dataset. Diagnosis is not a separate step: it happens inside the fix (`make-change`), where you read the captured trace and the code together before editing.
...
2. **Studio activity:** If `studioMode` is true, run `node "${CURSOR_PLUGIN_ROOT:-${CLAUDE_PLUGIN_ROOT}}/dist/commands/pushActivity.js" started "Capturing the scenario"`.
   ...
   **Lock in the failing scenario before fixing anything.** Do the following, in order:
   ...
   4. **If you picked a pre-existing dataset that already held other traces, load their labels now.** `fix` tails into Phase 5 at `pick-execution-mode`, skipping `pick-dataset`, so the dataset's sibling traces are never loaded for you. Run `node "${CURSOR_PLUGIN_ROOT:-${CLAUDE_PLUGIN_ROOT}}/dist/commands/readTracesBatched.js" <trace-id...> --scope full` over the dataset's trace IDs and `Read` the `outputFile` it prints, so every sibling's validated label + annotation is in context. `evaluate-results` scores each replayed trace against its label; without this load the siblings read as unlabeled skips and a regression on a previously-passing sibling goes undetected, defeating the regression guard this mode promises. **Skip this** when you just created the dataset or it held only this trace (no siblings to load).

   Then open the dataset review page so the captured red streams in live:
   ...
   Tell the user in one line: the scenario is captured as a failing test in dataset `<name>` (`<datasetId>`), and you'll now fix the code and replay the dataset to confirm. Then continue into the experiment loop, where `make-change` is the fix.
...
13. **Run only when mode is `wizard`, `dataset`, `experiment`, `cost-optimize`, `investigate` or `fix`.**
   ...
   **In `fix` mode, this run targeted THIS trace, so frame the result around it.** Lead with whether the captured trace went green (that was the goal) and call out any regressions the dataset replay surfaced. If the captured trace now passes and other dataset traces are still red, do NOT auto-loop through them: frame "Keep iterating" as an explicit offer, name how many traces are still failing and ask whether to fix those too or stop here. If the captured trace is still failing, "Keep iterating" means another fix attempt on it. The dataset now permanently guards this scenario regardless of what they pick.
