...
13. **Run only when mode is `fix`.**
   ...
   **Run only in `fix` mode.** This step is reached after replay verdicts were evaluated for a fix run.
   ...
   - **the targeted single-trace replay verdict is PASS (`label: true`)**: the target trace went green. Now capture it into a dataset as the regression test, then ask whether to re-run the full dataset, inspect the before/after, keep iterating, or stop → step 14
