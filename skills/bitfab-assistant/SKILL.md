...
18. **Run only when mode is `fix`.**
   ...
   **Idempotency guard, check first.** If `fixCaptured` is already set (an earlier pass turned the trace green, captured it, and a later iteration regressed it), the trace is already in the dataset: do **not** re-label or re-attach. Just tell the user it is already saved as a test in dataset `<name>` (`<datasetId>`) and is currently red again, then stop.
   ...
   3. **Attach the trace** with `mcp__Bitfab__add_traces_to_dataset` (`datasetId`, `[traceId]`; idempotent). **Report the tool's actual added/skipped counts**, do not claim "saved" if it skipped the trace.

   Set `fixCaptured = true`. Tell the user in one line: the still-failing scenario is saved as a failing test in dataset `<name>` (`<datasetId>`) to revisit, and the bug is **not** fixed yet (the saved red turns green only once a future fix passes on replay). Then stop.
