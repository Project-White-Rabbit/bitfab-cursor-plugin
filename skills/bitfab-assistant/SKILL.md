...
5. **Run only when mode is `wizard`, `dataset`, `experiment`, `investigate` or `benchmark`.**
   ...
   **Open the experiments page.** Pick exactly one case (they are mutually exclusive):
   ...
   `<tokensSuffix>` is a real placeholder, like `<experimentGroupId>`: resolve it from `costRun` before you open the URL. `costRun` set, substitute `&tokens=1`; `costRun` false, substitute the empty string. Because the slot is driven by the flag, the lens lands on for token/cost runs and stays off for quality runs, with nothing to remember to add on a cost run or strip on a quality run. The only way to get it wrong is to leave a literal `<tokensSuffix>` in the command.

   **Token-cost lens (`&tokens=1`).** When the URL ends in `&tokens=1` the page turns on the token-cost lens: each trace and the experiment header show the original → replay total-token trend, streaming in next to pass/fail. That is the entire effect of resolving `<tokensSuffix>` to `&tokens=1`. On a quality run (`costRun` false) it resolves to nothing and the page looks exactly as it does today, so a non-cost run never carries the lens.
...
14. **Run only when mode is `wizard`, `dataset`, `experiment`, `investigate` or `benchmark`.**
   ...
   **In `benchmark` mode without the `studio` flag, skip this step entirely** (no Studio is open). With the `studio` flag, benchmark behaves like the other modes below: skip if the experiments page was already opened via `experimentGroupId` in `open-experiments-before-replay`, otherwise navigate with the collected `testRunId`s. Either way this step's `next` goes to `evaluate-results`, which in benchmark mode scores the items, persists verdicts, and then routes to the terminal benchmark scorecard. (The numbered position of this step in the rendered list does not reflect run order: follow the `next` routing, not the list sequence.)
   ...
   In `benchmark` mode (with the `studio` flag), append `&mode=benchmark` here too so the page shows benchmark terminology. Likewise, when `costRun` is set (classified in argument routing; see the token-cost lens note in `open-experiments-before-replay`), append `&tokens=1` so the token-cost columns show, e.g. `/studio/experiments?testRunIds=<testRunId1>,<testRunId2>&tokens=1`. The command navigates an existing session or opens a new one automatically.
