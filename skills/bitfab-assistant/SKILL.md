...
5. **Run only when mode is `wizard`, `dataset`, `experiment`, `cost-optimize`, `investigate`, `benchmark` or `fix`.**
   ...
   **Before running: verify the replay script prints the full original and new output values AND each item's verdict persist key (`item.originalTraceId` on modern SDKs, or the server replay `item.traceId` on older ones) for every item** (not just lengths, counts, hashes, or truncated previews) so the run's `items/*.json` files carry them. Each item needs exactly one of the two keys, not both: modern SDKs persist by the lineage `item.originalTraceId` (older SDKs may print it under the deprecated `sourceTraceId` alias) and leave `item.traceId` null until the run completes; the oldest SDKs have no `item.originalTraceId` and persist by the server replay `item.traceId`. If it doesn't, fix the script first, the Replay Output Contract and example script live in the SDK reference at `https://docs.bitfab.ai/<language>-sdk.md`. Subagents can't evaluate an improvement from `5 → 7 (+2)`, and an item that carries no persist key at all (neither `originalTraceId` nor `traceId`) blocks verdict persistence for that item.
...
6. **Run only when mode is `wizard`, `dataset`, `experiment`, `cost-optimize`, `investigate`, `benchmark` or `fix`.**
   ...
   **Instead: tell the user what's wrong and offer exactly two workarounds.** Use use `AskUserQuestion` to surface a clear summary first, the failing trace ID(s), the error pattern, the function and span where it happens, then present the two options below. Pick a representative failing trace and call `mcp__Bitfab__get_traces` with `scope: "summary"` to read its `environment` field (production / staging / development), so option B can name the source environment concretely.
   ...
   - **high INFRA error rate (over half of items errored, and `shapeErrored` is not the dominant cause, shape mismatches take the branch above)**: signal is noisy. Flag the rate and ask the user whether to fix the env and retry, or proceed with the partial signal → step 9
   - **healthy or mixed run (at least one completed item, infra errors at most half of total)**: proceed. Carry `infraErrored` forward, surface as its own **unreplayable** bucket in the final report (the share-results step, or the benchmark scorecard's Unreplayable row in `benchmark` mode), never folded into pass/fail → step 9
...
21. **Run only when mode is `wizard`, `dataset`, `experiment`, `cost-optimize`, `investigate` or `fix`.**
   ...
   **Share results to the user.** Present the outcome, then route by the plan the user already approved (in Phase 4 diagnose, or Phase Cost for `cost-optimize`; the routing conditions are at the end of this step). Do **not** ask the user whether to keep going and do **not** offer to revert: you are either continuing through the already-approved plan or wrapping up, and neither is a prompt. What was removed is the unsolicited "want to run another?" question, **not** the execution of an approved multi-experiment plan.
   ...
   Keep `unreplayable` out of the pass-rate denominator. If `unreplayable > 0`, name the cause (missing record, write blocked, env mismatch) and note that fixing the env or trimming those trace IDs would clean up a future run. If `check-replay-health` fired in the previous iteration too, flag that infra has now blocked two runs so the user knows it needs fixing before running this again. This is a report, not a prompt: state it and let the routing conditions below decide whether to continue the approved plan or wrap up, do not stop to ask the user.
