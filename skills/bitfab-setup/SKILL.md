...
5. Summarize the setup health in one compact report:
   ...
   - **Subtree capture**: ✅ wired / ⚠️ `@bitfab/transform` installed but no adapter in the real build path / ❌ `@bitfab/transform` missing / ❌ Python below 3.12 / `n/a (no opt-out roots)` (from the subtree-capture check in step 4).
   ...
   - **Opt-out roots without subtree capture**: every trace is the root alone, so tracing looks healthy and the traces are useless for per-step diagnosis, replay mocking, and prompt iteration. For TypeScript, install `@bitfab/transform` in that workspace and wire the matching adapter into the build behind the server entrypoint, preserving existing config wrappers such as Sentry. For Python, this is a runtime upgrade to 3.12+; report it rather than attempting it.
...
7. **Apply fixes individually, confirm each before changing anything; never bundle them into one blanket change.** Go through only the items step 4 flagged as stale or missing, and for each, use `AskUserQuestion` (one decision per question) and apply only if the user approves. Skip any they decline and continue to the next.
   ...
   - **Subtree capture not wired** (TypeScript), this edits a build file, so **preview before touching code**: name the workspace, the adapter you would use, and the exact config file or start script you would change, then use `AskUserQuestion` to proceed. If yes, add `@bitfab/transform` as a development dependency in that workspace and wire the matching adapter: `@bitfab/transform/next` wrapping the existing `next.config`, the `/vite`, `/rollup`, `/rolldown`, `/esbuild`, `/bun`, `/webpack`, `/rspack`, `/rsbuild`, `/babel`, `/nest`, or `/typescript` entry in the existing build config, or `--import @bitfab/transform/register` on the real start command. Preserve every existing wrapper; never replace or bypass one. Python below 3.12 is a runtime upgrade, report it and do not attempt it.
