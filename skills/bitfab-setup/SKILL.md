...
5. Print one plain-markdown summary (no `AskUserQuestion`). Lead with a one-line count ("Uploaded N draft trace plans"), then a table or list with one row per uploaded plan: **trace function key**, **boundary** (`file:line`), a one-line **why it's worth tracing**, whether it is a **new** plan or a **diff** against existing instrumentation, and the **plan URL** (`https://bitfab.ai/studio/trace-plan/<id>`). For each **already-instrumented** candidate (one you set `alreadyTraced` on), state the delta in that row: how many **new spans** the plan recommends (captured nodes not `alreadyTraced`) and how many **already-traced spans** it would drop (`alreadyTraced` nodes left uncaptured); note "already fully traced, no changes recommended" when the recommendation matches what's there. After the list, note explicitly that:
