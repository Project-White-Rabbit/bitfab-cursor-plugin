...
6. Each round of the loop. **Every `mcp__Bitfab__get_template` and `mcp__Bitfab__update_template` call must include `traceFunctionKey: <key>`** (the key picked in step 1); without it you'd edit the org-global instead of this function's override.
   ...
   Read the background process's stdout; the `Template preview closed` line means the user clicked Close and the process has exited. **Use the same read to harvest any new `{"event":"click",...}` and `{"event":"focus",...}` JSON lines for step 1 of the next round.**
