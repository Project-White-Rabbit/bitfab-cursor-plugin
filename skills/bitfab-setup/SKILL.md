...
12. **🚨 A trace plan is delivered ONE way only: post it with `mcp__Bitfab__create_trace_plan` and open it in Studio with node "${CURSOR_PLUGIN_ROOT:-${CLAUDE_PLUGIN_ROOT}}/dist/commands/openTracePlan.js". Rendering the trace plan as a markdown table or prose in the chat is NOT a substitute and is forbidden, except as the error-only Inline fallback at the very bottom of this step (which applies only when `mcp__Bitfab__create_trace_plan` itself errors).** When the user says "create a trace plan", "give me the trace plan", "show me the trace plan", or "bitfab create a plan", that is a request to run this createTracePlan + openTracePlan path so it opens in Studio, never to type a plan into the conversation. Do not make the user separately ask you to "open it in Studio", building the plan and opening it in Studio are the same step. Build the tree in memory, then call the tools; do not print it first.
...
7. Tell the user how to run the app to generate a trace with the modified setup, exact command(s). Do NOT run it yourself. Then **MANDATORY STOP**: use `AskUserQuestion`:
   ...
   **Re-entry rule (applies after you leave this loop).** If, later in the conversation, the user asks to re-instrument or change another function's capture in plain language (`re-instrument <fn>`, `change what this span records`, `give me the updated trace plan for <fn>`), that is a fresh Modify (or Instrument) cycle: re-invoke `/bitfab-setup modify` (name the mode, so it goes straight to Modify rather than falling back to the full `wizard`) so it runs through the trace-plan UI. **Never satisfy such a request by hand-writing a trace plan or before/after diff as a chat message, that skips the Studio confirmation UI (`mcp__Bitfab__create_trace_plan` + `openTracePlan`).**
