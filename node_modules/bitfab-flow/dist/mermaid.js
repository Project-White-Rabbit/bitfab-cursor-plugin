import { branchLabel, computeModesPerStep, computeReachableSteps, nextRefsForMode, resolvePerTarget, resolveTargetableString, } from "./schema.js";
 ...
 */
export function renderMermaid(flow, mode) {
    ...
    const reachable = mode !== undefined
        ? new Set([...computeModesPerStep(flow, DIAGRAM_TARGET).entries()]
            .filter(([, modes]) => modes.has(mode))
            .map(([key]) => key))
        : computeReachableSteps(flow, DIAGRAM_TARGET);
    for (const phase of flow.phases) {
        const phaseSteps = phase.steps.filter((s) => reachable.has(`${phase.id}/${s.id}`));
        ...
        for (const step of phaseSteps) {
            ...
            const baseLabel = escapeMermaidLabel(resolveTargetableString(step.title, DIAGRAM_TARGET, "step.title"));
    ...
    const isReachable = (ref) => reachable.has(ref);
    for (const phase of flow.phases) {
        for (const step of phase.steps) {
                ...
                if (step.next === null || typeof step.next === "string") {
                    ...
                    for (const m of activeModes) {
                        ...
                        if (mode !== undefined) {
                            ...
                            lines.push(`  ${nodeId(src)} -->|"${escapeMermaidLabel(m)}"| ${nodeId(target)}`);
                        }
                    }
                }
            }
    ...
    lines.push("  classDef userDecision fill:#2a1f3a,stroke:#b58be0,color:#e8d6ff");
    lines.push("  classDef agentDecision fill:#3a2a1f,stroke:#e0a35b,color:#ffe1c7");
    return lines.join("\n");
}
/**
 * Three fork kinds:
 ...
 *     (the edges fan out labeled `all` / `plugin` / `sdk`).
 *   - **user decision** - a `branch` step whose options are tagged with
 *     `option` (rendered as A/B/C in AskUserQuestion).
