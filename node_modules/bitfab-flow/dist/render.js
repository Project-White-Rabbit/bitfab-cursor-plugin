...
/**
 * Frontmatter renderer for a flow's ENTRY artifact (the monolith file or the
 * split orchestrator). When `compile.entry` resolves to "skill" and the target
 * provides an `entryFrontmatter` renderer, the entry is emitted as a
 * model-invocable Agent Skill; otherwise it falls back to the target's normal
 * `frontmatter` (today's command-style entry).
 */
function entryFrontmatterFor(flow, target) {
    const entry = resolvePerTarget(flow.compile.entry, target.name) ?? "command";
    if (entry === "skill" && target.entryFrontmatter !== undefined) {
        return target.entryFrontmatter;
    }
 ...
 */
function renderDispatchSuffix(flow, edges, allModes) {
    if (edges.length === 0) {
        return null;
    }
    ...
    const lines = edges.map((edge) => {
        ...
        switch (edge.kind) {
            ...
            case "continue":
                ...
                break;
            case "dispatch": {
                const skill = `\`${flow.id}-${edge.targetPhase}\``;
                if (singleMode) {
...
function dispatchSuffixForUnit(flow, ctx, unitPhases) {
    return (step, stepKey) => renderDispatchSuffix(flow, computeDispatchEdges(step, stepKey, unitPhases, ctx), ctx.allModes);
}
/**
...
function renderOrchestrator(flow, target, ctx, inlineSet, subPhases, chain) {
    const inlinePhaseIds = flow.phases
        ...
        .map((phase) => phase.id);
    const sections = [
        entryFrontmatterFor(flow, target)({
            flowId: flow.id,
    ...
    for (const phase of flow.phases) {
        if (inlineSet.has(phase.id)) {
            const rendered = renderPhase(phase, target.name, ctx.allModes, ctx.modesPerStep, ctx.reachable, ctx.emitTemplate, true, chain ? dispatchSuffixForUnit(flow, ctx, inlineSet) : undefined, destinationRenderer(flow, target, inlineSet));
...
function renderSubSkill(flow, target, ctx, phase, chain) {
    ...
    const rendered = renderPhase(phase, target.name, ctx.allModes, ctx.modesPerStep, ctx.reachable, ctx.emitTemplate, false, chain ? dispatchSuffixForUnit(flow, ctx, unitPhases) : undefined, destinationRenderer(flow, target, unitPhases));
    if (rendered.length > 0) {
        sections.push(rendered);
    }
    if (chain) {
...
export function renderMarkdown(flow, target) {
    ...
    const argumentHint = flow.frontmatter.argumentHint !== undefined
        ? s(flow.frontmatter.argumentHint, target.name, "frontmatter.argumentHint")
        : undefined;
    sections.push(entryFrontmatterFor(flow, target)({
        flowId: flow.id,
        description,
        argumentHint,
        model: flow.frontmatter.model,
    ...
    const emitTemplate = flow.emitTemplate !== undefined
        ...
        : undefined;
    for (const phase of flow.phases) {
        const rendered = renderPhase(phase, target.name, allModes, modesPerStep, reachable, emitTemplate, true, undefined, destinationRenderer(flow, target));
...
destinationFor) {
    ...
    if (phase.stepStyle === "headers") {
        return renderPhaseAsHeaders(phase, visibleSteps, allModes, modesPerStep, target, emitTemplate, dispatchSuffixFor, destinationFor);
    }
    return renderPhaseAsList(phase, visibleSteps, allModes, modesPerStep, target, emitTemplate, includePhaseTitle, dispatchSuffixFor, destinationFor);
}
...
function renderPhaseAsHeaders(phase, visibleSteps, allModes, modesPerStep, target, emitTemplate, dispatchSuffixFor, destinationFor) {
    const blocks = [];
...
function renderPhaseAsList(phase, visibleSteps, allModes, modesPerStep, target, emitTemplate, includePhaseTitle, dispatchSuffixFor, destinationFor) {
    const blocks = [];
