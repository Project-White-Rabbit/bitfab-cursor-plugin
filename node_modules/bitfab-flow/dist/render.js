...
import { SECTION_TARGETS } from "./targets.js";
...
export function applyMustacheSections(text, targetName) {
    const sectionRegex = /\{\{([#^])([a-z][a-z-]*)\}\}([\s\S]*?)\{\{\/\2\}\}/g;
    return text.replace(sectionRegex, (_match, kind, name, inner) => {
        const members = SECTION_TARGETS[name];
        if (members === undefined) {
            throw new Error(`Unknown target name "${name}" in Mustache section. Valid names: ${MUSTACHE_SECTION_NAMES.join(", ")}.`);
        }
        const isMatch = members.includes(targetName);
