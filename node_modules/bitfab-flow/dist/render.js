 ...
 * entirely. Returns null when the step has nothing to dispatch.
 */
    ...
    const lines = edges.map((edge) => {
        ...
        switch (edge.kind) {
            ...
            case "dispatch": {
                ...
                if (singleMode) {
                    action = `invoke the ${skill} skill.`;
                }
                else if (edge.modes.length === 1) {
                    action = `invoke the ${skill} skill with mode \`${edge.modes[0]}\`.`;
                }
                else {
                    action = `invoke the ${skill} skill with the current mode (${modeList}).`;
                }
                break;
            }
        }
 ...
 * inline steps' own Next annotations. Returns null when every mode enters an
 * inline phase (the common case, e.g. a shared setup phase).
 */
...
function renderOrchestrator(flow, target, ctx, inlineSet, subPhases, chain) {
    ...
    for (const phase of flow.phases) {
        if (inlineSet.has(phase.id)) {
            const rendered = renderPhase(phase, target.name, ctx.allModes, ctx.modesPerStep, ctx.reachable, ctx.emitTemplate, true, chain ? dispatchSuffixForUnit(flow, target, ctx, inlineSet) : undefined, destinationRenderer(flow, target, inlineSet));
...
function renderModesDispatch(flow, target, dispatches) {
    ...
    const routing = flow.defaultMode !== undefined
        ? `Read \`$ARGUMENTS\` first. If its first token is exactly one of the mode names below, run that mode. Otherwise, when this skill documents how to route the remaining arguments (see its intro), follow that; if it doesn't, run \`${flow.defaultMode}\` and treat \`$ARGUMENTS\` as its input. ${follow}`
        : `Read \`$ARGUMENTS\` first. If its first token is exactly one of the mode names below, run that mode; otherwise route it using this skill's argument-routing guidance. ${follow}`;
    return [
        "## Modes",
        "",
        routing,
...
stepNumberOffset = 0) {
    ...
    if (phase.stepStyle === "headers") {
        return renderPhaseAsHeaders(phase, visibleSteps, allModes, modesPerStep, target, emitTemplate, dispatchSuffixFor, destinationFor, stepNumberOffset);
    }
...
function applyTokens(text, target, flow) {
    ...
    if (leftovers) {
        throw new Error(`Unresolved placeholder(s): ${leftovers.join(", ")}. Check spelling and namespace (step:, step-ref:, tool:, languages:, branches, branches:).`);
    }
    return result;
}
