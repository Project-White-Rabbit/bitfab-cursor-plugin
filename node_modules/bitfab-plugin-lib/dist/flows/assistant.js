...
import { STUDIO_REUSE_NOTE } from "./studioRules.js";
/**
...
- If a command exits quickly with a completion or error JSONL line, handle it immediately. If it remains running after the needed handshake, leave it in the background and move on.{{/codex}}`,
    ...
    defaultMode: "wizard",
    // Rich per-mode hints: the auto-generated Modes table is this flow's only mode
    // listing (monolith, no `## Dispatch`), so each hint carries the arg shape plus
    // the action. The fuller disambiguation (bare-UUID routing, the `studio`
    // opt-in) stays in the **Argument routing** prose below the intro.
    modeHints: {
        wizard: "`[<key>]` or a bare `<key>`. Full flow: pick function -> verify instrumentation -> pick or create dataset -> label -> diagnose -> iterate -> wrap up. A lone trace UUID is never `wizard` (it routes to `replay` or `fix` by intent, see argument routing).",
        dataset: "`<key>`. Build or extend a labeled dataset for one function, then diagnose failures and iterate with experiments. Picks an existing dataset or creates a new one.",
    ...
    compile: {
        // Claude compiles to split-chain: an orchestrator + one sub-skill per phase
        // with tail-call dispatch, so each mode loads only the orchestrator plus the
        // phases its chain reaches (e.g. `replay` pulls quick-replay + cleanup, not
        // the 50KB iterate phase). Cursor and Codex stay monolith (default). The
        // split path is exercised by a test (see assistant.test.ts "split-chain
        // compatibility") so it can't silently rot. Nothing is inlined: cleanup,
        // identify-function, and every shared phase are re-entered across mode
        // chains, so under split-chain each must be its own sub-skill (an inline
...
{{whens}}`,
                    branches: [
                        ...
                        {
                            when: "captured ReplayResult is empty (`resultCaptured` is true, child exited 0, and `total === 0`)",
                            description: `the command itself succeeded, but no traces were replayed. Report the empty result, diagnose the dataset/filter/script, and retry without calling it a command crash`,
                            next: "iterate/replay-against-dataset",
                        },
                        {
                            when: "replay command exited 0 but local ReplayResult capture is unavailable (`resultCaptured` is false), even if progress lines were captured, and `serverRecovered` is not true",
                            description: `do not call the replay failed or completed. Progress lines alone do not verify the result; the command succeeded but the replay outcome is unverified until the server resolves it`,
                            next: "iterate/resolve-unverified-replay",
                        },
                        {
                        ...
                        },
                        {
                            ...
                            next: "iterate/check-verdict-persistence",
                        },
                        {
                ...
                {
                    id: "resolve-unverified-replay",
                    kind: "branch",
                    toolCalls: ["getReplayStatus", "getExperimentTraces", "readTraces"],
...
When the server reports \`completed\` with \`traceCount > 0\`, call {{tool:getExperimentTraces}} for the same test run to enumerate every server replay trace ID and its \`original\` trace ID, then call {{tool:readTraces}} with the server replay IDs and \`scope: "full"\` to recover their inputs, outputs, and errors. Use each experiment trace's \`original\` field as its \`originalTraceId\` lineage key. Do not use the keys of {{tool:getReplayStatus}}'s \`traceIds\` mapping as original trace IDs: those keys are local client \`sourceTraceId\` values. Recompute \`completed\`, \`shapeErrored\`, \`infraErrored\`, and \`canPersistVerdicts\` from those server traces exactly as \`replay-against-dataset\` does, set \`serverRecovered = true\`, then continue normally. Do not infer success from the child exit code or local progress lines.

{{whens}}`,
                    branches: [
                        {
                            when: "server status is `completed` and `traceCount > 0`",
                            description: "the server confirms the replay completed. Route the recovered counts through the normal shape and infra health checks",
                            next: "iterate/check-replay-health",
                        },
                        {
...
{{whens}}`,
                    branches: [
                        {
                            when: '`status: "ok"` AND `persistedIds` (plus the unreplayable `item.error` bucket) account for every completed replay item',
                        ...
                        {
                            when: '`status: "ok"` but a completed (`item.error` unset) replay item is absent from `persistedIds` (your own coverage cross-check failed: the `expected*` list was under-scoped)',
                            description: "the script passed only because you handed it a truncated `expected*` list; the report would be silently empty or partial. Go back to `evaluate-results`, rebuild the verdicts file so the `expected*` list is the full set of completed replay items, and re-persist. Do NOT proceed to the report",
                            next: "iterate/evaluate-results",
                        },
                        {
                            when: '`status: "missing-coverage"` (script returned a non-empty `missingIds` array)',
                            description: "you under-verdicted. Read the traces behind the missing ids, which are keyed by the run's `idKind` (in lineage mode each `missingIds` entry is the `originalTraceId` of the trace replayed from; in direct mode it is the replay trace's own server id): run `{{command:readTracesBatched}} <trace-id...> --scope full` and `Read` its `outputFile` if you didn't already, since you are persisting pass/fail verdicts on their span content. Decide each one (PASS / FAIL with annotation, or `skip: true` if genuinely ambiguous), write a NEW verdicts file at the same path covering ALL the originally expected ids (the script needs the full `expected*` list each call, not just the gaps), and re-run the script. Loop back here with the new result",
                            next: "iterate/verify-replay-labels",
                        },
                        {
                        ...
                        {
                            when: '`status: "verification-failed"` (the readable write result did not show the expected labels)',
                            description: "the write returned but the `update_agent_labels` output did not include the expected effective label lines for the replay items. Do NOT report a scorecard. Read the script's `message`, `missingIds`, and `mismatchedIds` (keyed by the run's `idKind`: `originalTraceId` in lineage mode, `traceId` in direct), rerun the same verdict file after fixing the issue if needed, and loop back here. If it keeps failing, stop and surface the verification details",
                            next: "iterate/verify-replay-labels",
                        },
                    ],
                },
...
{{whens}}`,
                    branches: [
                        ...
                        {
                            when: "the targeted single-trace replay verdict is FAIL (`label: false`), skipped, unreplayable, shape-incompatible, or could not be persisted because the item carried no persist key (neither the lineage `originalTraceId` nor a direct server `traceId`)",
...
**Otherwise ask the user where to save this scenario.** This is the only place the fix flow decides dataset membership, so **always ask** so the user sees what's happening and stays in control, but **keep the prompt to at most four options, never one option per dataset** (that overflows the picker and overwhelms the user once a few datasets exist). First, if the user named a specific dataset ID in their invocation, skip the prompt and use it (confirm it's scoped to this function via {{tool:listDatasets}}; if it belongs to a different function, {{tool:addTracesToDataset}} would silently skip every trace, so {{askVerb}} and offer a correctly-scoped dataset instead). Otherwise call {{tool:listDatasets}} with the function key and present {{presentMode}}, choosing options by how many datasets exist:
...
- **"Choose a different dataset"** — **include this only when two or more datasets exist** (with exactly one, the first option already covers it), and place it **right after** the recommended dataset so the two dataset-picking choices sit together. On pick, present a second {{presentMode}} listing existing datasets scoped to this function (name · id · count, most recently used first, capped to what the picker holds) so the user can route to a specific one.
- **"Create a new dataset"** — on pick, silently call {{tool:createDataset}} with \`traceFunctionKey: <key>\` and \`name\` set to \`<key>\` when none exist yet, else \`"<key> #N"\` (N one more than the existing count); don't ask for a name. Mark this recommended when no dataset exists yet.
                    ...
                    branches: [
                        ...
                        },
                        {
                            option: {
                                ...
                                description: 'clear `fixDatasetSkipped` if it was set (this is a fresh attempt, so a later green should re-offer the save prompt the user declined only on the previous pass), set `fixReplayScope = "single-trace"`, set `fixSkipMakeChange = false`, and make another change against the target trace',
                            },
                            next: "iterate/open-experiments-before-replay",
                        },
                        {
                            option: {
                                ...
                                description: "stop here and leave the dataset as it is: the saved trace stays if one was added (`fixAddedToDataset`), and nothing is saved if the user continued without adding (`fixDatasetSkipped`)",
                            },
...
2. **Pick or create a dataset:** if the user named a dataset ID, use it (first confirm it's scoped to this function via {{tool:listDatasets}}; if it belongs to a different function, {{tool:addTracesToDataset}} would silently skip every trace, so {{askVerb}} and offer a correctly-scoped dataset instead). Otherwise call {{tool:listDatasets}} with the function key and ask where to save, **keeping the prompt to at most four options, never one per dataset**: **none exist** → create silently; **exactly one** → offer "Add to \`<name>\`" (recommended) and "Create a new dataset"; **two or more** → offer, in this order, "Add to \`<recommended>\`" (the most recently used dataset with traces), "Choose a different dataset" (which then presents a second {{presentMode}} listing existing datasets, most recently used first, so the user can route to a specific one), then "Create a new dataset". Hold the \`datasetId\`.
...
**Quick health check.** If the replay crashed (non-zero exit, no items) or the single item has \`item.error\` set, hold the error for the verdict step. Otherwise hold the completed item's new output alongside the original output you read in \`setup\`.`,
                ...
                },
                {
                    ...
                    body: `**Compare the single replay result to the original, report one line, then persist that verdict onto the replay trace.**
