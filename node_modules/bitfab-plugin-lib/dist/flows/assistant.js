...
| \`benchmark\` | \`{{cmd}}assistant benchmark <key> [<dataset-id>]\` | **No edits to the function under test.** Replay a labeled dataset against the current code as-is, evaluate each trace against its labels, and report a pass/fail scorecard, then stop. Use when the user wants to *measure the current code* (regression test, baseline, score after unrelated changes), not improve it. Infra fixes that unblock the replay (SDK / replay-script upgrade, \`mockOnReplay\` on a failing span) are allowed — they don't change the behavior being measured; what benchmark never does is make experiment-style edits to the traced function. If \`<dataset-id>\` is omitted, you'll be asked to pick one |
...
If a \`v<X> available\` upgrade notice appeared, pass it through to the user verbatim, but don't block on it; surface the notice once and move on. Ignore the authentication status (Studio handles login inline).`,
        ...
        },
        {
            ...
            intro: `The Studio is the companion browser surface for the entire assistant flow. It opens once at the start and stays open throughout all phases. Individual phases navigate the Studio to the relevant page (dataset review, experiment viewer, etc.) using \`openStudioTo.js\`.
            ...
            steps: [
                {
                    ...
                    body: `Open Studio at the initial path for this mode. \`openStudioTo.js\` is the single entry point for all Studio operations: it navigates an existing session or opens a new one automatically.
                    ...
                    branches: [
                        {
                            ...
                            description: `tell the user the function has no usable dataset yet and recommend running \`{{cmd}}assistant dataset <key>\` first; kill the Studio background process if one was opened (none in benchmark mode); then stop the flow`,
                            next: "cleanup/close-studio",
                        },
                        {
