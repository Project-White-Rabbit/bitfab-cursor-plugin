...
**MCP tools:** This skill uses \`list_trace_functions\`, \`search_traces\`, \`read_traces\`, \`update_agent_labels\`, \`list_datasets\`, \`create_dataset\`, \`add_traces_to_dataset\`, and \`remove_traces_from_dataset\` from the **local plugin MCP server** (bundled with this plugin){{#claude}}. Do NOT use the remote Bitfab MCP tools (\`mcp__Simforge__*\` or \`mcp__Bitfab__*\`) — use only the \`mcp__plugin_bitfab_Bitfab__*\` variants.{{/claude}}{{^claude}}, exposed under the \`mcp__Bitfab__*\` prefix.{{/claude}}
...
| Invocation | Action |
|---|---|
| \`{{cmd}}assistant studio\` | Full flow with Studio |
| \`{{cmd}}assistant studio dataset <key>\` | Dataset mode with Studio |
| \`{{cmd}}assistant studio experiment <key>\` | Experiment mode with Studio |
...
🚨 **Blocking-process rule (applies to any plugin command described as "blocks until the user does X"):** When you launch a plugin CLI that blocks on a browser handoff (\`startDataset.js\`, \`login.js\`, etc.), you MUST keep the exec session alive and keep polling it until the process exits on its own.
...
{{whens}}`,
                    branches: [
                        ...
                        },
                        {
                            ...
                            description: "continue to the Studio gateway (which opens Studio if studioMode is true, or passes through if false). If an upgrade notice appeared, pass it through to the user verbatim, but don't block on it; surface the notice once and move on",
                            next: "studio/open",
                        },
                    ],
                },
        ...
        },
        {
            ...
            intro: `**Only active when \`studioMode = true\`.** If \`studioMode\` is false, skip this phase entirely.
            ...
            steps: [
                {
                    ...
                    kind: "action",
                    toolCalls: ["bash"],
                    title: "Open the Studio (or skip)",
                    body: `**If \`studioMode\` is false**, skip this step entirely and proceed to the next step (the mode-based routing below handles it).

**If \`studioMode\` is true**, start the Studio as a long-running background process:

\`\`\`bash
node "{{pluginRoot}}/dist/commands/openStudio.js"
\`\`\`
...
> 🚨 **HARD RULE — DO NOT SKIP:** All approved candidate trace IDs MUST be attached to the dataset before opening the page. The page reviews the dataset's contents, not the trace function's label table. An empty dataset means an empty review.`,
                ...
                },
                {
                    ...
                    body: `Open the dataset review page for the user. The mechanism depends on whether Studio mode is active.
...
\`\`\`bash
node "{{pluginRoot}}/dist/commands/startDataset.js" <functionKey> <datasetId>
\`\`\`
...
After opening, tell the user you've opened the dataset page and are waiting for them to finish reviewing. Then proceed to \`await-event\`.`,
                ...
                },
                {
                    ...
                    body: `**Wait for user to finish dataset review.** Read events from the background process. The event source depends on which mode is active.
...
{{whens}}`,
                    branches: [
                        {
                            when: "`event: edit-with-agent` (Studio) or `event: modify` (non-Studio)",
                        ...
                        {
                            when: "`event: return-to-agent` (Studio) or `event: saved` (non-Studio)",
                        ...
                        {
                            when: "`event: session-ended` (Studio) or `event: cancelled` (non-Studio)",
                            description: "user closed Studio or cancelled. Stop the flow",
                            next: null,
                        },
                    ],
                },
...
Unapproved agent labels do **not** satisfy this gate by design — \`validated: true\` excludes them.`,
                ...
                },
                {
                    ...
                    body: `**Hold in-context** — This approved dataset is the benchmark for all experiments in Phase 5. Keep both the \`datasetId\` and the trace IDs in your working context throughout. In \`dataset\` mode, stop here: if \`studioMode\` is true, kill the Studio background process (send SIGINT or abort the background task). Surface the dataset summary (including the id) and exit so they can pick up later with \`{{cmd}}assistant experiment <key> <datasetId>\`.`,
                    next: {
                        byMode: {
                    ...
                    branches: [
                        {
                            ...
                            description: `tell the user the function has no usable dataset yet and recommend running \`{{cmd}}assistant dataset <key>\` first; if \`studioMode\` is true, kill the Studio background process; then stop the flow`,
                            next: null,
                        },
                        {
...
\`\`\`bash
node "{{pluginRoot}}/dist/commands/openExperiments.js" <testRunId1>,<testRunId2>,<testRunId3>
\`\`\`
