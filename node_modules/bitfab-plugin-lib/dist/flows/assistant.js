...
- **Trace function key** — the non-UUID slug, if the user gave one. Datasets are scoped per function, so the key is required and **all trace IDs in the batch must belong to the same function** ({{tool:addTracesToDataset}} silently skips IDs whose function key doesn't match the dataset, so a mixed batch would partially fail with no obvious error). Resolve and verify the key up front:
  - Call {{tool:readTraces}} with \`scope: "summary"\` on **every** trace ID in the batch, not just the first (batch in groups of 10 — the tool's per-call cap — for larger sets), and read each trace's \`traceFunctionKey\`.
...
2. Call {{tool:readTraces}} with \`scope: "summary"\` on the resulting trace IDs so the labels + annotations are in working context. Don't re-label them — these are already validated.
...
Then return to the await-event step. If the user wants more edits, they'll ask again and you'll re-enter this step.`,
                ...
                },
                {
                    ...
                    body: `**Build the dataset** — You already know the trace IDs in this dataset (you attached them in earlier steps and tracked any add/remove from modify rounds). Call {{tool:readTraces}} with all of them and \`scope: "full"\` to load the labels + annotations into context. This is the working set for confirm + every Phase 5 experiment.`,
...
\`\`\`bash
{{command:persistReplayLabels}} .bitfab/tmp/verdicts-<testRunId>.json
\`\`\`
