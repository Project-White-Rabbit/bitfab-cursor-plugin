...
import { SKIP_ASSERTION_JUDGING_FLAG } from "./replayFlags.js";
/**
...
**Product pages:** Link to the existing app pages for experiments, datasets, and other dashboard features. Use /plugin only for automatic-close login and template previews. Page commands print one JSON line with \`event: "link"\` and \`url\`, then exit. Relay the URL in chat. The user reports review decisions in chat; fetch current saved state with MCP before continuing. Never treat printing a link or closing a browser tab as approval.`,
    entries: {
        ...
        "cost-optimize": {
            // Mirrors `experiment` entry: open Bitfab, then pick an existing dataset.
        ...
        benchmark: {
            // Route through page/open so the `page` opt-in can open Bitfab; the
            // step does nothing (no command) and falls through to pick-dataset when the
            // flag is absent.
            default: "auth/login",
            codex: "preamble/resolve-plugin-dir",
            amp: "preamble/resolve-plugin-dir",
        ...
        fix: {
            ...
            // it passes add it to a dataset, then ask whether to open Bitfab for
    ...
    // the action. The fuller disambiguation (bare-UUID routing, the `page`
            ...
            steps: [
                {
                    ...
                    body: `First, resolve \`BITFAB_PLUGIN_DIR\` if it isn't already exported in this shell. Run this block verbatim, it auto-detects whichever install is active:
...
If the key is NOT found in the codebase, the function is instrumented elsewhere (the traces exist on Bitfab). {{#ask-user-question}}Use \`AskUserQuestion\` to ask:{{/ask-user-question}}{{^ask-user-question}}Ask:{{/ask-user-question}}
                    ...
                    branches: [
                        ...
                        },
                        {
                            option: {
                                ...
                                description: "seed the dataset with these traces, label them, and iterate against them later",
                            },
                            recommended: true,
                            next: "dataset/list-datasets",
                        },
                    ...
                    branches: [
                        ...
                        },
                        {
                            option: {
                                ...
                                description: "broad sample with no hypothesis; ignore how earlier traces were labeled",
                            },
                    ...
                    branches: [
                        {
                            option: {
                                ...
                                description: "search for more traces and label them, on top of the reused set",
                            },
                            next: "dataset/ask-search-intent",
                        },
                        {
...
Then return to the await-event step. If the user wants more edits, they'll ask again and you'll re-enter this step.`,
                ...
                },
                {
                    ...
                    body: `**Build the dataset**: You already know the trace IDs in this dataset (you attached them in earlier steps and tracked any add/remove from modify rounds). Load each trace's label + annotation + evidence into context. **Which tool depends on \`costRun\`:**
...
Unapproved agent labels do **not** satisfy this requirement by design, \`validated: true\` excludes them.`,
                    branches: [
                        {
                            when: "check fails (no validated failing label, search returns nothing)",
                            description: "tell the user and loop back to find or label more unlabeled traces",
                            next: "dataset/find-unlabeled",
                        },
                        {
                            when: "check passes (at least one validated failing label)",
...
If the dataset has no validated labels (only the \`≥1 trace\` minimum), say so in this confirmation: the token delta is still measured, but pass-rate is then a weaker guard, recommend labeling a few traces (\`{{cmd}}assistant dataset <key>\`) for a stronger guard. Get the user's confirmation, then continue to \`load-dataset/pick-execution-mode\`, which picks parallel vs serial execution and runs the first experiment.`,
        ...
        },
        {
            ...
            intro: `Entry for \`experiment\`, \`cost-optimize\`, and \`benchmark\` modes, which skip the function picker and the dataset-building phases. Pick the dataset to run against, locate the code, and (in \`experiment\` / \`cost-optimize\` mode) choose parallel vs serial execution, then continue into Phase 5's replay loop at \`detect-replay-capabilities\`. \`cost-optimize\` first detours through its cost-diagnosis phase (\`cost/diagnose\`) between dataset-pick and execution-mode. \`wizard\` / \`dataset\` / \`investigate\` modes never pass through here, they reach \`detect-replay-capabilities\` directly from Phase 4. \`fix\` mode tails in at \`pick-execution-mode\` (it already resolved its code and diagnosed the failure in Phase Fix, so it skips \`pick-dataset\`; the dataset is picked or created later, in \`fix-add-to-dataset\`, only after the target trace passes).`,
            stepStyle: "list",
            steps: [
                {
                    id: "pick-dataset",
...
- Unless the previous bullet told you to reuse the existing fix, {{#ask-user-question}}use \`AskUserQuestion\` to explain{{/ask-user-question}}{{^ask-user-question}}explain to the user{{/ask-user-question}} what you're changing and why, and confirm before editing
...
{{whens}}`,
                    branches: [
                        ...
                        {
                            when: "every item failed closed on the same selected-mock mismatch (`completed` is 0 and `mockErrored === total`)",
                            description: `report the span/tree/occurrence mismatch and source traces. Fix or move the mock boundary with the user before retrying; never suggest changing deployment environment`,
                            next: "iterate/replay-against-dataset",
                        },
                        {
                            when: "every item has replay setup/lease or dependency/config errors (`completed` is 0 and `setupErrored + dependencyErrored === total`)",
                            description: `report the structured mix and exact codes/messages. Diagnose the matching setup or dependency problem, confirm its fix with the user, then retry`,
                            next: "iterate/replay-against-dataset",
                        },
                        {
                            when: "every item threw from the changed replay root (`completed` is 0 and `codeErrored === total`)",
                            description: `report this as a changed-code failure, not infrastructure. Diagnose and confirm a code fix with the user before retrying`,
                            next: "iterate/replay-against-dataset",
                        },
                        {
                            when: "high non-shape error rate (over half of items are in `mockErrored + setupErrored + dependencyErrored + codeErrored`, and `shapeErrored` is not dominant)",
                            description: `signal is noisy. Show each error bucket separately and ask whether to address its concrete cause and retry or proceed with the partial completed signal; never prescribe a generic environment fix`,
                            next: "iterate/check-verdict-persistence",
                        },
                        {
                            when: "healthy or mixed run (at least one completed item, non-shape errors at most half of total)",
                            description: `proceed. Carry \`mockErrored\`, \`setupErrored\`, \`dependencyErrored\`, and \`codeErrored\` forward with their distinct causes; surface their sum as **unreplayable** in the final report (the share-results step, or the benchmark scorecard's Unreplayable row in \`benchmark\` mode), never fold errored items into pass/fail`,
                            next: "iterate/check-verdict-persistence",
                        },
                    ],
                },
...
**If you're a worktree subagent** (parallel mode from \`pick-execution-mode\`): after the script returns, hand the parsed result + \`experimentId\` + unreplayable list back to the main agent and exit. The main agent collects results from all parallel experiments before \`open-experiments\`.{{/claude}}`,
                ...
                },
                {
                    ...
                    body: `**Verify replay labels persisted.** Route on the \`status\` field of the JSON the script printed in \`evaluate-results\`. The script is the deterministic check, if it didn't return \`ok\`, the agent's verdicts are NOT yet on the replay traces and the experiment delta will be wrong on the next iteration.
                    ...
                    branches: [
                        ...
                        },
                        {
                            option: {
                                ...
                                description: "make another change and replay this trace again",
                            },
                            next: "iterate/open-experiments-before-replay",
                        },
                        {
                            option: {
                                ...
                                description: "stop here and leave the dataset as it is",
                            },
...
{{whens}}`,
                ...
                },
                {
                    ...
                    emit: "Running replay",
                    body: `**Run the replay against the one trace ID. No user interaction, and no extra flags beyond the one below.** Invoke the SDK-installed executable with the registry and pipeline located in \`setup\`:
            ...
            steps: [
                {
                    ...
                    body: `**Summary.** {{#ask-user-question}}Use \`AskUserQuestion\` to present{{/ask-user-question}}{{^ask-user-question}}Present{{/ask-user-question}} the final results similar to this. You may expand where appropriate based on context from the user:
            ...
            steps: [
                {
                    ...
                    emit: { displayName: "Done", action: "completed" },
                    body: `**Benchmark scorecard.** Present the results of replaying the dataset against the current code (no changes were made). {{#ask-user-question}}Print the scorecard as Markdown directly in chat (do NOT use \`AskUserQuestion\`, this is a terminal report with no decision to make, and tables don't render inside the question UI).{{/ask-user-question}}{{^ask-user-question}}Print the scorecard as Markdown directly in chat.{{/ask-user-question}} Use two tables.
