...
Then return to the await-event step. If the user wants more edits, they'll ask again and you'll re-enter this step.`,
                ...
                },
                {
                    ...
                    body: `**Build the dataset**: You already know the trace IDs in this dataset (you attached them in earlier steps and tracked any add/remove from modify rounds). Call {{tool:readTraces}} with all of them and \`scope: "full"\` to load the labels + annotations into context. The tool caps each call at 10 IDs, so when the dataset has more than 10 traces, issue the calls **in parallel in a single turn** (one message, several {{tool:readTraces}} tool calls) rather than one batch after another. You must make ⌈N/10⌉ calls in total; put **all** of them in that one message. Do **not** send any {{tool:readTraces}} call alone in its own message, and do **not** wait for one call's result before sending the next: a lead-off solo call followed by the rest is the exact mistake to avoid. **Re-read them fresh here even if you read them earlier in this phase:** the Studio labeling review persists human approvals and label/annotation edits to the DB, so cached context from the find / label steps can be stale. This is the working set for confirm + every Phase 5 experiment.`,
