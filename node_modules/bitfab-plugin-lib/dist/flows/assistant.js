...
Get the user's confirmation before proceeding.`,
        ...
        },
        {
            ...
            intro: `Entry for \`experiment\` and \`benchmark\` modes, which skip the function picker and the dataset-building phases. Pick the dataset to run against, locate the code, and (in \`experiment\` mode) choose parallel vs serial execution, then tail into Phase 5's replay loop at \`detect-replay-capabilities\`. \`wizard\` / \`dataset\` / \`investigate\` modes never pass through here, they reach \`detect-replay-capabilities\` directly from Phase 4.`,
            stepStyle: "list",
            steps: [
                {
                    id: "pick-dataset",
                    ...
                    body: `The trace function key comes from the argument and no prior phase has run. Pick the dataset to run against (\`experiment\` mode iterates against it; \`benchmark\` mode replays it once to measure the current code), then locate the code:
...
{{whens}}`,
                    branches: [
                        {
                            when: "no datasets exist for this function (`list_datasets` returned empty), or the picked dataset fails the mode's usability gate (experiment: no validated failing labels; benchmark: no traces at all)",
                        ...
                        {
                            when: "dataset loaded (experiment: ≥1 validated failing label; benchmark: ≥1 trace)",
                            description: `summarize the dataset for the user (counts of pass/fail) and the failure annotations. In \`experiment\` mode, pick a first experiment from the failure patterns. In \`benchmark\` mode, confirm the dataset and proceed to replay the full set`,
                            next: {
                                byMode: {
