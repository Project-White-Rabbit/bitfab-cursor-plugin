...
- When the process exits, immediately continue with the next step in the flow, do not wait for another user message.{{/codex}}`,
    entries: {
        ...
        fix: {
            // Full-flow mode: open Studio, then route (via studio/open's byMode) to
            // the fix phase, which captures the failing trace as a validated red and
            // tails into the Phase 5 experiment loop.
            default: "studio/open",
            codex: "preamble/resolve-plugin-dir",
        },
    },
...
{{branches}}`,
                ...
                },
                {
                    ...
                    body: `**Upgrade the SDK and replay script.** The replay script references SDK APIs (\`experimentGroupId\`, \`codeChangeDescription\`, per-item \`traceId\`, \`adaptInputs\` / \`adapt_inputs\`) that require a recent SDK. Upgrade the SDK first, then regenerate the script.
...
Hold the trace ID, function key, code path, and the trace's failure details in working context.`,
                ...
                },
                {
                    ...
                    commandCalls: ["openStudio", "readTracesBatched"],
...
{{whens}}`,
        ...
        },
        {
            ...
            intro: `In \`experiment\` mode this is an iterative improvement loop (each iteration makes a change and replays). In \`benchmark\` mode it is a single replay of the current code followed by a terminal scorecard, no changes, no iteration.
...
**3. Re-check capabilities.** After upgrading and editing, re-run \`{{command:detectCapabilities}}\` and re-read the \`capabilities\` object for this SDK (the probe now sees the upgraded version). Combine again with the script-side grep from step 2 and update the flags in working context. If any are still missing after the upgrade, note it but continue.`,
                ...
                },
                {
                    ...
                    toolCalls: ["bash"],
                    commandCalls: ["openStudio"],
                    title: "Open experiments page before making changes",
                    body: `**Generate the experiment group ID and open the experiments page before making changes or running replay.** This lets the user watch results stream in live from the moment replay starts.
...
- {{^codex}}Use \`AskUserQuestion\` to explain{{/codex}}{{#codex}}Explain to the user{{/codex}} what you're changing and why, and confirm before editing
...
- If a file is newly created, the before snapshot is the empty string \`""\`. If a file is deleted, the after snapshot is \`""\`. The path is always the repo-relative file path, no \`repo\`, \`commit\`, or other context fields`,
                ...
                },
                {
                    ...
                    kind: "action",
                    toolCalls: ["bash"],
                    title: "Replay against the dataset",
                    emit: "Running replay",
                    body: `**Replay against the dataset.** Collect the trace IDs from the labeled dataset (built in Phase 3 in \`wizard\` and \`dataset\` modes, or rehydrated in Phase 5 Setup's \`pick-dataset\` step in \`experiment\` and \`benchmark\` modes). The experiment group ID was already generated in the \`open-experiments-before-replay\` step (which also opened the experiments page in Studio in non-benchmark modes, and in \`benchmark\` mode only when the \`studio\` flag was passed).
...
cd <project-dir> && {{command:replayProgress}} --label <pipeline-name> -- npx tsx scripts/replay.ts <pipeline-name> --dataset-id <datasetId> --code-change /tmp/bitfab-code-change-<experimentN>.json --experiment-group-id <experimentGroupId>
# Fallback (older script/SDK without dataset-id support): pass the dataset's resolved trace IDs
cd <project-dir> && {{command:replayProgress}} --label <pipeline-name> -- npx tsx scripts/replay.ts <pipeline-name> --trace-ids <id1>,<id2>,<id3>,... --code-change /tmp/bitfab-code-change-<experimentN>.json --experiment-group-id <experimentGroupId>
\`\`\`
                    ...
                    branches: [
                        {
                            option: {
                                ...
                                description: "run another experiment from the plan",
                            },
                            recommended: true,
                            next: "iterate/open-experiments-before-replay",
                        },
