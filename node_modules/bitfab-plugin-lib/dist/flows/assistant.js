...
{{whens}}`,
                ...
                },
                {
                    ...
                    body: `**Ask what kinds of traces to find** — Before searching, find out what the user is actually trying to surface. The trace function may have thousands of traces; "what should I label?" is the question that makes the rest of this phase useful.
...
- Spill to a tmp file if context gets big. Keep the \`unreplayable\` list (trace ID + error string) alongside pass/fail for \`share-results\`.{{#claude}}
- **If you're a worktree subagent** (parallel mode from \`pick-execution-mode\`): return your scored items + \`testRunId\` + unreplayable list to the main agent and exit. The main agent collects results from all parallel experiments before \`open-experiments\`.{{/claude}}`,
