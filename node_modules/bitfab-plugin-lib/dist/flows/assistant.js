...
|---|---|---|
| \`wizard\` | \`{{cmd}}assistant\`, \`{{cmd}}assistant wizard [<key>]\`, or \`{{cmd}}assistant <key>\` (\`<key>\` is a function key, a lone trace UUID is NOT a key and routes to \`replay\` instead) | Full flow: pick function → verify instrumentation → pick or create dataset → label → diagnose → iterate → wrap up |
...
\`\`\`bash
{{command:openStudio}} <path> --monitor
\`\`\`
...
**The \`--monitor\` flag is load-bearing and belongs ONLY on this step.** This is the one step that establishes the durable event loop for the whole run. \`--monitor\` makes it the single monitor whether it opens a fresh window OR reuses one left over from a prior run, without it, reusing an existing window would navigate-and-exit and the run would have NO monitor, silently missing Done / Edit-with-agent / session-ended. Every later \`{{command:openStudio}}\` call (dataset page, experiments page, trace plans) is a plain navigation: omit \`--monitor\` so it fires-and-exits instead of spawning a duplicate poller.
...
With \`--monitor\` this command enters an event loop and stays running for the whole session, whether it opened a fresh window or reused an existing one. {{#claude}}Run it with \`run_in_background: true\` on the Bash tool. **Do NOT append \`&\` to the command string.**{{/claude}}{{#cursor}}Run it as a long-running terminal process.{{/cursor}}{{#codex}}Run it via your runtime's "long-running exec session" mechanism.{{/codex}} It is the single backgrounded process for the run; the later navigation steps (without \`--monitor\`) exit immediately.
...
If the dataset has no validated labels (only the \`≥1 trace\` minimum), say so in this confirmation: the token delta is still measured, but pass-rate is then a weaker guard, recommend labeling a few traces (\`{{cmd}}assistant dataset <key>\`) for a stronger guard. Get the user's confirmation, then continue to \`load-dataset/pick-execution-mode\`, which picks parallel vs serial execution and runs the first experiment.`,
        ...
        },
        {
            ...
            intro: `Entry for \`experiment\`, \`cost-optimize\`, and \`benchmark\` modes, which skip the function picker and the dataset-building phases. Pick the dataset to run against, locate the code, and (in \`experiment\` / \`cost-optimize\` mode) choose parallel vs serial execution, then tail into Phase 5's replay loop at \`detect-replay-capabilities\`. \`cost-optimize\` first detours through its cost-diagnosis phase (\`cost/diagnose\`) between dataset-pick and execution-mode. \`wizard\` / \`dataset\` / \`investigate\` modes never pass through here, they reach \`detect-replay-capabilities\` directly from Phase 4.`,
            stepStyle: "list",
            steps: [
                {
                    id: "pick-dataset",
