...
export const assistantFlow = Flow.parse({
    ...
    emitTemplate: '**Studio activity:** If `studioMode` is true, run `node "{{pluginRoot}}/dist/commands/pushActivity.js" <sessionId> {action} "{displayName}"`.',
    tools: {
...
| \`experiment\` | \`{{cmd}}assistant experiment <key> [<dataset-id>]\` | Run experiments to fix failing traces against a labeled dataset, then wrap up. If \`<dataset-id>\` is omitted, you'll be asked to pick one. If the function has no datasets yet, run \`{{cmd}}assistant dataset <key>\` first |
...
- When the process exits, immediately continue with the next step in the flow, do not wait for another user message.{{/codex}}`,
    ...
    phases: [
        {
            id: "phase-0",
            title: "Phase 0: Status + Update Check",
            stepStyle: "list",
            steps: [
                {
...
${CODEX_RESOLVE_PLUGIN_DIR_BASH}`,
                ...
                },
                {
                    ...
                    kind: "branch",
                    toolCalls: ["bash"],
                    title: "Run the status command",
                    body: `Then run the status command:
...
{{whens}}`,
                    branches: [
                        {
                            ...
                            description: "stop the flow immediately. Tell the user to run `{{cmd}}setup login` first",
                            next: null,
                        },
                        {
                            when: "authenticated (with or without a `v<X> available` upgrade notice)",
                            description: "continue to the Studio gateway. If an upgrade notice appeared, pass it through to the user verbatim, but don't block on it; surface the notice once and move on",
                            next: "studio/open",
                        },
                    ],
                },
        ...
        },
        {
            ...
            intro: `The Studio is the companion browser surface for the entire assistant flow. It opens once at the start and stays open throughout all phases. Individual phases navigate the Studio to the relevant page (dataset review, experiment viewer, etc.) using \`navigateStudio.js\`. If the Studio background process outputs \`{"event":"session-ended",...}\` at any point, the user has closed the Studio early. This is not an error: continue the flow normally, but skip any \`navigateStudio.js\` calls for the rest of the session (the session is gone). Do **not** attempt to reopen the Studio.
            ...
            steps: [
                {
                    ...
                    body: `**If you already have a \`sessionId\` in context** from a previous \`studio/open\` step in this conversation, skip opening a new Studio. Instead, navigate the existing session to the desired page:

\`\`\`bash
node "{{pluginRoot}}/node_modules/bitfab-plugin-lib/dist/commands/navigateStudio.js" <sessionId> <initialPath>
\`\`\`
...
\`\`\`bash
node "{{pluginRoot}}/dist/commands/openStudio.js" <initialPath> [agentSessionId]
\`\`\`

\`initialPath\` is required. Always pass it explicitly, even when using the default \`/studio\`.

{{#claude}}Run it with \`run_in_background: true\` on the Bash tool. **Do NOT append \`&\` to the command string.** The \`run_in_background\` parameter handles backgrounding; adding \`&\` causes the shell to return immediately (exit 0), the harness reports the task as completed, and the long-running process is killed. This breaks all subsequent \`navigateStudio.js\` calls.{{/claude}}{{#cursor}}Run it as a long-running terminal process.{{/cursor}}{{#codex}}Run it via your runtime's "long-running exec session" mechanism.{{/codex}}
...
\`\`\`bash
node "{{pluginRoot}}/node_modules/bitfab-plugin-lib/dist/commands/recoverStudio.js" <agentSessionId>
\`\`\`
...
If the user chooses **"Instrument now"**, invoke \`{{cmd}}setup instrument\`{{#claude}} using the Skill tool{{/claude}}, then verify whether a replay script exists for this function. If **"Continue anyway"**, skip the replay-script check and start building the dataset — there's no local code to iterate on yet.`,
                    branches: [
                        {
...
If the user chooses **"Create replay now"**, invoke \`{{cmd}}setup replay\`{{#claude}} using the Skill tool{{/claude}}, then start building the dataset.`,
                    branches: [
                        {
...
{{whens}}`,
                ...
                },
                {
                    ...
                    body: `Open the dataset review page for the user **immediately** after picking or creating the dataset. Use \`navigateStudio.js\` to route the already-open Studio to the dataset review page using the \`sessionId\` captured in the \`studio/open\` step:

\`\`\`bash
node "{{pluginRoot}}/node_modules/bitfab-plugin-lib/dist/commands/navigateStudio.js" <sessionId> "/studio/trace-functions/<functionKey>/datasets/<datasetId>"
\`\`\`
...
**Template editing during labeling.** The user may also ask to edit a template in chat while the Monitor is waiting (e.g. "change the LLM view", "edit the function template", "make the output less verbose"). This is NOT a Studio event; it arrives as a regular user message. If the user asks to edit a template, go to the edit-template-loop step. **Do NOT invoke \`{{cmd}}setup templates\`** — that navigates Studio away from the dataset page and breaks the labeling flow. Template editing is handled inline here.
...
Get the user's confirmation before proceeding.`,
        ...
        },
        {
            ...
            intro: `Run an iterative improvement loop. Each iteration:
            ...
            steps: [
                {
                    ...
                    body: `The trace function key comes from the argument and no prior phase has run. Pick the dataset to iterate against, then locate the code:
...
{{whens}}`,
                    branches: [
                        {
                            when: "no datasets exist for this function (`list_datasets` returned empty), or the picked dataset has no validated failing labels",
                            description: `tell the user the function has no usable dataset yet and recommend running \`{{cmd}}assistant dataset <key>\` first; kill the Studio background process; then stop the flow`,
                            next: null,
                        },
                        {
                            when: "dataset loaded (≥1 validated failing label)",
                            description: `summarize the dataset for the user (counts of pass/fail) and the failure annotations. Pick a first experiment from the failure patterns and continue`,
                            next: "phase-5/pick-execution-mode",
                        },
                    ],
                },
                {
...
{{whens}}`,
                ...
                },
                {
                    ...
                    body: `**Detect replay script capabilities.** Before starting experiments, check what the replay script supports. These flags determine how experiment results are tracked and displayed. **If you already ran this step earlier in this session, skip it and continue to \`make-change\`.**
...
**2. Regenerate the replay script.** Invoke \`{{cmd}}setup replay\`{{#claude}} using the Skill tool{{/claude}} to regenerate the script with full flag support.
...
- If a file is newly created, the before snapshot is the empty string \`""\`. If a file is deleted, the after snapshot is \`""\`. The path is always the repo-relative file path — no \`repo\`, \`commit\`, or other context fields`,
                ...
                },
                {
                    ...
                    emit: "Running replay",
                    body: `**Replay against the dataset.** Collect the trace IDs from the labeled dataset (built in Phase 3 in \`all\` and \`dataset\` modes, or rehydrated at the start of this phase in \`experiment\` mode).
...
\`\`\`bash
node "{{pluginRoot}}/node_modules/bitfab-plugin-lib/dist/commands/navigateStudio.js" <sessionId> "/studio/experiments?experimentGroupId=<experimentGroupId>"
\`\`\`
...
{{whens}}`,
                    branches: [
                        ...
                        },
                        {
                            ...
                            description: `proceed. Carry \`infraErrored\` forward — surface as its own bucket in share-results, never folded into pass/fail`,
                            next: "phase-5/check-trace-id-support",
                        },
                    ],
                },
...
{{whens}}`,
                    branches: [
                        {
                            ...
                            description: "the SDK and server support trace ID mapping. Open the experiments page in Studio first (so the user can watch verdicts populate in real time), then evaluate and persist labels",
                ...
                },
                {
                    ...
                    body: `**Evaluate results in-agent without persisting.** This path runs when replay trace IDs are unavailable (old SDK or server). The agent still compares original vs new outputs and derives pass/fail verdicts, but cannot persist them via \`persistReplayLabels.js\` or show them in Studio.
...
\`\`\`bash
node "{{pluginRoot}}/dist/commands/persistReplayLabels.js" .bitfab/tmp/verdicts-<testRunId>.json
\`\`\`
                    ...
                    branches: [
                        {
                            ...
                            description: "labels are persisted on the replay traces and the verdicts file is gone. Continue to share-results (experiments page was already opened before evaluation)",
                            next: "phase-5/share-results",
                        },
                        {
...
\`\`\`bash
node "{{pluginRoot}}/node_modules/bitfab-plugin-lib/dist/commands/navigateStudio.js" <sessionId> "/studio/experiments?testRunIds=<testRunId1>,<testRunId2>,<testRunId3>"
\`\`\`
