...
export const assistantFlow = Flow.parse({
    ...
    commands: {
        ...
        replayProgress: {
            ...
            args: "[--label <key>] [--total N] [--out <path>] -- <replay command...>",
        },
        readTracesBatched: {
            file: "readTracesBatched.js",
...
- When the process exits, immediately continue with the next step in the flow, do not wait for another user message.{{/codex}}`,
    entries: {
        ...
        fix: {
            // Fix starts terminal-only: capture the target trace, replay only that
            // trace, then ask whether to open Studio for the full-dataset experiment.
            default: "fix/resolve",
            codex: "preamble/resolve-plugin-dir",
        },
    },
...
- **They didn't pass a key:** read their description (failure pattern, customer complaint, "something seems off with X", etc.). Call {{tool:listFunctions}} to see what exists. If exactly one function obviously matches the description by key + recent activity, use it (and grep for it). If several plausibly match, {{askVerb}} to pick one (recommend the best fit; list 2-4 alternatives by key, trace count, last activity). If nothing matches, ask the user to clarify or pass a key explicitly.
...
Do NOT invent or infer descriptions of what each function does from its key name. Use only what {{tool:listFunctions}} returns plus what's in the codebase.`,
                ...
                },
                {
                    ...
                    body: `Free-form investigation: use whatever combination of MCP and local tools fits the user's described concern. There is no fixed sequence. Typical moves:
...
Hold the trace ID, function key, code path, and the trace's failure details in working context.`,
                ...
                },
                {
                    ...
                    title: "Capture the scenario as a validated red",
                    emit: "Capturing the scenario",
                    body: `**Lock in the failing scenario before fixing anything.** Do the following, in order:

1. **Label the trace as a validated fail.** Decide the verdict from the trace + code you read in \`resolve\` (the output is wrong). Write a one-or-two-sentence \`annotation\` describing the bug, what the output got wrong, and what correct behavior is, then call {{tool:setHumanLabels}} once with \`[{ traceId, label: false, annotation }]\`. This records a **validated human verdict** (not an unapproved agent suggestion), so the trace counts as a real regression test the instant it's written, with no Studio approval step. (Use \`label: false\` because a captured bug is a failing scenario; on the rare occasion the user is capturing an already-*passing* trace to guard against future regressions, use \`label: true\`.)
2. **Pick or create a dataset.** If the user named a dataset ID, use it (confirm it's scoped to this function via {{tool:listDatasets}}). Otherwise call {{tool:listDatasets}} with the function key: if none exist, silently call {{tool:createDataset}} with \`traceFunctionKey: <key>\` and \`name: <key>\`; if one or more exist, present them {{presentMode}} (one option per dataset: name · id · count, plus "Create new"), recommend the most recently used dataset that has traces. Hold the \`datasetId\`.
3. **Attach the trace.** Call {{tool:addTracesToDataset}} with the \`datasetId\` and \`[traceId]\` (idempotent, a safe no-op if already present).
4. **If you picked a pre-existing dataset that already held other traces, load their labels now.** \`fix\` may later run the full dataset after the target trace passes, so the dataset's sibling labels need to be ready for that optional experiment. Run \`{{command:readTracesBatched}} <trace-id...> --scope full\` over the dataset's trace IDs and \`Read\` the \`outputFile\` it prints, so every sibling's validated label + annotation is in context. **Skip this** when you just created the dataset or it held only this trace (no siblings to load).
...
cd <project-dir> && {{command:replayProgress}} --label <pipeline-name> -- npx tsx scripts/replay.ts <pipeline-name> --trace-ids <capturedTraceId> --name "<experimentName>" --code-change /tmp/bitfab-code-change-<experimentN>.json --experiment-group-id <experimentGroupId>
# Preferred (supportsDatasetId true): --dataset-id alone replays the dataset; no --trace-ids needed
cd <project-dir> && {{command:replayProgress}} --label <pipeline-name> -- npx tsx scripts/replay.ts <pipeline-name> --dataset-id <datasetId> --name "<experimentName>" --code-change /tmp/bitfab-code-change-<experimentN>.json --experiment-group-id <experimentGroupId>
# Fallback (older script/SDK without dataset-id support): pass the dataset's resolved trace IDs
cd <project-dir> && {{command:replayProgress}} --label <pipeline-name> -- npx tsx scripts/replay.ts <pipeline-name> --trace-ids <id1>,<id2>,<id3>,... --name "<experimentName>" --code-change /tmp/bitfab-code-change-<experimentN>.json --experiment-group-id <experimentGroupId>
\`\`\`
...
{{whens}}`,
                    branches: [
                        ...
                        },
                        {
                            ...
                            description: "the captured trace went green. Ask whether to inspect the before/after, run the full dataset, keep iterating, or stop",
                        ...
                        },
                        {
                            ...
                            description: "the fix did not prove the target trace green. Ask whether to inspect the before/after, keep iterating, stop, or run the full dataset anyway",
                ...
                },
                {
                    ...
                    kind: "branch",
                    toolCalls: ["ask"],
                    title: "Ask what to do after the target trace passed",
                    body: `**Run only in \`fix\` mode when \`fixReplayScope = "single-trace"\` and the targeted replay passed.**
                    ...
                    branches: [
                        {
                            option: {
                                letter: "A",
                                label: "Open before/after",
                                description: "open Studio to the targeted single-trace experiment using `fixSingleTraceExperimentGroupId` (or `testRunIds` fallback), then ask this question again",
                            },
                        ...
                        },
                        {
                            option: {
                                ...
                                description: 'set `fixReplayScope = "dataset"`, set `fixSkipMakeChange = true`, open Studio, and run the same code change as a dataset experiment',
                            },
                            next: "iterate/open-experiments-before-replay",
                        },
                        {
...
{{branches}}`,
                    branches: [
                        {
                            option: {
                                letter: "A",
                                label: "Open before/after",
                                description: "open Studio to the targeted single-trace experiment using `fixSingleTraceExperimentGroupId` (or `testRunIds` fallback), then ask the failed-result question again",
                            },
                            next: "iterate/fix-open-single-trace-studio",
                        },
                        {
                        ...
                        },
                        {
                            option: {
                                letter: "C",
                                label: "Stop and wrap up",
                                description: "leave the captured dataset guard in place and report that the target is still not green",
                            },
                            next: "wrap-up/summary",
                        },
                        {
                            option: {
                                letter: "D",
                                label: "Run dataset anyway",
                                description: 'set `fixReplayScope = "dataset"`, set `fixSkipMakeChange = true`, open Studio, and run the same code change across the dataset despite the target still failing',
                            },
                            next: "iterate/open-experiments-before-replay",
                        },
                    ],
                },
                {
                    ...
                    body: `**Run only when the user chose "Open before/after" from a post-target-replay fix prompt.** Open Studio to the single-trace replay's experiment view so the user can inspect the original failing trace, the replay trace, the verdict, and the code-change before/after.

Use the experiment group from the targeted replay whenever possible:

\`\`\`bash
{{command:openStudio}} "/studio/experiments?experimentGroupId=<fixSingleTraceExperimentGroupId>"
\`\`\`

If \`supportsExperimentGroups\` was false but you captured \`testRunId\`, use the fallback instead:

\`\`\`bash
{{command:openStudio}} "/studio/experiments?testRunIds=<testRunId>"
\`\`\`
