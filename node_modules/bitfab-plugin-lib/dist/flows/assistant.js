...
| Invocation | Action |
|---|---|
| \`{{cmd}}assistant\` or \`{{cmd}}assistant all\` | Full flow: pick function → verify instrumentation → pick or create dataset → label → diagnose → iterate → wrap up |
| \`{{cmd}}assistant investigate [<key>]\` | Free-form investigation of an issue the user is describing. Read traces and code as needed to characterize the problem, then offer to stop with a summary, write a written analysis report, or roll into dataset building and continue through experiments. \`<key>\` is optional, the agent picks the function from what the user says when it isn't given |
| \`{{cmd}}assistant dataset <key>\` | Build or extend a labeled dataset for one function, then diagnose failures and iterate with experiments. Picks an existing dataset or creates a new one |
| \`{{cmd}}assistant experiment <key> [<dataset-id>]\` | Run experiments to fix failing traces against a labeled dataset, then wrap up. If \`<dataset-id>\` is omitted, you'll be asked to pick one. If the function has no datasets yet, run \`{{cmd}}assistant dataset <key>\` first |
