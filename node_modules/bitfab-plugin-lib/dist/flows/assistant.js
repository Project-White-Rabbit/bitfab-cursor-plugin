...
import { CODEX_RESOLVE_PLUGIN_DIR_BASH } from "./shared.js";
...
**MCP tools:** This skill uses \`list_trace_functions\`, \`search_traces\`, \`read_traces\`, \`update_agent_labels\`, \`list_datasets\`, \`create_dataset\`, \`add_traces_to_dataset\`, \`remove_traces_from_dataset\`, and \`get_trace_plan\` from the **local plugin MCP server** (bundled with this plugin){{#claude}}. Do NOT use the remote Bitfab MCP tools (\`mcp__Simforge__*\` or \`mcp__Bitfab__*\`) — use only the \`mcp__plugin_bitfab_Bitfab__*\` variants.{{/claude}}{{^claude}}, exposed under the \`mcp__Bitfab__*\` prefix.{{/claude}}
...
🚨 **Blocking-process rule (applies to any plugin command described as "blocks until the user does X"):** When you launch a plugin CLI that blocks on a browser handoff (\`login.js\`, etc.), you MUST keep the exec session alive and keep polling it until the process exits on its own.

- Printing the URL is NOT completion. The process is still running, racing a loopback callback and a server-polled ticket — it will exit only after the user's browser action delivers a result (or after the timeout).
- After sending the URL to the user, keep polling the live shell/exec session at least every few seconds with your normal "read more output" tool (\`write_stdin\`, \`read\`, or whatever your runtime's equivalent is for the long-running shell). Do not idle waiting for a user message.
...
- When the process exits, immediately continue with the next step in the flow — don't wait for another user message.{{/codex}}`,
    entries: {
        all: {
            default: "studio/open",
...
\`\`\`bash
node "{{pluginRoot}}/dist/commands/navigateStudio.js" <sessionId> <initialPath>
\`\`\`
...
{{#claude}}Run it with \`run_in_background: true\` on the Bash tool.{{/claude}}{{#cursor}}Run it as a long-running terminal process.{{/cursor}}{{#codex}}Run it via your runtime's "long-running exec session" mechanism.{{/codex}}
...
\`\`\`bash
node "{{pluginRoot}}/dist/commands/recoverStudio.js" <agentSessionId>
\`\`\`
                    ...
                    branches: [
                        ...
                        },
                        {
                            option: {
                                ...
                                description: "skip further labeling; go straight to dataset review",
                            },
                            recommended: true,
...
> 🚨 **HARD RULE — DO NOT SKIP:** All approved candidate trace IDs MUST be attached to the dataset before opening the page. The page reviews the dataset's contents, not the trace function's label table. An empty dataset means an empty review.`,
                ...
                },
                {
                    ...
                    body: `Open the dataset review page for the user. Use \`navigateStudio.js\` to route the already-open Studio to the dataset review page using the \`sessionId\` captured in the \`studio/open\` step:
            ...
            steps: [
                {
                    id: "pick-dataset",
                    kind: "branch",
                    toolCalls: ["bash", "grep", "listDatasets", "readTraces", "ask"],
...
- Edit the iteration target (prompt, code, tools, parameters)`,
                ...
                },
                {
                    ...
                    emit: "Running replay",
                    body: `**Replay against the dataset.** Collect the trace IDs from the labeled dataset (built in Phase 3 in \`all\` mode, or rehydrated at the start of this phase in \`experiment\` mode). Run the replay script with those specific traces.
...
{{whens}}`,
                    branches: [
                        {
                            ...
                            description: "labels are persisted on the replay traces and the verdicts file is gone. Continue to open-experiments",
                ...
                },
                {
                    ...
                    body: `**Open experiment viewer.** If no \`testRunId\`s were captured (e.g. the replay script didn't print them), skip this step and continue, but flag it to the user in \`share-results\` so the script can be fixed before the next iteration.
