...
|---|---|---|
| \`all\` | \`{{cmd}}assistant\`, \`{{cmd}}assistant all [<key>]\`, or \`{{cmd}}assistant <key>\` | Full flow: pick function → verify instrumentation → pick or create dataset → label → diagnose → iterate → wrap up |
...
If the user chooses **"Instrument now"**, tell the user to run \`{{cmd}}setup instrument\` first, then come back with \`{{cmd}}assistant all <key>\`. Do NOT invoke the setup skill from within this flow; it will break the assistant flow's continuity. If **"Continue anyway"**, skip the replay-script check and start building the dataset — there's no local code to iterate on yet.`,
                    branches: [
                        {
...
Unapproved agent labels do **not** satisfy this gate by design — \`validated: true\` excludes them.`,
                ...
                },
                {
                    ...
                    next: {
                        byMode: {
                            all: "phase-4/understand-failures",
...
Get the user's confirmation before proceeding.`,
                    next: {
                        byMode: {
                            all: "phase-5/detect-replay-capabilities",
...
{{whens}}`,
                    branches: [
                        ...
                        },
                        {
                            ...
                            next: {
                                byMode: {
                                    all: "phase-5/pick-execution-mode",
