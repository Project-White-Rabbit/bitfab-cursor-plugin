...
export const assistantFlow = Flow.parse({
    ...
    commands: {
        ...
        closeStudio: {
            file: "closeStudio.js",
            description: "Close the Studio browser tab for an agent session",
            args: "<sessionId>",
        },
        clearStudio: {
...
If no active Studio session exists, the command opens a new one and enters an event loop (stays running). {{#claude}}Run it with \`run_in_background: true\` on the Bash tool. **Do NOT append \`&\` to the command string.**{{/claude}}{{#cursor}}Run it as a long-running terminal process.{{/cursor}}{{#codex}}Run it via your runtime's "long-running exec session" mechanism.{{/codex}} If an active session already exists, it navigates and exits immediately.
                    ...
                    branches: [
                        {
                            ...
                            description: `tell the user the function has no usable dataset yet and recommend running \`{{cmd}}assistant dataset <key>\` first; kill the Studio background process if one was opened (none in benchmark mode unless the \`studio\` flag was passed); then stop the flow`,
                            next: "cleanup/close-studio",
                        },
                        {
            ...
            steps: [
                {
                    ...
                    title: "Close Studio",
                    body: `If a Studio session was opened at any point during this flow (any command that emitted a \`{"event":"session-ready","sessionId":"<uuid>"}\` JSONL line), close it now:

\`\`\`bash
{{command:closeStudio}} <sessionId>
\`\`\`
