...
- When the process exits, immediately continue with the next step in the flow, do not wait for another user message.{{/codex}}`,
    entries: {
        wizard: {
            default: "studio/open",
            codex: "phase-0/resolve-plugin-dir",
        },
        dataset: {
            default: "studio/open",
            codex: "phase-0/resolve-plugin-dir",
        ...
        "add-trace": "phase-add/resolve-context",
        replay: {
            default: "phase-replay/setup",
            codex: "phase-0/resolve-plugin-dir",
        },
    },
    defaultMode: "wizard",
            ...
            steps: [
                {
                    ...
                    body: `**In \`benchmark\` mode, first check the Studio opt-in flag** (set during argument routing when the \`studio\` keyword was passed). If benchmark did NOT opt in, skip this entire step without running any command and continue to \`phase-5/pick-dataset\`. In all other modes, and in \`benchmark\` with the \`studio\` flag, proceed.
...
**Recovering after compaction:** Automatic. \`openStudioTo.js\` and \`pushActivity.js\` read the active-session file on disk.`,
                    next: {
                        byMode: {
                            wizard: "phase-1/list-or-arg",
                            dataset: "phase-3/list-datasets",
                            experiment: "phase-5/pick-dataset",
                            investigate: "phase-investigate/gather-context",
                            benchmark: "phase-5/pick-dataset",
                            "add-trace": null,
                            replay: "phase-replay/setup",
                        },
                    },
...
- **⚠️ not found in this repo**: traces exist on Bitfab but the key isn't in this codebase (likely another repo or a renamed key)`,
                ...
                },
                {
                    ...
                    next: "phase-2/check-instrumentation",
                },
            ],
        },
        {
                    ...
                    branches: [
                        {
                            ...
                            next: "phase-2/check-replay-script",
                        },
                        {
                            option: {
                                letter: "A",
                    ...
                    branches: [
                        {
                            ...
                            next: "phase-2/detect-replay-capabilities",
                        },
                        {
                            option: {
                                letter: "A",
                    ...
                    branches: [
                        {
                            ...
                            next: {
                                byMode: {
                                    wizard: "phase-3/list-datasets",
                                    dataset: "phase-3/list-datasets",
                                    experiment: "phase-5/make-change",
                                    investigate: "phase-3/list-datasets",
                                    benchmark: "phase-5/replay-against-dataset",
                                    "add-trace": null,
                        ...
                        },
                        {
                            ...
                            recommended: true,
                            next: "phase-2/upgrade-replay-script",
                        },
                        {
                            option: {
                                letter: "B",
...
Do NOT invent or infer descriptions of what each function does from its key name. Use only what {{tool:listFunctions}} returns plus what's in the codebase.`,
                    next: "phase-investigate/explore",
                },
                {
                    id: "explore",
                    kind: "action",
...
Stop exploring once you can give the user a clear, evidence-backed account: what's going wrong (or "nothing obvious is going wrong"), when, how often, what the failure shape is, what code path is implicated, and one or two leading hypotheses. Hold the findings in working context for the next step. Cite specific trace IDs and code locations rather than vague summaries.`,
                    next: "phase-investigate/present-and-branch",
                },
                {
...
Options A and B end at the cleanup step, which closes Studio and stops its background process. Option C continues through dataset building, diagnosis, and experiments, with Studio staying open throughout until cleanup at wrap-up.`,
                    branches: [
                        ...
                        },
                        {
                            ...
                            next: "phase-investigate/write-report",
                        },
                        {
                            option: {
                                letter: "C",
...
- **Trace function key**: the non-UUID slug, if the user gave one. Datasets are scoped per function, so the key is required and **all trace IDs in the batch must belong to the same function** ({{tool:addTracesToDataset}} silently skips IDs whose function key doesn't match the dataset, so a mixed batch would partially fail with no obvious error). Resolve and verify the key up front:
  - Call {{tool:readTraces}} with \`scope: "summary"\` on **every** trace ID in the batch, not just the first. The tool caps each call at 10 IDs, so when there are more than 10, issue the calls **in parallel in a single turn** (one message, several {{tool:readTraces}} tool calls) rather than one batch after another. You must make ⌈N/10⌉ calls in total; put **all** of them in that one message. Do **not** send any {{tool:readTraces}} call alone in its own message, and do **not** wait for one call's result before sending the next: a lead-off solo call followed by the rest is the exact mistake to avoid. Then read each trace's \`traceFunctionKey\`.
                    ...
                    branches: [
                        {
                            ...
                            next: "phase-3/ask-search-intent",
                        },
                        {
                            option: {
                                letter: "B",
                            ...
                            recommended: true,
                            next: "phase-3/seed-from-existing-labels",
                        },
                        {
                            option: {
                                letter: "C",
...
Hold the user's answer (the chosen option **and** any free-text detail) in working context, the next step uses it to shape the {{tool:searchTraces}} filters and which traces to prioritise reading. If they pick C, default to recent + diverse + non-empty outputs.`,
                ...
                },
                {
                    ...
                    next: "phase-3/ask-labeling-mode",
                },
                {
...
Recommend Option A, an agent first pass turns the labeling page into a quick approve/edit review. But respect the user's choice: if they pick B, do **not** call {{tool:updateAgentLabels}} for any of these candidates. They want to label from scratch in the labeling page, with no agent verdicts pre-filled. If no new candidate traces were found in the previous step, skip this question and continue.`,
                    branches: [
                        ...
                        },
                        {
                            ...
                            next: "phase-3/attach-to-dataset",
                        },
                    ],
                },
                {
...
{{whens}}`,
                    branches: [
                        ...
                        },
                        {
                            ...
                            next: "phase-3/edit-template-loop",
                        },
                    ],
                },
                {
...
Then return to the await-event step. If the user wants more edits, they'll ask again and you'll re-enter this step.`,
                ...
                },
                {
                    id: "build-dataset",
                    kind: "action",
                    toolCalls: ["readTraceLabels", "readTraces"],
...
- **\`costRun\` is false (the common case):** call {{tool:readTraceLabels}} with all of them. It returns only the verdict fields (label + annotation + approved, no span content) and takes up to 100 IDs per call, so for almost every dataset this is a single call, well beyond the 10-ID cap {{tool:readTraces}} carries. Only if the dataset has more than 100 traces, split the IDs into chunks of 100 and call {{tool:readTraceLabels}} once per chunk; the responses are small, so plain sequential calls are fine (you never need the parallel fan-out {{tool:readTraces}} requires).
- **\`costRun\` is set:** call {{tool:readTraces}} with \`scope: "full"\` instead, because the Phase 5 cost-delta step reads each original trace's recorded token usage from this load and {{tool:readTraceLabels}} does not carry it. {{tool:readTraces}} caps each call at 10 IDs, so when the dataset has more than 10 traces, issue the calls **in parallel in a single turn** (one message, several {{tool:readTraces}} tool calls), ⌈N/10⌉ calls total, all in that one message; do **not** send a lead-off solo call and do **not** wait for one result before sending the next.
...
Unapproved agent labels do **not** satisfy this gate by design, \`validated: true\` excludes them.`,
                ...
                },
                {
                    ...
                    next: {
                        byMode: {
                            wizard: "phase-4/understand-failures",
                            dataset: "phase-4/understand-failures",
                            experiment: "cleanup/close-studio",
                            investigate: "phase-4/understand-failures",
...
Get the user's confirmation before proceeding.`,
                    next: {
                        byMode: {
                            wizard: "phase-5/detect-replay-capabilities",
                            dataset: "phase-5/detect-replay-capabilities",
                            experiment: "cleanup/close-studio",
                            investigate: "phase-5/detect-replay-capabilities",
                            benchmark: "cleanup/close-studio",
                            "add-trace": null,
                            replay: "cleanup/close-studio",
                        },
...
3. **Load it.** Call {{tool:readTraces}} with the dataset's trace IDs and \`scope: "full"\` so labels + annotations are in context.
...
{{whens}}`,
                    branches: [
                        ...
                        },
                        {
                            ...
                            next: {
                                byMode: {
                                    wizard: "phase-5/pick-execution-mode",
                                    dataset: "phase-5/pick-execution-mode",
                                    experiment: "phase-5/pick-execution-mode",
                                    investigate: "phase-5/pick-execution-mode",
                                    benchmark: "phase-5/detect-replay-capabilities",
                                    "add-trace": null,
                                    replay: null,
                                },
                            },
                    ...
                    branches: [
                        {
                            ...
                            next: "phase-5/detect-replay-capabilities",
                        },
                        {
                    ...
                    branches: [
                        {
                            when: "all flags are true",
                            description: "full capability. Hold the flags in working context and continue silently",
                            next: "phase-5/open-experiments-before-replay",
                        },
                        {
                            option: {
                                letter: "A",
                            ...
                            recommended: true,
                            next: "phase-5/upgrade-replay-script",
                        },
                        {
                            option: {
                                letter: "B",
...
This is a navigation call, not a long-running process. The existing Studio session handles it. If \`supportsExperimentGroups\` is false, skip this navigation (the \`open-experiments\` fallback will navigate with \`testRunIds\` after the replay completes).`,
                    next: {
                        byMode: {
                            wizard: "phase-5/make-change",
                            dataset: "phase-5/make-change",
                            experiment: "phase-5/make-change",
                            investigate: "phase-5/make-change",
                            benchmark: "phase-5/replay-against-dataset",
                            "add-trace": null,
                            replay: null,
                        },
...
- If a file is newly created, the before snapshot is the empty string \`""\`. If a file is deleted, the after snapshot is \`""\`. The path is always the repo-relative file path, no \`repo\`, \`commit\`, or other context fields`,
                    next: "phase-5/replay-against-dataset",
                },
                {
...
If \`completed === 0\`, do not score pass/fail on an empty set, branch to \`check-replay-health\`. Carry \`shapeErrored\` forward so \`check-replay-health\` routes shape mismatches to input adaptation instead of burying them as infra noise.`,
                    next: "phase-5/check-replay-health",
                },
                {
                    ...
                    branches: [
                        {
                            ...
                            next: "phase-5/adapt-replay-inputs",
                        },
                        {
                        ...
                        },
                        {
                            ...
                            next: "phase-5/check-trace-id-support",
                        },
                        {
...
{{whens}}`,
                    branches: [
                        ...
                        },
                        {
                            ...
                            next: "phase-5/evaluate-results-text-only",
                        },
                    ],
                },
                {
...
Hold the verdicts in working context for the final report, the \`share-results\` step in \`wizard\`/\`dataset\`/\`experiment\`/\`investigate\` modes, or the **benchmark scorecard** in \`benchmark\` mode. This step's \`next\` routes there directly: it does NOT run the \`evaluate-results\` (persist) or \`verify-replay-labels\` steps. Since trace IDs are unavailable, do NOT attempt to run \`persistReplayLabels.js\` or open the experiments page; the report is the terminal step from here.`,
                    next: {
                        byMode: {
                            wizard: "phase-5/share-results",
                            dataset: "phase-5/share-results",
                            experiment: "phase-5/share-results",
                            investigate: "phase-5/share-results",
                            benchmark: "phase-benchmark/scorecard",
                            "add-trace": null,
                            replay: null,
                        },
                    },
...
**If you're a worktree subagent** (parallel mode from \`pick-execution-mode\`): after the script returns, hand the parsed result + \`testRunId\` + unreplayable list back to the main agent and exit. The main agent collects results from all parallel experiments before \`open-experiments\`.{{/claude}}`,
                    next: "phase-5/verify-replay-labels",
                },
                {
                    id: "verify-replay-labels",
                    kind: "branch",
                    toolCalls: ["read", "write", "bash", "readTraces"],
...
{{whens}}`,
                    branches: [
                        ...
                        },
                        {
                            ...
                            description: 'you under-verdicted. Read the missing replay trace IDs (use {{tool:readTraces}} with `scope: "full"` if you didn\'t already, since you are persisting pass/fail verdicts on their span content), decide each one (PASS / FAIL with annotation, or `skip: true` if genuinely ambiguous), write a NEW verdicts file at the same path covering ALL the originally expected IDs (the script needs the full `expectedTraceIds` list each call, not just the gaps), and re-run the script. Loop back here with the new result',
                            next: "phase-5/verify-replay-labels",
                        },
                        {
