...
| \`{{cmd}}assistant dataset <key>\` | Build or extend a labeled dataset for one function, then stop. No experiments run. Picks an existing dataset or creates a new one |
...
Unapproved agent labels do **not** satisfy this gate by design — \`validated: true\` excludes them.`,
                ...
                },
                {
                    ...
                    body: `**Hold in-context** — This approved dataset is the benchmark for all experiments in Phase 5. Keep both the \`datasetId\` and the trace IDs in your working context throughout. In \`investigate\` mode, stop here: kill the Studio background process (send SIGINT or abort the background task). Surface the dataset summary (including the id) and exit so they can pick up later with \`{{cmd}}assistant experiment <key> <datasetId>\`.`,
                    next: {
                        byMode: {
