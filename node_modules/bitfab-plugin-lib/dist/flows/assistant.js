...
export const assistantFlow = Flow.parse({
    ...
    tools: {
        ...
        saveDataset: { kind: "mcp", name: "save_dataset" },
...
- **Trace evidence:** call {{tool:searchTraces}} with filters that match the user's description (failure shape, recency, label state, user / session if mentioned), then {{tool:getTraces}} with \`scope: "summary"\` or \`scope: "full"\` on the most informative ones.
...
Hold the (single, verified) function key, the trace IDs that belong to it, and any dataset ID in working context. Do **not** grep the codebase, this mode never touches local code.`,
                ...
                },
                {
                    id: "pick-dataset",
                    kind: "branch",
                    toolCalls: ["listDatasets", "saveDataset", "ask"],
...
{{whens}}`,
                    branches: [
                        ...
                        },
                        {
                            ...
                            description: `**don't ask**: silently call {{tool:saveDataset}} with \`traceFunctionKey: <key>\` and \`name: <key>\`. Hold the returned \`datasetId\` and continue`,
                            next: "add-trace/attach",
                        },
                        {
                            when: "one or more datasets already exist",
                            description: `present them {{presentMode}}, one option per existing dataset (name · id · current trace count) plus a "Create new" option. Recommend the most recently used dataset that has traces. On an existing pick, hold its id. On "Create new", silently call {{tool:saveDataset}} with \`name: "<key> #N"\` (N one more than the existing count), don't ask for a name. Hold the id and continue`,
                            next: "add-trace/attach",
                        },
                    ],
                },
                {
                    ...
                    body: `**Attach raw, then stop.** Call {{tool:addTracesToDataset}} once with the \`datasetId\` from the previous step and the full array of trace IDs. The call is idempotent, so re-attaching IDs already in the dataset is a safe no-op. Do **not** call {{tool:saveAgentLabels}}, this mode attaches traces without labels.
...
2. Call {{tool:getTraceLabels}} on the resulting trace IDs so the labels + annotations are in working context. Don't re-label them, these are already validated. {{tool:getTraceLabels}} returns only label + annotation + approved (no span content) and takes up to 100 IDs per call, so the deduped set from step 1 is usually a single call; if it deduped more than 100 IDs, split them into chunks of 100 and call {{tool:getTraceLabels}} once per chunk (sequential is fine, the responses are small).
...
Hold the user's answer (the chosen option **and** any free-text detail) in working context, the next step uses it to shape the {{tool:searchTraces}} filters and which traces to prioritise reading. If they pick C, default to recent + diverse + non-empty outputs.`,
                ...
                },
                {
                    ...
                    body: `**Ask how the user wants to label**: Before any verdicts go on these candidate traces, {{askVerb}} how the user wants to label them. There are exactly two modes, and the answer determines whether you call {{tool:saveAgentLabels}} at all:

{{branches}}

Recommend Option A, an agent first pass turns the labeling page into a quick approve/edit review. But respect the user's choice: if they pick B, do **not** call {{tool:saveAgentLabels}} for any of these candidates. They want to label from scratch in the labeling page, with no agent verdicts pre-filled. If no new candidate traces were found in the previous step, skip this question and continue.`,
                    branches: [
                        {
                            option: {
                                letter: "A",
...
The dataset page reflects each add/remove live (SSE), so the user sees changes flow in as you make them. When you're done, summarize what changed in chat and **return to the await-event step to read the next event**. The user can click Edit with agent again for another modify round, or Done to finalize.`,
                ...
                },
                {
                    ...
                    kind: "action",
                    toolCalls: ["getTemplateReference", "getTemplate", "saveTemplate"],
...
5. Call {{tool:saveTemplate}} with the full edited body. The dataset page re-renders automatically via SSE.
...
Then return to the await-event step. If the user wants more edits, they'll ask again and you'll re-enter this step.`,
                ...
                },
                {
                    id: "build-dataset",
                    kind: "action",
                    toolCalls: ["getTraceLabels", "bash", "read"],
                    commandCalls: ["readTracesBatched"],
...
- Call {{tool:getTraces}} on 3–5 failed traces with \`scope: "full"\`
            ...
            steps: [
                {
                    id: "diagnose",
                    kind: "action",
                    toolCalls: ["getTraces", "read", "grep", "ask"],
                    commandCalls: ["readTracesBatched"],
...
{{whens}}`,
                ...
                },
                {
                    ...
                    kind: "branch",
                    toolCalls: ["bash", "read", "write", "edit", "ask", "getTraces"],
...
- **"Create a new dataset"** — on pick, silently call {{tool:saveDataset}} with \`traceFunctionKey: <key>\` and \`name\` set to \`<key>\` when none exist yet, else \`"<key> #N"\` (N one more than the existing count); don't ask for a name. Mark this recommended when no dataset exists yet.
...
1. **Label the original trace as a validated fail** using the failure annotation from Phase Fix \`resolve\`: call {{tool:saveHumanLabels}} once with \`[{ traceId, label: false, annotation }]\` on the **original** trace.
