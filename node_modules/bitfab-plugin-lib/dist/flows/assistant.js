...
> "Your replay script doesn't support \`--code-change\` yet, so this experiment's code edit won't appear in the dashboard. Run \`{{cmd}}setup replay\` after this session to regenerate the script with the flag."
...
{{whens}}`,
                ...
                },
                {
                    ...
                    body: `**Route on whether replay trace IDs are available.** Check the \`hasTraceIds\` flag from \`replay-against-dataset\`. This determines whether verdicts can be persisted to the server and whether the experiments page in Studio will show meaningful results.
...
{{whens}}`,
                    branches: [
                        ...
                        },
                        {
                            ...
                            description: "tell the user: \"Your Bitfab SDK or replay script needs to be updated to support replay trace IDs. Update to @bitfab/sdk 0.13.5+ and ensure your server's completeReplay endpoint returns the traceIds mapping. Without this, experiment results can't be persisted to Studio.\" Then proceed to text-only evaluation so the user still sees comparison results in-agent",
