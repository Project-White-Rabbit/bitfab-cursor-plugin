...
| Invocation | Action |
|---|---|
| \`{{cmd}}setup\` or \`{{cmd}}setup wizard\` | Run login, then instrument + replay ({{togetherness}} per workflow) |
...
Setup runs in two phases:
  ...
  2. INSTRUMENT + REPLAY   — {{^codex}}run in parallel per workflow{{/codex}}{{#codex}}written together per workflow{{/codex}}:
...
**Inline fallback** (use only if {{tool:createTracePlan}} errors, e.g. offline or MCP unreachable): present the trace plan **using the format defined in the "Trace Plan Format" reference section below** (legend → grammar → template precedence → canonical example). **STOP** — {{askVerb}} to confirm before writing code.`,
                ...
                },
                {
                    ...
                    body: `{{^codex}}**Write instrumentation (main agent) AND replay pipeline (subagent) concurrently — to overlap code *generation*, not just file I/O.** Dispatch in a single message: your Edit calls for 11a, plus {{subagentInvocation}} for 11b. The subagent generates its replay code in parallel with your instrumentation generation — parallel Edit calls alone only overlap millisecond file writes, a subagent overlaps the seconds-to-minutes of token generation. Skip the subagent entirely for Go-only projects (Go does not support replay).
