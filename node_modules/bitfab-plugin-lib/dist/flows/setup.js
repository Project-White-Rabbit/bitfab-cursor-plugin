...
- The script emits JSONL to stdout. If it emits \`{"event":"window-opened","url":"..."}\`, immediately tell the user \`Studio opened: <url>\` in a normal chat message before continuing to poll. \`{"event":"session-ready","sessionId":"<uuid>"}\` appears once the Studio session is established (on a logged-out run, an \`{"event":"auth-required",...}\` then \`{"event":"authenticated",...}\` line precede it while the user signs in, keep waiting for \`session-ready\`). On exit, parse the final JSON line:
  - \`{"event":"confirmed","planId":"<uuid>"}\`, the user confirmed in the browser. The \`planId\` may differ from the original if a mid-session \`create_trace_plan\` call created a new plan (the script auto-tracks the latest plan via \`tracePlan:created\` events). Call {{tool:getTracePlan}} with the returned \`planId\` to read the authoritative \`capturedNodeIds\` for step {{step:instrument/write-instrumentation}}. If it differs from your initial recommendation, prune \`[auto]\` lines whose ancestor manual span was uncaptured, and drop manual \`●\` wraps that aren't in the set. **For any node the user newly captured in the UI that you hadn't classified** (a \`pure\` context node toggled into the set, it has no \`analysis\`), classify it now with the same decision procedure and set its \`mockOnReplay\` before instrumenting, never wrap a captured span without a mock decision (the UI renders an unclassified captured node as plain "runs live", which would silently let a side effect fire).
...
1. **Post the modified plan and open the UI.** Call {{tool:createTracePlan}} with \`{ language, tree, capturedNodeIds, traceFunctionKey }\` (and \`stats\` if you have a sample run from the existing trace function):
   - \`tree\`, the modified \`after\` \`TracePlanTree\` from step {{step:modify/build-modified-plan}}, with the ~10 surrounding callers / ~10 surrounding callees included as \`pure\` context nodes. Each captured node carries the \`analysis\` you set/carried-forward in that step (omitted on \`pure\` context nodes).
