...
export const setupFlow = Flow.parse({
    ...
    frontmatter: {
        ...
        argumentHint: "[all|login|login headless|instrument|modify|replay]",
    },
    tools: {
...
{{/codex}}
This skill has four phases: **login**, **instrument**, **modify**, and **replay**. Run individually or all at once (\`all\` runs login → instrument → replay; \`modify\` is only invoked explicitly or as a branch from the Instrument step {{step:instrument/search-existing}} menu).
...
**MCP tools:** This skill uses \`get_bitfab_api_key\` from the **local plugin MCP server** (bundled with this plugin){{#claude}}. Do NOT use the remote Bitfab MCP tools (\`mcp__Simforge__*\` or \`mcp__Bitfab__*\`) — use only the \`mcp__plugin_bitfab_Bitfab__*\` variants{{/claude}}{{^claude}}, exposed under the \`mcp__Bitfab__*\` prefix{{/claude}}.
