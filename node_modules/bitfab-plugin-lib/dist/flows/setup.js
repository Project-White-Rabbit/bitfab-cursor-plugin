...
import { SHARED_VARIABLES } from "./sharedVariables.js";
...
{{/codex}}
This skill has six phases: **login**, **instrument**, **modify**, **view**, **replay**, and **templates**. Run individually or all at once (\`all\` runs login → instrument → replay; \`modify\` is only invoked explicitly or as a branch from the Instrument step {{step:instrument/search-existing}} menu; \`view\` is only invoked explicitly; \`templates\` is only invoked explicitly).

Within an Instrument cycle, **instrumentation and the replay pipeline for the cycle's trace function are written {{^codex}}in parallel{{/codex}}{{#codex}}in the same batch of tool calls{{/codex}}** once the trace plan is confirmed (see step {{step:instrument/write-instrumentation}}). The Replay phase in \`all\` mode is therefore a coverage-verification/backfill sweep — it typically finds every key already wired up.
{{#codex}}
...
| Invocation | Action |
|---|---|
| \`{{cmd}}setup\` or \`{{cmd}}setup all\` | Run login, then instrument + replay ({{togetherness}} per workflow) |
| \`{{cmd}}setup login\` | Authenticate via browser OAuth and retrieve API key |
| \`{{cmd}}setup login headless\` | Authenticate by pasting a token (no browser callback needed) |
...
| \`{{cmd}}setup templates [<key>]\` | Iterate on the span-rendering templates for one trace function |`,
    entries: {
        all: "preamble/render-intro",
        login: "login/status-check",
        "login-headless": "login-headless/determine-service-url",
...
\`\`\`bash
node "{{pluginRoot}}/dist/commands/status.js"
\`\`\`{{#codex}}
...
If \`login.js\` is still running and the URL it printed includes \`ticket=<uuid>\`, the ticket channel is live — keep waiting; do not abandon it for a paste flow.{{/codex}}`,
                    branches: [
                        {
                            when: "login succeeds",
                            next: "login/get-api-key",
                        },
                        {
                            ...
                            next: "login-headless/determine-service-url",
                        },
                    ],
                },
                {
...
\`\`\``,
                    next: {
                        byMode: {
                            all: "instrument/detect-language",
                            login: null,
                            "login-headless": null,
                            instrument: null,
                            modify: null,
        ...
        {
            id: "login-headless",
            title: "Login (headless)",
            stepStyle: "list",
            intro: "{{^codex}}Use this flow when the browser callback can't reach the terminal — SSH sessions, sandboxed environments, cloud IDEs, Codespaces, CI runners. Triggered explicitly by `{{cmd}}setup login headless`, or as an automatic fallback when the normal Login flow above fails.{{/codex}}{{#codex}}Use this flow only when `login.js` itself can't deliver — i.e. both the loopback callback AND the server-polled ticket channel failed to start, or `login.js` exited with an error / hit the 10-minute timeout. Sandboxed environments (Codex seatbelt, containers, SSH, cloud IDEs) on their own are NOT a reason to use this flow — the ticket channel inside `login.js` handles those. Triggered explicitly by `{{cmd}}setup login headless`, or as an automatic fallback per the strict conditions in the Login step above.{{/codex}}",
            steps: [
                {
                    id: "determine-service-url",
                    kind: "action",
                    toolCalls: ["bash", "read"],
                    title: "Determine the service URL",
                    body: "Determine the service URL. Default is `https://bitfab.ai`. If the user has a custom deployment, read it from `~/.config/bitfab/config.json` (field `serviceUrl`) or the `BITFAB_SERVICE_URL` environment variable.",
                    next: "login-headless/show-url",
                },
                {
                    id: "show-url",
                    kind: "action",
                    title: "Show the auth URL to the user",
                    body: `Tell the user:
> Open this URL in a browser on any device: **{serviceUrl}/plugin/auth/{{authTarget}}**
>
> Sign in with your Bitfab account. The page will show an API key with a copy button. Paste the token here when you have it.`,
                    next: "login-headless/wait-for-token",
                },
                {
                    id: "wait-for-token",
                    kind: "action",
                    title: "Wait for the user's token",
                    body: "Wait for the user's next message — it will contain the token. Do NOT {{askVerb}} here (it adds an unnecessary extra step before the user can paste).",
                    next: "login-headless/validate-token",
                },
                {
                    id: "validate-token",
                    kind: "action",
                    ...
                    body: `When the user pastes the token, validate it with curl — do NOT echo the token back to the user or print it in any output:
...
If this returns 200 with a JSON body containing \`user.email\`, the token is valid. If it fails, tell the user the token was invalid and ask them to re-paste (do not re-print the bad token).`,
                    next: "login-headless/save-token",
                },
                {
                    id: "save-token",
                    kind: "action",
                    toolCalls: ["bash", "write"],
                    title: "Save the token to credentials",
                    body: `Save the token to \`~/.config/bitfab/credentials.json\` {{#codex}}writing a file{{/codex}}{{^codex}}using the \`Write\` tool{{/codex}} with this exact content (replace \`<TOKEN>\` with the pasted value, nothing else):
...
\`\`\``,
                    next: "login-headless/confirm-success",
                },
                {
                    id: "confirm-success",
                    kind: "action",
                    title: "Confirm sign-in to the user",
                    body: 'Confirm success to the user by referencing the email returned from `/api/plugin/whoami` — e.g. "Signed in as alice@example.com." **Never echo the token back.**',
                    next: "login-headless/ask-session-log-consent",
                },
                {
                    id: "ask-session-log-consent",
                    kind: "action",
...
node -e "const fs=require('fs'),os=require('os'),p=require('path').join(os.homedir(),'.config/bitfab/config.json');fs.mkdirSync(require('path').dirname(p),{recursive:true});const c=JSON.parse(fs.existsSync(p)?fs.readFileSync(p,'utf8'):'{}');c.sessionLogConsent=CONSENT;fs.writeFileSync(p,JSON.stringify(c,null,2)+'\\n')"
\`\`\``,
                    next: "login-headless/continue-or-stop",
                },
                {
                    id: "continue-or-stop",
                    kind: "action",
                    title: "Continue with setup or stop",
                    body: "Continue with the rest of setup, or stop if running `login headless` only.",
                    next: {
                        byMode: {
                            all: "instrument/detect-language",
                            login: null,
...
\`\`\`bash
node "{{pluginRoot}}/dist/commands/openTracePlan.js" <planId>
\`\`\`

(\`{{pluginRoot}}\` resolves to the plugin directory; \`<planId>\` is the id returned by {{tool:createTracePlan}}.) The script opens the trace plan page and **blocks** until the user clicks **Confirm** or **Chat about this** in the browser{{#codex}} (up to 30 minutes){{/codex}}.{{#codex}} The script prints a handoff URL (with \`callbackPort=\` and \`ticket=\` query params) — surface it to the user verbatim so they can paste it into a browser if the auto-launch didn't open one.
...
- On exit, parse the final stdout line:
  - \`Trace plan confirmed [via …]\` — the user confirmed in the browser. Call {{tool:getTracePlan}} with the plan id to read the authoritative \`capturedNodeIds\` for step {{step:instrument/write-instrumentation}}. If it differs from your initial recommendation, prune \`[auto]\` lines whose ancestor manual span was uncaptured, and drop manual \`●\` wraps that aren't in the set.
                    ...
                    branches: [
                        ...
                        },
                        {
                            option: {
                                ...
                                description: "proceed to Replay (in `all` mode) / Done (in `instrument` mode)",
                            },
                            next: {
                                byMode: {
                                    all: "replay/gather-keys",
...
(\`{{pluginRoot}}\` resolves to the plugin directory; \`<planId>\` is the id returned by {{tool:createTracePlan}}.) The script opens the trace plan page and **blocks** until the user clicks **Confirm** or **Cancel** in the browser{{#codex}} (up to 30 minutes){{/codex}}.{{#codex}} The script prints a handoff URL (with \`callbackPort=\` and \`ticket=\` query params) — surface it to the user verbatim so they can paste it into a browser if the auto-launch didn't open one.

**Polling (mandatory — see the Blocking-process rule at the top of this skill):** after surfacing the URL, keep polling the live exec session until the process exits. Do NOT stop after printing the URL. Do NOT wait for a chat message from the user — their confirmation arrives as stdout on the already-running process, not as a new prompt. Poll every few seconds until one of these terminal conditions, then route accordingly:{{/codex}}

3. **On exit, route by the final stdout line:**
   - \`Trace plan confirmed [via …]\` — call {{tool:getTracePlan}} with \`{ planId }\` to read the authoritative \`capturedNodeIds\` (the user may have toggled \`pure\` context nodes into the captured set or removed previously-captured nodes in the UI). Reconcile your edit plan with what's now in \`capturedNodeIds\` — drop manual \`●\` wraps no longer captured, add wraps for any newly captured nodes — then take branch **A** (Proceed).
   - \`Trace plan cancelled [via …]\` — the user cancelled from the browser. Take branch **C** (Modifications) — {{askVerb}}: what do they want to change? Their answer feeds back into step {{step:modify/build-modified-plan}}.
...
(\`{{pluginRoot}}\` resolves to the plugin directory; \`<planId>\` is the id parsed from step {{step:view/fetch-plan}}.) The script opens the trace plan page and **blocks** until the user closes the browser tab or clicks Confirm/Cancel{{#codex}} (up to 30 minutes){{/codex}}. View is read-only — whichever button the user clicks, do **not** apply edits or call {{tool:getTracePlan}} again. When the process exits, report that the plan was viewed and stop.{{#codex}}
...
**Polling (mandatory — see the Blocking-process rule at the top of this skill):** after surfacing the URL, keep polling the live exec session until the process exits. Do NOT stop after printing the URL. Do NOT wait for a chat message from the user — their dismissal arrives as stdout on the already-running process, not as a new prompt.{{/codex}}`,
                    next: null,
                },
            ],
        },
...
5. Call {{tool:updateTemplate}} with \`spanType\`, \`traceFunctionKey: <key>\`, and the full edited body. The tool upserts the per-function row in place (no version bump, no row juggling). On the first save for a span type the row is created; subsequent edits update it.
