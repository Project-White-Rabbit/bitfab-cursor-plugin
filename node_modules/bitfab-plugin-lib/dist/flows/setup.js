...
{{/codex}}
This skill has seven phases: **login**, **session-logs**, **instrument**, **modify**, **view**, **replay**, and **templates**. Run individually or all at once (\`wizard\` runs login → instrument → replay; \`session-logs\` is standalone and does not require login; \`modify\` is only invoked explicitly or as a branch from the Instrument step {{step:instrument/search-existing}} menu; \`view\` is only invoked explicitly; \`templates\` is only invoked explicitly).
