...
- **Trace-processor registrations ({{traceProcessorNames}}) too**, even though they are unkeyed in code: the registration site (\`setTraceProcessors\` / \`set_trace_processors\` with the Bitfab processor) is itself an instrumented workflow whose key is derived server-side from the workflow name. Note whether each processor registration is wrapped in a manual \`withSpan\`/\`@span\` root, the replayability check in step {{step:inspect/check-freshness}} needs this (a processor with no manual root is not replayable).
...
4. **Replayability of each root**: script coverage is only half of replay, a script that wraps a non-replayable root still won't run. Determine each key's replayability statically from source (this step does not fetch recorded trace inputs, so reason from signatures, not trace data):
   ...
   - **Trace-processor keys** ({{traceProcessorNames}}): the processor captures the run but its root span records an empty input, so a processor-only key (no manual \`withSpan\`/\`@span\` root wrapping the run) is NOT replayable, flag it ⚠️ root not replayable and recommend adding a manual root that takes the run input. If the key DOES have a manual root wrapping the SDK call, check that root's signature like any decorated key (next bullet).
                    ...
                    branches: [
                        ...
                        },
                        {
                            option: {
                                letter: "C",
                                label: "Leave as-is",
                                description: "add a header comment noting why the key isn't replayable (not callable, or a non-replayable root such as a processor-only key with an empty-input root) and flag that the script will rot",
                            },
                            next: "cleanup/close-studio",
                        },
                    ],
