...
export const setupFlow = Flow.parse({
    ...
    commands: BITFAB_PLUGIN_COMMANDS.defineCommands({
        ...
        switchOrg: {
            description: "List the user's Bitfab orgs (no args), or switch the plugin's active org and replace the local API key (with a <authOrganizationId> arg)",
    ...
    intro: `{{#ask-user-question}}**Always use \`AskUserQuestion\` when asking questions or presenting choices** (one exception: a step that explicitly says its answer is free-form, such as asking which file or function to instrument once the user has said they know, asks in plain chat and waits, because a menu there would stand between the user and the answer only they hold). Never print a question as text and wait.{{/ask-user-question}}{{^ask-user-question}}**When the flow branches, always present the options clearly and wait for the user's answer before proceeding.** Number or letter the options so the user can pick by reference.{{/ask-user-question}} Rules:
...
**Product pages:** Link to the existing app pages for experiments, datasets, and other dashboard features. Use /plugin only for automatic-close login and template previews. Page commands print one JSON line with \`event: "link"\` and \`url\`, then exit. Relay the URL in chat. The user reports review decisions in chat; fetch current saved state with MCP before continuing. Never treat printing a link or closing a browser tab as approval.`,
    ...
    modeHints: {
        ...
        cloud: "Run your normal replay on your own GitHub Actions by adding --cloud. One workflow file in the repository; no Bitfab repository connection or GitHub CLI required.",
    ...
    appendices: [
        {
            ...
            title: "Refactor confirmation (applies to Instrument's workflow-selection step, Replay's non-replayable-root step, and any write-instrumentation step that turns out non-additive)",
...
3. **{{#ask-user-question}}AskUserQuestion{{/ask-user-question}}{{^ask-user-question}}Ask the user{{/ask-user-question}}** with exactly two options:
   - **"Apply refactor"**: proceed to write the changes
   - **"Cancel"**: return to the previous {{askNoun}} (Instrument's workflow-selection (a)/(b)/(c), or Replay's non-replayable-root three-option prompt) so the user can pick a different resolution
    ...
    phases: [
        {
            ...
            title: "Cloud replay",
            stepStyle: "list",
            intro: "Run replay in the customer's GitHub Actions. Use this standalone mode for 'setup cloud', 'set up cloud replay', or 'replay on GitHub'. Do not enter login or connect a repository to Bitfab. Normal replay still uses a Bitfab API key on the runner.",
            steps: [
                {
                    id: "inspect",
                    kind: "action",
...
If the output includes \`v<X> available, run ... to update\`, surface that line verbatim to the user once before proceeding; do not block on it.{{/no-startup-banner}}
...
**If {{tool:apiKey}} is not available in this session**, {{^amp}}the MCP server is switched off, not broken: it ships inside this plugin, so an absent tool is a setting rather than a failed install.{{/amp}}{{#amp}}the Bitfab plugin did not load, so its tools never registered.{{/amp}} Say so and hand the user the fix below; do not diagnose further, and do not fall back to drafting anything by hand.

{{#claude}}Tell them to run \`/mcp\`, enable **Bitfab**, then re-run this skill. The tools load into the running session, so nothing is lost. Do **not** suggest restarting: the setting is stored per project and survives a relaunch, so they would come back just as stuck. Ignore \`claude mcp list\` here, it health-checks the server in its own process and prints \`Connected\` even when this session cannot reach it.{{/claude}}{{#cursor}}Tell them to re-enable the Bitfab MCP server in their editor's MCP settings, then re-run this skill.{{/cursor}}{{#codex}}Tell them to re-enable the Bitfab MCP server in their editor's MCP settings, then re-run this skill.{{/codex}}{{#amp}}Tell them to run \`plugins: reload\` from the command palette (Ctrl-O), or restart Amp, then re-run this skill. \`amp plugins list\` shows whether the Bitfab plugin loaded and, if not, why.{{/amp}}
...
{{^mode:explain}}Stop there and continue to {{skill-ref:approach}}. Do not render the mode menu below: mid-setup, a menu of other modes is noise.{{/mode:explain}}

{{#mode:explain}}Follow the block above with this one, as a code block, exactly as laid out:
...
then close with one line: to start tracing, run \`{{cmd}}setup\`; to debug an existing setup, run \`{{cmd}}setup inspect\`. Then stop.{{/mode:explain}}`,
                    next: {
                        byMode: {
        ...
        },
        {
            ...
            title: "Approach",
            stepStyle: "list",
            intro: "Settle who does the instrumenting before any code is read or written. Runs once, in `wizard` mode only, after Login.",
            steps: [
                {
                    id: "choose-driver",
                    kind: "branch",
                    toolCalls: ["ask"],
                    title: "Ask whether to run the instrumentation or hand it over",
                    body: `The user is authenticated now. {{askVerbCap}} to settle who does the instrumenting:
                    ...
                    branches: [
                        {
                            option: {
                                ...
                                description: "I drive the instrumentation end to end, checking with you at each decision",
                            },
                            recommended: true,
                            next: "approach/whats-next",
                        },
                        {
                            option: {
                                letter: "B",
                                label: "I'll instrument myself",
                                description: "hand over the docs and stop, no scanning, no code changes",
                            },
                            next: "approach/self-serve-handoff",
                        },
                    ],
                },
                {
                    id: "whats-next",
                    kind: "action",
                    title: "Set expectations before the first decision",
                    body: `The user asked to be walked through it. **Before anything else** (before dispatching to {{skill-ref:instrument}}, before a single probe or file read), render the content of the block below **verbatim** as formatted markdown: no code fence, no rewording, no additions.
...
Then go to {{skill-ref:instrument}} and start at its first step. The guided path ends here: do **not** continue into the self-serve handoff that follows, which belongs to option B and tells you to stop.`,
                    next: "instrument/detect-language",
                },
                {
...
- If found: {{askVerb}}, **listing the existing trace function keys inside the question's own text** (\`<key>\` · its root function, or its handler registration site for handler-only keys), so the user picks against the actual list instead of a message scrolled above the prompt:
...
{{branches}}
  ...
  **B and C are both free-text options, and this is the earliest point either answer can land.** Say so in the question: they can type a workflow to instrument, or the key of an existing trace to change, instead of picking anything.
  - **B (a target to instrument)**: carry that target through the SDK install and doc steps and treat the discovery question at step {{step:instrument/choose-discovery}} as already answered by it, then read just that location at step {{step:instrument/read-target}}. Without this option a user who already knows their target has to answer "search" here, sit through the install, and then answer the same question again four steps later, which is asking them to repeat themselves.
                    ...
                    branches: [
                        ...
                        },
                        {
                            option: {
                                ...
                                description: "find workflows that aren't traced yet",
                            },
                            recommended: true,
                            next: "instrument/get-api-key",
                        },
                        {
                            option: {
                                letter: "B",
                                label: "I'll name the workflow to instrument",
                        ...
                        },
                        {
                            option: {
                                letter: "C",
                                label: "Change what an existing trace captures",
                                description: "adjust what one of the keys above records; type its key to go straight to it",
                            },
...
Recommend one to start with. **Ask the user to pick exactly ONE workflow to instrument first.** Never accept "multiple" or "all", instrumenting one workflow produces exactly one trace function with one set of code changes. If the user wants to instrument several, they will be done sequentially via the loop in step {{step:instrument/next-workflow}}, one at a time.`,
                ...
                },
                {
                    ...
                    title: "Write instrumentation and replay registry together",
                    body: `{{^codex}}**Write the instrumentation edits (11a) and the replay registry module (11b) for this trace function in the same cycle.** Reuse the Replay section already in context. The project owns the registry module only; the installed SDK owns the executable. Skip 11b for Go-only projects.
            ...
            steps: [
                {
                    ...
                    body: `**Skip the search when you already hold the keys**: arriving from Instrument's existing-SDK-usage menu means you ran exactly this search one step ago, and re-running it makes the user watch the same greps twice for the same answer. Otherwise **gather existing trace functions** by searching for SDK patterns ({{sdkPatterns}}). List each key alongside its root function (or, for keys registered only via a framework handler, the handler registration site, handler keys have no decorated root and that is expected). If none are found, tell the user Modify needs existing instrumentation and suggest \`{{cmd}}setup instrument\`.
                    ...
                    branches: [
                        ...
                        },
                        {
                            option: {
                                ...
                                description: "pick another traced function to adjust",
                            },
                            next: "modify/pick-function",
                        },
                        {
...
Mark each key as ✅ traces arriving (with most recent timestamp), ⚠️ instrumented here but no traces yet, or ❓ traces exist in the org but the key isn't found in this repo. If not authenticated (from step {{step:inspect/status-check}}), skip the tool calls and note that arrival can't be checked until login.`,
                ...
                },
                {
                    ...
                    body: `Check whether the plugin, SDK, and replay registry modules are current, so the report can offer to fix what's stale:
...
- **Subtree capture**: ✅ wired / ⚠️ \`@bitfab/sdk\` installed but no transform adapter in the real build path / ❌ \`@bitfab/sdk\` missing / ❌ Python below 3.12 / \`n/a (no opt-out roots)\` (from the subtree-capture check in step {{step:inspect/check-freshness}}).
...
Then continue to the fix prompt. Inspect does not open Bitfab.`,
                ...
                },
                {
                    ...
                    body: `If the report surfaced anything stale or missing (plugin behind, SDK out of date or on the legacy \`bitfab\` package, replay registry modules missing/incomplete, or TypeScript subtree capture not wired), {{askVerb}} whether to apply them, each fix is then confirmed individually in the next step (nothing is changed blanket). If everything is healthy, skip the question and go straight to cleanup.
...
- **Subtree capture not wired** (TypeScript), this edits a build file, so **preview before touching code**: name the workspace, the adapter you would use, and the exact config file or start script you would change, then {{askVerb}} to proceed. If yes, use the installed SDK and wire the matching \`@bitfab/sdk/transform\` adapter: \`@bitfab/sdk/transform/next\` wrapping the existing \`next.config\`, the \`/vite\`, \`/rollup\`, \`/rolldown\`, \`/esbuild\`, \`/bun\`, \`/webpack\`, \`/rspack\`, \`/rsbuild\`, \`/babel\`, \`/nest\`, or \`/typescript\` entry in the existing build config, or \`--import @bitfab/sdk/transform/register\` on the real start command. Preserve every existing wrapper; never replace or bypass one. Python below 3.12 is a runtime upgrade, report it and do not attempt it.
...
If **already authenticated**, continue to step {{step:switch-org/pick-org}}. If **not authenticated**, tell the user to sign in first with \`{{cmd}}setup login\`, then stop; do NOT run the login flow as part of switching.`,
                ...
                },
                {
                    ...
                    body: `Call {{tool:listOrganizations}} to list the organizations the signed-in user belongs to. Each entry has a name, the user's role, an \`id:\` (the \`authOrganizationId\`), and the org the plugin uses now is marked \`[current]\`.
...
Only ever use an \`id\` value returned by {{tool:listOrganizations}}; never invent one. Carry the chosen id into the next step.`,
                ...
                },
                {
                    ...
                    body: `Switch to the chosen org by passing its \`authOrganizationId\`:

\`\`\`bash
{{command:switchOrg}} <authOrganizationId>
\`\`\`
...
- **Copy-pasteable registry template + installed-command contract + input serialization caveat:** \`/typescript-sdk.md\`, \`/python-sdk.md\`, \`/ruby-sdk.md\`. Use this for the language-specific registry shape and the standard \`bitfab-replay --registry <path> <pipeline>\` invocation.`,
            steps: [
                ...
                },
                {
                    ...
                    kind: "action",
                    toolCalls: ["glob", "grep"],
                    title: "Search for existing replay registries",
                    body: "**Search for existing replay registry modules**: files matching `scripts/replayRegistry.*`, `scripts/replay_registry.*`, or files defining an SDK `ReplayRegistry` / `defineReplayRegistry` (fall back to direct replay API calls for legacy scripts).",
                ...
                },
                {
                    ...
                    body: `**Compare coverage.** Replay is non-interactive once entered, do not ask the user whether to create or add registry modules. Determine which case applies:
...
- **Some keys are missing entries, or no replay registry exists yet:** add the missing entries or create the registry module next.`,
                    branches: [
                        {
                            when: "every trace function key already has a conformant registry entry (coverage complete)",
                            next: "replay/safety-net",
                        },
                        {
                            when: "one or more keys still need a registry entry or module",
...
**Locate the replay registry module(s)** you'll edit later: search for files importing/calling the SDK's \`replay\` (commonly under \`scripts/\`). If there are **no** replay registry modules yet, tell the user to run \`{{cmd}}setup replay\` first to create them, then come back (route to cleanup), DB-snapshot augments an existing replay registry module, it does not create one from scratch. No client-config edit is needed: snapshot capture is always on, so there is nothing to add to \`new Bitfab({ ... })\`.`,
                    branches: [
                        {
...
When the status is \`checking\`, wait ~15 seconds before calling the tool again, do not hammer it. When it is \`none\` or \`failed\`, the user hasn't finished connecting (or it errored); re-surface the Integrations URL, give them a moment, then re-check. Only proceed once it reports \`connected\`.`,
                ...
                },
                {
                    ...
                    title: "Wire the replay branch into the replay registry module",
                    body: `Update the replay registry module(s) from step {{step:db-snapshot/preflight}} so the replayed function connects to the per-trace branch. Ground every edit in https://docs.bitfab.ai/db-branching.md and your SDK's \`replay\` / branch-accessor reference, fetch the page for the project's language first; the replay option and the accessor names differ per SDK.
...
Leave the live request path untouched: only the replayed function reads the branch. (Optional, TypeScript only: you can pass \`dbSnapshot: { provider: "neon" }\` to \`new Bitfab({ ... })\` to pin the provider at capture time. It is **not required**: capture works without it; the provider is otherwise resolved at replay time.)`,
                ...
                },
                {
                    id: "smoke-test",
                    kind: "action",
                    toolCalls: ["bash", "read", "grep", "searchTraces"],
