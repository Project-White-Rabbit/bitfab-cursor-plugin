...
export const setupFlow = Flow.parse({
    ...
    frontmatter: {
        ...
        argumentHint: "[wizard|explain|login|instrument|modify|inspect|switch-org|view|replay|db-snapshot|session-logs|templates|analyze-repo] [<what to do>]",
    },
...
`,
    entries: {
        ...
        "analyze-repo": "analyze-repo/preflight",
    },
    defaultMode: "wizard",
    modeHints: {
        wizard: "Run login, then instrument + replay (together per workflow).",
        ...
        replay: "Create or update replay scripts for instrumented workflows.",
    ...
    appendices: [
        {
            ...
            title: "Refactor confirmation (applies to Instrument's workflow-selection step, Replay's safety-net step, and any write-instrumentation step that turns out non-additive)",
...
If they pick **A**, do the full codebase scan in step {{step:instrument/identify-workflows}}. If they pick **B**, ask which file, function, or directory they want to instrument (if they haven't already named it) and go to step {{step:instrument/read-target}} to read just that location, skipping the broad scan.`,
                    branches: [
                        ...
                        },
                        {
                            option: {
                                letter: "B",
                                label: "Instrument a specific target",
...
- **11a. Instrumentation edits**: follow the SDK reference exactly, purely additive. Never change behavior, arguments, return values, error handling, variable names, types, control flow, or code structure. **Wrapping means wrapping, not silently rewriting.** Default to attaching the span to the existing call. Rewriting, re-implementing, inlining, or hand-reconstructing a framework/SDK call to seat a root is sometimes genuinely necessary, but it is a refactor, not additive instrumentation: never do it silently as part of the write step. Any such rewrite must preserve behavior exactly (labeled *visibility* or *structural* in the refactor plan); a rewrite that would change behavior is never allowed, fall back to an additive root or move the boundary inward instead. Try the additive root first, when the natural root is opaque or stream-returning, reach for the framework handler/middleware or the \`finalize\` option wrapped around the *unchanged* call. If none of those fit and a rewrite really is required, STOP and run the "Refactor confirmation" gate (present the plan, get an explicit yes), then proceed once the user approves. **Likewise, if the additive wrap doesn't typecheck, STOP and surface it rather than dropping or loosening an argument, weakening validation, or otherwise quietly changing runtime behavior to make it compile** (the same "no type-checker escape hatches, don't paper over it" rule the replay step enforces): a compile error on a purely-additive wrap means the wrap isn't additive, so either find the additive form or take the change through the refactor gate. Batch repetitive edits in parallel (one message, many Edit calls); for large mechanical fan-outs (>10 files of the same wrapper pattern), validate the pattern on one file, then do the remaining files. **For each span the trace plan marked \`mockOnReplay: true\`, pass that SpanOption when you wrap it** (TypeScript \`withSpan(key, { type, mockOnReplay: true }, fn)\`, Python \`@client.span(key, type=..., mock_on_replay=True)\`, Ruby equivalent), so replay's \`mock: "marked"\` strategy serves its recorded output. Spans the plan left not-mocked get the normal wrapper with no \`mockOnReplay\`.
...
The trace plan's \`Files changed:\` list must include the replay script path for this cycle (new or edited) alongside the instrumented files.`,
                ...
                },
                {
                    ...
                    body: `Tell the user how to run the app to generate the first trace AND, once traces exist, how to run the replay script for this pipeline, give exact command(s) for both. Do NOT run them yourself. (Omit the replay command for Go-only projects.) **If step {{step:instrument/present-workflows}} flagged this function as reading stored DB state** (TypeScript, Python, Ruby), add one line: replay currently reads today's data, run \`{{cmd}}setup db-snapshot\` to make it replay against the database state at trace time.
...
\`\`\``,
                ...
                },
                {
                    ...
                    body: `**MANDATORY STOP, never silently end the cycle without the A/B/C/D prompt.** {{askVerbCap}} (we recommend **A**: get a real trace flowing before instrumenting the next workflow):

{{branches}}

**For option A**, go to step {{step:instrument/wait-for-trace}} and watch for a trace before asking whether to try replay.

B and C return to step {{step:instrument/present-workflows}} for the selected workflow. A returns there after the trace/replay follow-up prompts unless the user chooses to be done. Only D exits the Instrument loop. **If the next workflow the user wants isn't already in the discovered list** (common when the first cycle came from the point-to-it path, where step {{step:instrument/read-target}} only read the one named location), first run another discovery pass, scan via step {{step:instrument/identify-workflows}} or read another named location via step {{step:instrument/read-target}}, then present. Never tell the user there's nothing left to instrument just because the targeted read only surfaced one workflow.
...
**Re-entry rule (applies after you leave this loop).** If, later in the conversation, the user asks for another function's trace plan in plain language (\`give me the trace plan to instrument the second one\`, \`instrument the next one\`, \`instrument another function\`), that is a fresh Instrument cycle: re-invoke \`{{cmd}}setup instrument\` (name the mode, so it goes straight to Instrument rather than falling back to the full \`wizard\`) so it runs through the trace-plan UI. **Never satisfy such a request by hand-writing a trace plan as a chat message (a markdown table of captured nodes), that skips the Studio confirmation UI ({{tool:createTracePlan}} + \`openTracePlan\`) and is exactly the miss this rule prevents.**`,
                    branches: [
                        {
                            option: {
                                letter: "A",
                                label: "Wait for the first trace [current workflow]",
                                description: "you run the app (or let me); I watch for the trace to land and report it",
                            },
                            recommended: true,
                            next: "instrument/wait-for-trace",
                        },
                        {
                            option: {
                                letter: "B",
                        ...
                        },
                        {
                            option: {
                                ...
                                description: "stop adding workflows; continue to replay verification, or finish if you ran Instrument on its own",
                            },
                            next: {
                                byMode: {
                                    wizard: "replay/gather-keys",
                ...
                },
                {
                    ...
                    body: `Restate the run command from step {{step:instrument/tell-user-how-to-run}} so the user can run it (or let you run it for them). Before starting the wait, tell the user verbatim: \`Run your app now to produce a trace (or tell me to run it for you). I'll watch and report the first trace when it lands, up to ~10 min. Press Esc to cancel.\` Then run {{^codex}}with \`Bash\` (timeout: 660000ms){{/codex}}{{#codex}}in a shell (allow up to ~11 min){{/codex}}:

\`\`\`bash
{{command:waitForTrace}} <trace-function-key>
\`\`\`
...
{{whens}}`,
                    branches: [
                        {
                            when: '`{"status":"found","traceId":"...","url":"..."}`',
                            description: "report the trace URL and preserve the returned traceId for the replay prompt",
                            next: "instrument/offer-replay-after-trace",
                        },
                        {
                            when: '`{"status":"timeout",...}`',
                            description: "note that no trace arrived yet, then return to workflow selection",
                            next: "instrument/present-workflows",
                        },
                        {
                            when: '`{"status":"interrupted",...}`',
                            description: "note that the user cancelled, then return to workflow selection",
                            next: "instrument/present-workflows",
                        },
                    ],
                },
                {
                    id: "offer-replay-after-trace",
                    kind: "branch",
                    toolCalls: ["ask"],
                    title: "Ask whether to try replay",
                    body: `After reporting the first trace URL, {{askVerb}} whether to try the replay script against that exact trace now. Recommend **A** for non-Go projects because this validates the replay pipeline while the user still has the workflow fresh.

{{branches}}

For option A, run only the current workflow's generated replay command from step {{step:instrument/tell-user-how-to-run}}, adding \`--trace-ids <traceId>\` with the trace id from step {{step:instrument/wait-for-trace}}. If the project is Go-only and no replay command was generated, say Go has no replay support and choose option B or C instead.`,
                    branches: [
                        {
                            option: {
                                letter: "A",
                                label: "Try replay now",
                                description: "run the generated replay script against the trace that just landed",
                            },
                            recommended: true,
                            next: "instrument/run-single-trace-replay",
                        },
                        {
                            option: {
                                letter: "B",
                                label: "Keep instrumenting",
                                description: "skip replay for now and pick the next workflow",
                            },
                            next: "instrument/present-workflows",
                        },
                        {
                ...
                {
                    id: "run-single-trace-replay",
                    kind: "action",
                    toolCalls: ["bash"],
                    title: "Run replay for the captured trace",
                    body: `Run the exact replay command generated in step {{step:instrument/tell-user-how-to-run}} for the current workflow, adding \`--trace-ids <traceId>\` for the trace id returned by step {{step:instrument/wait-for-trace}}. Keep the command's normal env loader (for example \`pnpm with-env\`, \`dotenv run\`, or the project's equivalent). Do not substitute mocks or a different entrypoint.
...
If the replay exits 0, summarize the test run URL and whether items passed, changed, or errored, then continue to the next-move prompt. If it exits non-zero, surface the failing command and the important stderr/stdout lines; make only high-confidence fixes to the replay script or instrumentation, rerun once, then continue to the next-move prompt. Do not run the broad Replay phase from here; this step is only the just-captured trace smoke test.`,
                    next: "instrument/post-trace-replay-next-move",
                },
                {
                    id: "post-trace-replay-next-move",
                    kind: "branch",
                    toolCalls: ["ask"],
                    title: "Pick the next move after replay",
                    body: `After the single-trace replay attempt finishes, {{askVerb}} for the next move:
                    ...
                    branches: [
                        {
                            option: {
                                letter: "A",
                                label: "Keep instrumenting",
                                description: "pick the next workflow to trace",
                            },
                            recommended: true,
                            next: "instrument/present-workflows",
                        },
                        {
                            option: {
                                letter: "B",
                                label: "Instrument another target",
                                description: "scan again or read a different named location",
                            },
                            next: "instrument/present-workflows",
                        },
                        {
...
1. **Post the modified plan and open the UI.** Call {{tool:createTracePlan}} with \`{ language, tree, capturedNodeIds, traceFunctionKey }\` (and \`stats\` if you have a sample run from the existing trace function):
   - \`tree\`, the modified \`after\` \`TracePlanTree\` from step {{step:modify/build-modified-plan}}, with the ~10 surrounding callers / ~10 surrounding callees included as \`pure\` context nodes. Every node carries the \`analysis\` you set/carried-forward in that step, including uncaptured context nodes.
...
(\`{{pluginRoot}}\` resolves to the plugin directory; \`<planId>\` is the id returned by {{tool:createTracePlan}}.) The script navigates Studio to the trace plan page and **blocks** until the user clicks **Confirm** or **Cancel**{{#codex}} (up to 30 minutes){{/codex}}. If it emits \`{"event":"window-opened","url":"..."}\`, immediately tell the user \`Studio opened: <url>\` in a normal chat message before continuing to poll.{{#codex}}
...
**Re-entry rule (applies after you leave this loop).** If, later in the conversation, the user asks to re-instrument or change another function's capture in plain language (\`re-instrument <fn>\`, \`change what this span records\`, \`give me the updated trace plan for <fn>\`), that is a fresh Modify (or Instrument) cycle: re-invoke \`{{cmd}}setup modify\` (name the mode, so it goes straight to Modify rather than falling back to the full \`wizard\`) so it runs through the trace-plan UI. **Never satisfy such a request by hand-writing a trace plan or before/after diff as a chat message, that skips the Studio confirmation UI ({{tool:createTracePlan}} + \`openTracePlan\`).**`,
                    branches: [
                        {
                            option: {
                                letter: "A",
...
- **Handler-instrumented keys (no decorated function in the app) replay by explicit key.** When a key is registered only via a framework handler ({{keyedHandlerKeyRefs}}), there is no decorated function to import; that does NOT make the key unreplayable. Define the pipeline's replay function in the script as a plain callable and pass the key explicitly (Python: \`client.replay("<key>", fn, ...)\`; TypeScript: \`bitfab.replay("<key>", fn, opts)\`), re-invoking the same framework entrypoint production calls with the recorded root input (a dict root input arrives as a single positional argument) plus a freshly constructed environment (framework config, dependency objects). On SDKs that predate explicit-key replay, wrap the callable under the same key yourself (Python \`@bitfab.span("<key>")\` with a \`(**state)\` signature for dict roots; TS \`getFunction(key).withSpan(...)\`). Substitute safe no-ops only for side-effectful wiring with no live counterpart at replay time (billing/credit callbacks, notification senders). The pattern is documented in the SDK docs' Replay section (handler subsection).
- **Replay root parity (hard rule):** for keys with a decorated or manually wrapped root function, the function passed to \`bitfab.replay(...)\` / \`client.replay(...)\` must be the exact same exported top-level traced wrapper that production/runtime calls to create the root span. You must do this unless it is genuinely impossible in the host app; inconvenience, extra refactoring, an inline wrapper, or needing to move code is not impossible. If production currently creates that wrapper inline inside a route, job, handler, callback, or local file scope, extract it into the nearest appropriate service/module, export it, and update both production and replay to import and call that same symbol. Do not replay a convenient inner helper unless that exact helper is also the production root traced wrapper. Avoid duplicate semantic wrappers split across production and replay with names like \`runX\`, \`processX\`, or \`generateX\`; one production root symbol owns the key, and replay imports that symbol. For handler-instrumented keys with explicit-key replay, verify parity against the production keyed handler/run-wrapper entrypoint and call the same framework entrypoint production calls; this handler path is the explicit-key exception to exported-function replay, not permission to call a different helper. If exported-symbol parity is impossible, stop and document the concrete blocker that prevents any shared exported root symbol.
...
If **zero** valid candidates remain, skip to step {{step:analyze-repo/report}} and say so.`,
                ...
                },
                {
                    ...
                    kind: "reference",
                    refs: ["reference"],
                    toolCalls: ["read", "createTracePlan", "getTracePlan"],
                    title: "Build and upload a draft trace plan for each candidate",
                    body: `For **each** candidate selected in step {{step:analyze-repo/select}}, build a \`TracePlanTree\` and upload it with {{tool:createTracePlan}}. This is the same plan construction as an interactive Instrument cycle, **minus the browser confirmation**: do NOT run \`openTracePlan.js\`, do NOT open Studio, and do NOT run the "Presentation step" \`AskUserQuestion\` described in the Reference section{{#claude}} (the phase is not granted that tool){{/claude}}. Consult the **Trace Plan Format** and **Trace Plan Accuracy** rules in the Reference section below for span-type vocabulary and the tree grammar. Read each candidate's root signature (and any function whose parameter names or return fields the plan references) before building its tree; never guess names.
...
For every candidate call {{tool:createTracePlan}} with \`{ language, tree, capturedNodeIds, traceFunctionKey, source: "analyze_repo" }\`. **Always pass \`source: "analyze_repo"\`** here so these auto-drafted, unconfirmed plans are marked distinctly from interactively-confirmed ones (an interactive Instrument cycle omits \`source\`, which defaults to \`interactive\`):
