...
3. **On exit, parse the final JSONL line and route:**
   ...
   - \`{"event":"cancelled","planId":"<uuid>"}\`, the user cancelled from the browser. Take branch **C** (Modifications), {{askVerb}}: what do they want to change? Their answer feeds back into step {{step:modify/build-modified-plan}}. When the loop re-runs \`openTracePlan.js\` with the new plan, the script reuses the existing Studio browser tab automatically. **Exception:** a \`"reason":"never-connected"\` field means the user did NOT cancel, the Studio window never actually opened. Say that instead, and offer to re-run \`openTracePlan.js\` (the stale session is auto-cleared, so the re-run opens a fresh window).
...
{{tool:createTracePlan}} returns a plan id and a \`https://bitfab.ai/studio/trace-plan/<id>\` URL. **Collect the id, trace function key, workflow description, frameworks used, boundary \`file:line\`, refactor complexity to instrument, suggested methods to capture, methods to mock on replay, and real-data improvement for each candidate, but do not print plan URLs in the final report.** Process candidates independently: if one fails to build or upload, record the failure and continue with the rest, do not abort the batch. You may optionally read a plan back with {{tool:getTracePlan}} to confirm it persisted.`,
                ...
                },
                {
                    ...
                    body: `Print one plain-markdown summary (no \`AskUserQuestion\`{{#claude}}; the phase is not granted that tool{{/claude}}). Lead with a one-line count ("Uploaded N draft trace plans"), then a table or list with one row per uploaded plan:
- **trace function key**;
