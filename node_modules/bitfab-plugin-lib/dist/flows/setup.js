...
After building the plan and posting it with {{tool:createTracePlan}}, render it inline as ASCII (rules above), then {{askVerb}}:
...
What you can run
  {{cmd}}setup            Login, then instrument workflows until done
            ...
            steps: [
                {
                    ...
                    body: `**Detect the project language** (TypeScript, Python, Ruby, or Go). In a monorepo, identify which directories are **applications** (services, APIs, agents) vs **libraries** (SDKs, shared packages). Focus on application directories. **The only output of this step is that language-and-framework verdict; finding the AI workflows to instrument comes later, at step {{step:instrument/choose-discovery}}, so a grep hit here is a framework signal, not a workflow to read and characterize.** Also scan imports and package manifests for supported framework signals, and note which framework each application directory uses, step {{step:instrument/read-sdk-reference}} fetches the matching framework page alongside the language reference:
...
If the chosen root clears neither, **STOP, do not build or post the plan.** Return to step {{step:instrument/present-workflows}} and resolve it there first: **(a)** instrument via the framework handler/processor, **(b)** move the boundary inward to the first function whose inputs are serializable (not a refactor), or **(c)** refactor via the "Refactor confirmation" gate. Only build and post the tree once the root is replayable under (1) or (2). The sole exception is a root the user has EXPLICITLY accepted as observe-only / non-replayable for this workflow; absent that explicit acceptance, an unserializable root is never a plan you create. (Go-only projects don't support replay, so this gate doesn't apply, skip it.)
...
Then post the plan via {{tool:createTracePlan}}, render it inline as ASCII, and offer Studio as an optional review surface (the render-and-ask sequence is spelled out at the bottom of this step).
...
**Polling (mandatory, see the Blocking-process rule at the top of this skill):** keep polling the live exec session until the process exits. Do NOT wait for a chat message from the user; their confirmation arrives as stdout on the already-running process, not as a new prompt. Poll every few seconds until one of these terminal conditions, then route accordingly:{{/codex}}
  ...
  - The script emits JSONL to stdout. If it emits \`{"event":"window-open-requested","url":"..."}\`, immediately surface the URL in a normal chat message, e.g. \`Opening Studio: <url> - click it if a window doesn't appear\`, before continuing to poll. (This event means the open was *requested*, not that a window is confirmed on screen; the link is the reliable fallback when nothing surfaces.) \`{"event":"session-ready","sessionId":"<uuid>"}\` appears once the Studio session is established (on a logged-out run, an \`{"event":"auth-required",...}\` then \`{"event":"authenticated",...}\` line precede it while the user signs in, keep waiting for \`session-ready\`). On exit, parse the final JSON line:
    ...
    - non-zero exit (including \`{"event":"timeout",...}\`), surface the error and **STOP**, do not write instrumentation. A timeout or crash means Studio never resolved, that is NOT the user choosing Continue, so do not auto-proceed. {{askVerb}} whether to retry {{command:openTracePlan}} or to explicitly Continue (which then confirms the plan via {{tool:confirmTracePlan}} and proceeds to step {{step:instrument/write-instrumentation}}). Act only on their choice.

**If {{tool:createTracePlan}} itself errors** (offline or MCP unreachable, so there is no plan id and no browser option): render the ASCII plan from your in-memory tree using the "Trace Plan Format" reference section, derive the per-node mock decisions yourself with the procedure above, and **STOP**: {{askVerb}} to confirm before writing code.`,
...
The trace plan's \`Files changed:\` list must include the replay script path for this cycle (new or edited) alongside the instrumented files.`,
                ...
                },
                {
                    ...
                    body: `Give the user a clear completion message that explains how to run the instrumented workflow and, once traces exist, the replay script for this pipeline. If the repository reveals an exact command or user action that drives the real instrumented path, provide it. If it does not, name the application path or workflow that must be exercised without inventing a command. Always give the exact replay command when one was generated. Do NOT run either command yourself. (Omit the replay command for Go-only projects.) **If step {{step:instrument/present-workflows}} flagged this function as reading stored DB state** (TypeScript, Python, Ruby), add one line: replay currently reads today's data, run \`{{cmd}}setup db-snapshot\` to make it replay against the database state at trace time.
...
\`\`\``,
                ...
                },
                {
                    ...
                    body: `After the run instructions from step {{step:instrument/tell-user-how-to-run}}, {{askVerb}} what to do next:
                    ...
                    branches: [
                        {
                            option: {
                                ...
                                description: "[why it is the next highest-value workflow]",
                            },
                            recommended: true,
                            next: "instrument/identify-workflows",
                        },
                        {
                            option: {
                                ...
                                description: "scan again or read a different file, function, or directory",
                            },
                            next: "instrument/choose-discovery",
                        },
                        {
...
When applying a requested modification, read the relevant signatures so the plan stays accurate: for added context, name the exact keys/values and the span they attach to; for new instrumented spans, read each callee's signature and pick a type annotation (\`function\`, \`llm\`, \`tool\`, \`agent\`, \`handoff\`); for span removals, list each by name and confirm the underlying call is left untouched; for a new upstream/downstream root, read the new function's signature and confirm it still covers the interesting LLM/tool activity (upstream) or remains a common ancestor of every LLM/tool span (downstream).`,
                ...
                },
                {
                    ...
                    body: `**Post the modified plan, render it inline as ASCII, then {{askVerb}} whether to review it in the browser or just continue**, same delivery pattern as Instrument's build-trace-plan step. The inline ASCII is what the user reviews in chat; Studio is the optional richer surface where they can adjust the captured set (selecting/deselecting any of the surrounding \`pure\` context nodes added in step {{step:modify/build-modified-plan}}). Continue (or Confirm in Studio) = apply the diff. Cancel in Studio = ask the user what they want to change.

1. **Post the modified plan.** Call {{tool:createTracePlan}} with \`{ language, tree, capturedNodeIds, traceFunctionKey }\` (and \`stats\` if you have a sample run from the existing trace function). Do NOT open Studio here, rendering (step 2) and the browser-or-continue ask (step 3) come next:
...
3. **Then {{askVerb}}** what they want to do next. The primary choice is **View in browser** (open Studio, the richer surface for reviewing and toggling the captured set) or **Continue** (apply the diff using the plan as rendered). The full option set and routing:

{{branches}}

   - **View in browser**: run \`{{command:openTracePlan}} <planId>\` (\`{{pluginRoot}}\` resolves to the plugin directory; \`<planId>\` is the id from step 1). The script navigates Studio to the trace plan page and **blocks** until the user clicks **Confirm** or **Cancel**{{#codex}} (up to 30 minutes){{/codex}}. If it emits \`{"event":"window-open-requested","url":"..."}\`, immediately surface the URL in a normal chat message, e.g. \`Opening Studio: <url> - click it if a window doesn't appear\`, before continuing to poll.{{#codex}} Keep polling the live exec session until it exits (see the Blocking-process rule at the top of this skill); the user's decision arrives as stdout, not as a chat message.{{/codex}} On exit, parse the final JSONL line: \`{"event":"confirmed",...}\` routes to branch **A** (apply), \`{"event":"cancelled",...}\` routes to branch **C** (ask what to change, unless the line carries \`"reason":"never-connected"\`, which means the window never opened, say so and offer to re-run); a non-zero exit surfaces the error, then re-ask below. On \`confirmed\`, call {{tool:getTracePlan}} with the returned \`planId\` (which may differ from the original if a mid-session \`create_trace_plan\` created a new plan; \`openTracePlan.js\` auto-tracks the latest via \`tracePlan:created\` events) to read the authoritative \`capturedNodeIds\` (the user may have toggled \`pure\` context nodes into the set or removed captured ones), reconcile your edit plan with it (drop \`●\` wraps no longer captured, add wraps for newly captured nodes), then take branch **A**.
   - **Continue** (branch **A**): call {{tool:confirmTracePlan}} with the plan id from step 1 and your recommended \`capturedNodeIds\` to persist the modified plan as *confirmed* (Studio's Confirm does this on the browser path; Continue MUST do it here, or a later \`{{cmd}}setup view\`/\`{{cmd}}setup modify\` for this key won't find it), then apply the diff using the authoritative \`capturedNodeIds\` and per-node mock decisions it returns. Every node should already be classified; if a captured node somehow lacks \`analysis\`, classify it now with the decision procedure from step {{step:modify/build-modified-plan}} before instrumenting, never wrap a captured span without a mock decision.

**If {{tool:createTracePlan}} itself errors** (offline or MCP unreachable, so there is no plan id or browser option): render the inline before/after ASCII from your in-memory tree, derive the mock decisions yourself, and **STOP**: {{askVerb}} using the options above before writing edits.`,
                    branches: [
                        {
                            option: {
                                letter: "A",
                                label: "Continue",
                                description: "apply the diff using the plan as rendered inline (or the browser-confirmed capture set)",
                            },
                            recommended: true,
                        ...
                        },
                        {
                            option: {
                                ...
                                description: "open the plan in Studio to review and toggle the captured set; Confirm applies the diff, Cancel asks what to change",
                            },
                            next: "modify/apply-changes",
                        },
                        {
...
**Re-entry rule (applies after you leave this loop).** If, later in the conversation, the user asks to re-instrument or change another function's capture in plain language (\`re-instrument <fn>\`, \`change what this span records\`, \`give me the updated trace plan for <fn>\`), that is a fresh Modify (or Instrument) cycle: re-invoke \`{{cmd}}setup modify\` (name the mode, so it goes straight to Modify rather than falling back to the full \`wizard\`) so it runs through the trace-plan flow. **Never satisfy such a request by hand-writing a trace plan or before/after diff you made up as a chat message, that skips the {{tool:createTracePlan}} + inline ASCII render (and optional Studio review) this flow runs.**`,
                    branches: [
                        {
                            option: {
                                letter: "A",
...
- **"No prior confirmed trace plan found"**: there is no plan to view (key created outside the skill, never confirmed, or never instrumented via this skill). Tell the user there's nothing to view yet and suggest \`{{cmd}}setup modify\` to build and confirm a plan for this key. Take branch **B** (Stop).`,
                ...
                },
                {
                    ...
                    body: `**Render the trace plan inline as ASCII** from the plan fetched in step {{step:view/fetch-plan}}, using the "Trace Plan Format" reference section (default view: the captured spans, their types, and the tree as recorded). This is read-only, do not edit anything. Then {{askVerb}} whether to open it in the browser or finish:
