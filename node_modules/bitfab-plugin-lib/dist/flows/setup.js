...
export const setupFlow = Flow.parse({
    ...
    frontmatter: {
        ...
        argumentHint: "[all|login|login headless|instrument|modify|replay|templates]",
    },
    tools: {
...
{{/codex}}
This skill has five phases: **login**, **instrument**, **modify**, **replay**, and **templates**. Run individually or all at once (\`all\` runs login → instrument → replay; \`modify\` is only invoked explicitly or as a branch from the Instrument step {{step:instrument/search-existing}} menu; \`templates\` is only invoked explicitly).
