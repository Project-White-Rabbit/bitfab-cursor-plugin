...
Then present the trace plan **using the format defined in the "Trace Plan Format" reference section below** (legend → grammar → template precedence → canonical example). **STOP** — {{askVerb}} to confirm before writing code.`,
...
Every Modify cycle targets **exactly one** trace function and picks **exactly one** of five directions. Never batch multiple trace functions or mix directions in one cycle — if the user wants more, loop via the step {{step:modify/mandatory-stop}} menu.`,
            steps: [
                {
                    id: "gather-existing",
                    kind: "action",
                ...
                },
                {
                    ...
                    toolCalls: ["read"],
                    refs: ["reference"],
                    title: "Reconstruct the current trace plan",
                    body: '**Reconstruct the current trace plan.** Read the instrumented files to map the existing span tree. Render it as the "before" plan using the Default view template from the **Trace Plan Format** reference section. Do not present it yet — it becomes the left-hand side of the diff in step {{step:modify/present-diff}}.',
                    next: "modify/pick-direction",
                },
                {
                    id: "pick-direction",
                    kind: "action",
                    toolCalls: ["ask"],
                    title: "Pick exactly ONE direction",
                    body: `**Pick exactly ONE direction.** {{askVerbCap}} with all five directions below — recommend the one that matches the user's original ask and explain why in one line. Never mix directions in a single cycle.
...
| 5 | **Move root downstream** | Replace the root with a **callee** of the current root (narrower scope) | Interesting LLM/tool activity still sits under the new root |`,
                ...
                },
                {
                    ...
                    refs: ["reference"],
                    toolCalls: ["read"],
                    title: "Build the modified trace plan",
                    body: `**Build the modified trace plan under the same PURELY ADDITIVE constraint as Instrument step {{step:instrument/build-trace-plan}}.** The modified tree must be implementable without behavior changes. If the chosen direction requires awaiting a stream that wasn't awaited, delaying a call, reordering operations, blocking a callback, or restructuring control flow, the direction is invalid for this cycle — tell the user which direction doesn't fit and why, then return to step {{step:modify/pick-direction}} for a different direction (or suggest splitting into multiple cycles). Never present a behavior-changing approach as an option.
...
- **Move root downstream** — the new root must still cover the interesting LLM/tool activity. If critical LLM spans live outside the downstream callee, downstream is invalid.`,
                ...
                },
                {
                    ...
                    toolCalls: ["ask"],
                    refs: ["reference"],
                    title: "Present before/after diff and confirm",
                    body: `**Present a before/after diff** using the **Trace Plan Format** reference section:
...
Below the two plans, list \`Files changed:\` for the edits this cycle will make — paths only, no annotations. **STOP** — {{askVerb}}:

{{branches}}`,
                    branches: [
                        {
                            option: { letter: "A", label: "Proceed" },
                            recommended: true,
                            next: "modify/decide-key",
                        },
                        {
                            option: {
                                letter: "B",
                                label: "Expand details",
                                description: "re-render both plans in the expanded view",
                            },
                            next: "modify/present-diff",
                        },
                        {
                            option: {
                                letter: "C",
                                label: "Adjust",
                                description: "user wants changes — ask what",
                            },
                            next: "modify/build-modified-plan",
                        },
                        {
                            option: { letter: "D", label: "Cancel" },
                            next: null,
                        },
                    ],
                },
                {
                    ...
                    kind: "branch",
                    toolCalls: ["ask"],
                    title: "Decide the trace function key",
                    body: `**Decide the trace function key.** Directions 1–3 always keep the existing key. Directions 4–5 change the root function, so the existing key may no longer describe it. {{askVerbCap}}:
...
Skip this step for directions 1–3.`,
                    branches: [
                        {
                            option: {
                                letter: "A",
                                label: "Keep `<existing>`",
                                description: "new traces continue to aggregate with historical traces on the same key (when the new root plays the same role)",
                            },
                            recommended: true,
                        ...
                        },
                        {
                            option: {
                                letter: "B",
                                label: "Rename to `<suggested-new-key>`",
                                description: "starts a fresh trace function. Historical traces on the old key are preserved but will not appear under the new key",
                            },
                            next: "modify/apply-changes",
                        },
                    ],
                },
                {
                    ...
                    body: "**Apply the changes — purely additive to behavior.** Same rules as Instrument step {{step:instrument/write-instrumentation}}: never change arguments, return values, error handling, variable names, types, control flow, or code structure. Removing a `withSpan`/`@span` wrapper (direction 3) is the only structural edit allowed, and only when it leaves the wrapped call, its arguments, and its return value untouched. Batch repetitive edits in parallel (one message, many Edit calls).",
