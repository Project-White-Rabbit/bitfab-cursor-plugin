...
Every Modify cycle targets **exactly one** trace function. Never batch multiple trace functions in one cycle, if the user wants more, loop via the step {{step:modify/mandatory-stop}} menu.`,
            steps: [
                ...
                },
                {
                    ...
                    kind: "action",
                    toolCalls: ["getTracePlan", "read", "grep"],
                    refs: ["reference"],
...
1. Call {{tool:getTracePlan}} with \`{ traceFunctionKey: "<chosen key>" }\` (no \`planId\`). Two outcomes:
   ...
   - **"No prior confirmed trace plan found"**: there is no stored snapshot for this key yet (key created outside the skill, or first Modify cycle that predates this column). You have no prior plan id, so step {{step:modify/present-diff}} creates a plan after the current-code read.
