...
export const setupFlow = Flow.parse({
    ...
    tools: {
        ...
        updateTracePlan: { kind: "mcp", name: "update_trace_plan" },
            ...
            steps: [
                {
                    ...
                    body: `Render the block below **verbatim** as a single message, as formatted markdown (do **not** wrap it in a code fence, do **not** reword it, and do **not** add a summary or an ASCII diagram). This is the education the rest of setup depends on: a user who does not understand \`withSpan\` and \`replay\` cannot make the per-method decisions instrumentation asks of them. Do **not** authenticate, scan the codebase, {{^codex}}use AskUserQuestion{{/codex}}{{#codex}}ask any questions{{/codex}}, or edit anything here, in either mode.
                    ...
                    branches: [
                        ...
                        },
                        {
                            option: {
                                ...
                                description: "adjust what an existing trace captures",
                            },
...
Then {{askVerb}} how they'd like to find what to instrument:
...
If they pick **A**, go to step {{step:instrument/reconcile-existing-plan}} to wire up the picked drafts (a draft is a starting point, not a commitment: it is still presented for review and can be adjusted or cancelled before any code is written). If they pick **B**, do the full codebase scan in step {{step:instrument/identify-workflows}}. If they pick **C**, ask which file, function, or directory they want to instrument (if they haven't already named it) and go to step {{step:instrument/read-target}} to read just that location, skipping the broad scan.`,
                    branches: [
                        {
                            option: {
                                letter: "A",
                ...
                },
                {
                    ...
                    kind: "branch",
                    toolCalls: ["ask", "read", "grep", "getTracePlan", "listFunctions"],
...
**A** is the normal reuse case: it carries the existing draft plan id into step {{step:instrument/build-trace-plan}}, which presents the already-built draft (no re-build) and opens/confirms it exactly like a freshly built plan. **B** and **C** skip this draft (no presentation, no write) and loop back here for the next queued draft, so a moot draft never reaches the user. For **B**, tell the user it is already set up and offer \`{{cmd}}setup modify <key>\` to change what it captures (or \`{{cmd}}assistant\` to iterate on its quality). For **C**, tell the user the draft is out of date and offer to re-discover the workflow via a fresh scan (option B/C in step {{step:instrument/choose-discovery}}) before skipping it.`,
                    branches: [
                        {
                        ...
                        },
                        {
                            ...
                            option: {
                                letter: "B",
                                label: "Already instrumented, skip",
                                description: "re-wrapping would duplicate the root span; offer `setup modify` and move to the next draft",
                            },
                            next: "instrument/reconcile-existing-plan",
                        },
                        {
                            when: "stale: the root or captured nodes no longer exist at the recorded location (renamed, moved, or deleted)",
                            option: {
                                letter: "C",
                                label: "Stale draft, skip",
                                description: "the plan no longer matches the code; offer a fresh scan and move to the next draft",
                            },
                            next: "instrument/reconcile-existing-plan",
                        },
                    ],
...
**Reuse entry (analyze-repo draft):** if you reached this step from step {{step:instrument/reconcile-existing-plan}}, the plan already exists as a draft, you hold its plan id and read its tree there with {{tool:getTracePlan}}. Do NOT build a new tree or call {{tool:createTracePlan}}, and skip the replayability requirement and every tree-construction bullet below: analyze-repo already built and classified the draft, and reconcile already checked its root against the current code. Go straight to the "**Render the trace plan inline as ASCII**" bullet using the draft's plan id and tree, then present and confirm it (Continue → {{tool:confirmTracePlan}} the draft id; View in browser → {{command:openTracePlan}} the draft id) exactly as for a freshly built plan. Everything from that bullet on is shared.
...
(\`{{pluginRoot}}\` resolves to the plugin directory; \`<planId>\` is the id returned by {{tool:createTracePlan}}.) The script navigates Studio to the trace plan page and stays alive until the user leaves that page{{#codex}} (up to 30 minutes){{/codex}}. Studio offers one button there: **Close** (leave the plan exactly as you drafted it) or **Save** (leave, saving the capture and mock toggles they made). **Nothing on that page is an abort:** Close, Update, and closing the window all mean "keep going", and none of them is a request for another round of plan edits.
...
**Polling (mandatory, see the Blocking-process rule at the top of this skill):** keep polling the live exec session for its terminal line; the user's Close/Update arrives as stdout on the already-running process, not as a chat message.{{/codex}}
  ...
  - **Nothing yet, and they typed a change instead:** close the open plan with \`{{command:openStudio}} "/studio"\`, stop the background process, then rebuild the tree and start this step over from {{tool:createTracePlan}}. The superseded plan needs no cleanup; it expires on its own.
...
**First, silently re-probe {{tool:listTracePlans}} with \`{ source: "analyze_repo", status: "awaiting" }\` on EVERY pass through this step** (always, even if you already probed at step {{step:instrument/choose-discovery}} earlier this session: the remaining-draft set shrinks as you wire them up, so a draft the user hasn't instrumented yet must still surface here). Do not narrate the probe itself. **Exclude from the offer any draft you already reconciled and skipped this session as already-instrumented or stale at step {{step:instrument/reconcile-existing-plan}}**: skipping does not confirm or cancel it, so it stays \`awaiting\` and the probe keeps returning it, but re-recommending a draft you already told the user is moot just loops them on it. If every remaining awaiting draft has already been instrumented or skipped this session, treat it as no drafts left (omit option A). Then {{askVerb}} what to do next:
                    ...
                    branches: [
                        ...
                        },
                        {
                            option: {
                                letter: "C",
                                label: "Instrument a specific target",
                                description: "name a different file, function, or directory to instrument",
                            },
                            next: "instrument/read-target",
                        },
                        {
...
1. Call {{tool:getTracePlan}} with \`{ traceFunctionKey: "<chosen key>" }\` (no \`planId\`). Two outcomes:
   - **Prior plan found**: parse the JSON block in the response. Use its \`tree\` as the \`before\` \`TracePlanTree\` and its \`capturedNodeIds\` as the current capture set. You do not need to re-read the instrumented files. **Also record the plan's \`id\`** (the \`Trace plan: <uuid>\` line at the top of the response): step {{step:modify/present-diff}} updates THAT plan in place rather than uploading a new one. Skip step 2.
   - **"No prior confirmed trace plan found"**: there is no plan for this key yet (key created outside the skill, or first Modify cycle that predates this column). Fall through to step 2. You have no prior plan id, so step {{step:modify/present-diff}} creates a plan instead of updating one.
...
1. **Post the modified plan.** Do NOT open Studio here, rendering (step 2) and the browser-or-continue ask (step 3) come next. Which tool you call depends on whether step {{step:modify/reconstruct-current-plan}} found a prior plan:
   ...
   - **No prior plan (the code-reading fallback in step {{step:modify/reconstruct-current-plan}}): call {{tool:createTracePlan}}** with \`{ language, tree, capturedNodeIds, traceFunctionKey }\` (and \`stats\` if you have one), exactly as Instrument does. Persisting the key lets the next Modify cycle bootstrap from this plan.

   For either tool: \`tree\` is the modified \`after\` \`TracePlanTree\` from step {{step:modify/build-modified-plan}}, with the ~10 surrounding callers / ~10 surrounding callees included as \`pure\` context nodes. Every node carries the \`analysis\` you set/carried-forward in that step, including uncaptured context nodes. \`capturedNodeIds\` is your initial recommendation and must form a connected sub-tree with exactly one entry point (selecting any descendant implies its ancestors); surrounding \`pure\` context nodes are not included. The server derives the validation card (status pill + aggregate counts) from the per-node \`analysis\`, so you don't send a summary. Both tools return the plan id (and a \`https://bitfab.ai/studio/trace-plan/<id>\` URL); for the update path that is the same id you already held.
...
**If the post in step 1 itself errors** ({{tool:updateTracePlan}} or {{tool:createTracePlan}}, e.g. offline or MCP unreachable, so there is no browser option): render the inline before/after ASCII from your in-memory tree, derive the mock decisions yourself, and **STOP**: {{askVerb}} using the options above before writing edits. One error is not fatal in the same way: if {{tool:updateTracePlan}} reports the plan is **cancelled**, that plan is dead and cannot be revived, fall back to {{tool:createTracePlan}} for a fresh plan and carry on.`,
                    branches: [
                        {
                            option: {
                                letter: "A",
...
**If the user picks "Refactor" (or a boundary move that requires rewriting callers), present a refactor plan labeled as *visibility* or *structural* and get a second confirmation before modifying code (the "Refactor confirmation" rules below say what the plan must contain).**`,
                    branches: [
                        {
                            when: "every instrumented function is invocable from the replay script and its root is replayable (nothing left to resolve)",
...
For every candidate call {{tool:createTracePlan}} with \`{ language, tree, capturedNodeIds, traceFunctionKey, source: "analyze_repo" }\`. **Always pass \`source: "analyze_repo"\`** here so these auto-drafted, unconfirmed plans are marked distinctly from interactively-confirmed ones (an interactive run omits \`source\`, which defaults to \`interactive\`):
