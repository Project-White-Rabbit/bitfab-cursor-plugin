...
export const setupFlow = Flow.parse({
    ...
    intro: `{{^codex}}**Always use \`AskUserQuestion\` when asking questions or presenting choices** (one exception: a step that explicitly says its answer is free-form, such as asking which file or function to instrument once the user has said they know, asks in plain chat and waits, because a menu there would stand between the user and the answer only they hold). Never print a question as text and wait.{{/codex}}{{#codex}}**When the flow branches, always present the options clearly and wait for the user's answer before proceeding.** Number or letter the options so the user can pick by reference.{{/codex}} Rules:
...
**If {{tool:apiKey}} is not available in this session**, the MCP server is switched off, not broken: it ships inside this plugin, so an absent tool is a setting rather than a failed install. Say so and hand the user the fix below; do not diagnose further, and do not fall back to drafting anything by hand.

{{#claude}}Tell them to run \`/mcp\`, enable **Bitfab**, then re-run this skill. The tools load into the running session, so nothing is lost. Do **not** suggest restarting: the setting is stored per project and survives a relaunch, so they would come back just as stuck. Ignore \`claude mcp list\` here, it health-checks the server in its own process and prints \`Connected\` even when this session cannot reach it.{{/claude}}{{^claude}}Tell them to re-enable the Bitfab MCP server in their editor's MCP settings, then re-run this skill.{{/claude}}
