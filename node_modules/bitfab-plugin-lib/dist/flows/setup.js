...
3. **{{^codex}}AskUserQuestion{{/codex}}{{#codex}}Ask the user{{/codex}}** with exactly two options:
   ...
   - **"Cancel"** — return to the previous {{askNoun}} (step {{step:instrument/present-workflows}}'s (a)/(b), or Replay step {{step:replay/safety-net}}'s three-option prompt) so the user can pick a different resolution
...
- **Inputs** — the shape of the function's inputs, and an explicit note that they're serializable by the SDK's tracing layer. If the natural outer boundary's inputs are unserializable (live browser/runtime objects, HTTP req/res, stream writers, sockets, opaque request contexts), state that here and present the two resolutions from step {{step:instrument/explain-trace-boundary}} as part of this workflow's entry: **(a) move the boundary inward to \`<specific inner function with serializable inputs>\`** (recommended when an obvious candidate exists — not a refactor), or **(b) refactor**. Do not proceed to step {{step:instrument/read-signatures}} until the user picks one — never instrument an unserializable root. **If the user picks (b), present a refactor plan — labeled as *visibility* (extract + export, logic unchanged) or *structural* (new pure-core fn) — and get an explicit second confirmation before modifying code. See the "Refactor confirmation" rule below.**
            ...
            steps: [
                {
                    ...
                    body: "**Gather existing trace functions** by searching for SDK patterns ({{sdkPatterns}}). List each key alongside its root function. If none are found, tell the user Modify needs existing instrumentation and suggest `{{cmd}}setup instrument`.",
            ...
            steps: [
                {
                    ...
                    toolCalls: ["grep", "glob"],
                    title: "Gather existing trace functions",
                    body: "**Gather existing trace functions** by searching for SDK patterns ({{sdkPatterns}}). List each key alongside its root function. If none are found, tell the user View needs existing instrumentation and suggest `{{cmd}}setup instrument`.",
...
- Live in a \`scripts/\` directory (or the project's existing scripts location)`,
                ...
                },
                {
                    ...
                    body: `**Safety net for legacy instrumentation.** If an already-instrumented function (introduced before step {{step:instrument/explain-trace-boundary}}'s serializability gate, or via another path) can't be invoked from the replay script — most commonly because it isn't exported, is defined inline in a route handler, or takes unserializable inputs — {{askVerb}} offering step {{step:instrument/explain-trace-boundary}}'s two resolutions:
