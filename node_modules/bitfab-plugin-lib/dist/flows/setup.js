...
{{/codex}}
This skill has seven phases: **login**, **session-logs**, **instrument**, **modify**, **view**, **replay**, and **templates**. Run individually or all at once (\`all\` runs login → instrument → replay; \`session-logs\` is standalone and does not require login; \`modify\` is only invoked explicitly or as a branch from the Instrument step {{step:instrument/search-existing}} menu; \`view\` is only invoked explicitly; \`templates\` is only invoked explicitly).
...
\`\`\`
{{^codex}}
This opens Studio for sign-in and polls until authentication completes. Run with 600000ms timeout (10 minutes). If the command **exits with an error** or **times out**, fall through to the **Login (headless)** flow below.{{/codex}}{{#codex}}
...
Only fall through to **Login (headless)** below when \`login.js\` exits non-zero or the 10-minute timeout elapsed.{{/codex}}`,
                    branches: [
                        {
                            when: "login succeeds",
                            next: "login/get-api-key",
...
\`\`\``,
                    next: {
                        byMode: {
                            ...
                            "login-headless": "cleanup/close-studio",
        ...
        },
        {
            ...
            steps: [
                ...
                },
                {
                    ...
                    body: `Tell the user:
> Open this URL in a browser on any device: **{serviceUrl}/studio/auth/{{authTarget}}**
>
