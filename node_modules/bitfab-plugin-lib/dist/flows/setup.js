...
| \`{{cmd}}setup db-branching\` | Set up per-trace database branching so replay runs against the DB state at trace time (TypeScript, Python, Ruby) |
...
| \`{{cmd}}setup templates [<key>]\` | Iterate on the span-rendering templates for one trace function |`,
    entries: {
        ...
        "db-branching": "db-branching/preflight",
    ...
    appendices: [
        {
            ...
            title: "Refactor confirmation (applies to Instrument step {{step:instrument/present-workflows}} and Replay step {{step:replay/safety-net}})",
...
3. **{{^codex}}AskUserQuestion{{/codex}}{{#codex}}Ask the user{{/codex}}** with exactly two options:
   ...
   - **"Cancel"**: return to the previous {{askNoun}} (step {{step:instrument/present-workflows}}'s (a)/(b)/(c), or Replay step {{step:replay/safety-net}}'s three-option prompt) so the user can pick a different resolution

Never modify existing code on a refactor path without completing this three-step confirmation. Adding new instrumentation wrappers to unchanged functions is not a refactor, this rule does not apply to step {{step:instrument/write-instrumentation}}'s purely-additive instrumentation.`,
        },
        {
            id: "reference",
            title: "Reference",
...
Then close with one line: to start tracing, run \`{{cmd}}setup\`; to debug an existing setup, run \`{{cmd}}setup inspect\`.`,
        ...
        },
        {
            ...
            outro: "**If running `login` only**, stop here and report the result.",
            steps: [
                {
                    id: "status-check",
                    kind: "branch",
...
\`\`\``,
                    next: {
                        byMode: {
                            ...
                            "db-branching": "cleanup/close-studio",
...
If the output is \`true\`, tell the user session logs are currently **enabled**. If \`false\`, tell the user session logs are currently **disabled**. Then {{askVerb}}:
- **Question:** "Allow Bitfab to collect session logs?"
...
Recommend one to start with. **Ask the user to pick exactly ONE workflow to instrument first.** Never accept "multiple" or "all", each Instrument cycle produces exactly one trace function with one trace plan and one set of code changes. If the user wants to instrument several, they will be done sequentially via the loop in step {{step:instrument/mandatory-stop}}, one at a time.`,
                ...
                },
                {
                    ...
                    body: '**Read function signatures you\'ll reference in the trace plan**: root function first, then any whose parameter names or return fields aren\'t already obvious from the step {{step:instrument/identify-workflows}} scan. Skipped leaf functions only need their names; don\'t Read them unless their shape appears in the plan. Never guess names. See "Trace Plan Format" and "Trace Plan Accuracy" in the Reference section below.',
...
**Inline fallback** (use only if {{tool:createTracePlan}} errors, e.g. offline or MCP unreachable): present the trace plan **using the format defined in the "Trace Plan Format" reference section below** (legend → grammar → template precedence → canonical example). **STOP**: {{askVerb}} to confirm before writing code.`,
                ...
                },
                {
                    ...
                    body: `{{^codex}}**Write the instrumentation edits (11a) and the replay pipeline (11b) for this trace function in the same cycle. Default to writing both yourself, inline**, reusing what you already hold in context: the \`#replay\` section you fetched in step {{step:instrument/read-sdk-reference}} and the root / deps / entrypoint files you read in steps {{step:instrument/identify-workflows}} and {{step:instrument/read-signatures}}. Do NOT re-fetch docs or re-read those files. That cold-context reload is the main cost a subagent adds, and for a typical single-root instrumentation (a handful of edits) inline is faster than dispatching one. Skip the replay pipeline entirely for Go-only projects (Go does not support replay).
...
A, B, and C all return to step {{step:instrument/present-workflows}} for the selected workflow. Only D exits the Instrument loop.
...
**After D in \`wizard\` mode, Replay ALWAYS runs** as a coverage-verification/backfill sweep. Step {{step:instrument/write-instrumentation}} already wrote a replay pipeline for every trace function instrumented in this session, so Replay is usually a no-op that confirms coverage; it still runs to catch any pre-existing trace function keys that don't yet have a pipeline and to verify Replay Output Contract compliance across all pipelines. Replay does not depend on traces existing, replay scripts are built from trace function keys in the instrumented code, not captured trace data. In \`instrument\` mode, D stops after the Instrument loop.`,
        ...
        },
        {
            ...
            title: "Modify",
            stepStyle: "list",
            intro: `Adjust an **existing** trace setup. Requires existing SDK usage in the codebase, if none exists, run Instrument first. Triggered explicitly by \`{{cmd}}setup modify\`, or selected from the {{askNoun}} at Instrument step {{step:instrument/search-existing}} when existing SDK usage is found.
...
2. **Code-reading fallback.** Read the instrumented files to map the existing span tree into a \`TracePlanTree\` (\`{ rootId, nodes: { [id]: TraceNode } }\`, same shape used in Instrument step {{step:instrument/build-trace-plan}}). Each \`TraceNode\` carries \`id\`, \`name\`, \`kind\` ("manual" | "auto" | "pure"), \`file\`, \`line\`, \`signature\`, \`parentId\`, \`childIds\`, plus \`framework\` for \`[auto]\` lines.
...
**Every captured node MUST include \`sampleInput\` and \`sampleOutput\`**: same hard rule as Instrument step {{step:instrument/build-trace-plan}}. Carry samples forward unchanged for surviving nodes; for newly added nodes (intermediate spans, deeper leaves, a new upstream/downstream root), construct realistic example values from the function's parameter and return types (Read the file and its return-type imports if needed). Do not advance to step {{step:modify/present-diff}} with a captured node missing either field.
...
When applying a requested modification, read the relevant signatures so the plan stays accurate: for added context, name the exact keys/values and the span they attach to; for new instrumented spans, read each callee's signature and pick a type annotation (\`function\`, \`llm\`, \`tool\`, \`agent\`, \`handoff\`); for span removals, list each by name and confirm the underlying call is left untouched; for a new upstream/downstream root, read the new function's signature and confirm it still covers the interesting LLM/tool activity (upstream) or remains a common ancestor of every LLM/tool span (downstream).`,
                ...
                },
                {
                    ...
                    body: `**Send the modified plan straight to the trace plan UI, it is the user's primary surface for confirming or editing the change**, not the inline before/after diff. The user can adjust the captured set directly in the UI (selecting/deselecting any of the surrounding \`pure\` context nodes added in step {{step:modify/build-modified-plan}}). Confirm in the UI = apply the diff. Cancel = ask the user what they want to change. Same delivery pattern as Instrument step {{step:instrument/build-trace-plan}}.
...
{{branches}}`,
                ...
                },
                {
                    ...
                    body: "**Apply the changes, purely additive to behavior.** Same rules as Instrument step {{step:instrument/write-instrumentation}}: never change arguments, return values, error handling, variable names, types, control flow, or code structure. Removing a `withSpan`/`@span` wrapper is the only structural edit allowed, and only when it leaves the wrapped call, its arguments, and its return value untouched. The trace function key from step {{step:modify/pick-function}} stays the same, do not rename keys. Batch repetitive edits in parallel (one message, many Edit calls).",
...
B returns to step {{step:modify/pick-function}}. A and C exit the Modify loop. After exit, stop (Modify does not auto-continue to Replay, the user can invoke \`{{cmd}}setup replay\` separately).`,
                    branches: [
                        {
                            option: {
                                letter: "A",
...
- **Replay**: ✅ covers all keys / ⚠️ missing keys / ❌ none (from step {{step:inspect/check-freshness}}).
...
Do not print or ask for the API key, and do not surface the \`apiKey\` value to the user; the command replaces the plugin's copy for you and hands you that value solely for the next step.`,
                ...
                },
                {
                    ...
                    body: `**Only run this step if the previous step reported \`{"event":"switched"}\`.** If the switch returned \`not-member\` or \`error\` you already stopped, the plugin key was not replaced, and there is nothing to sync, so skip this step entirely.
...
- **Copy-pasteable script template + replay output contract + input serialization caveat:** \`/typescript-sdk#replay\`, \`/python-sdk#replay\`, \`/ruby-sdk#replay\`. Use this for the \`scripts/replay.<ext>\` shape and the rules for what to print to stdout.`,
            steps: [
                ...
                },
                {
                    ...
                    body: `**Compare coverage.** Replay is non-interactive once entered, do not ask the user whether to create or add scripts:
...
- If replay scripts exist but are missing trace function keys: add the missing scripts in step {{step:replay/create-scripts}}.
- If no replay scripts exist: create them in step {{step:replay/create-scripts}}.`,
                ...
                },
                {
                    ...
                    body: `**Create the replay script** following the example in the SDK reference's Replay section (\`https://docs.bitfab.ai/<language>-sdk#replay\`), adapted to this codebase. The non-negotiables (enforced by the docs page, repeated here so the script review catches them):
...
- Live in a \`scripts/\` directory (or the project's existing scripts location)`,
                ...
                },
                {
                    ...
                    body: `**Safety net for legacy instrumentation.** If an already-instrumented function (introduced before step {{step:instrument/explain-trace-boundary}}'s serializability gate, or via another path) can't be invoked from the replay script, most commonly because it isn't exported, is defined inline in a route handler, or takes unserializable inputs, {{askVerb}} offering step {{step:instrument/explain-trace-boundary}}'s boundary resolutions:

**Handler-instrumented keys are not a safety-net case.** A key registered only via a framework handler has no decorated function by design; create its pipeline with the key-based replay pattern from step {{step:replay/create-scripts}} instead of offering these resolutions.
                    ...
                    branches: [
                        ...
                        },
                        {
                            option: {
                                letter: "C",
                                label: "Leave as-is",
                                description: "add a header comment noting why the function isn't callable and flag that the script will rot",
                            },
                            next: "cleanup/close-studio",
                        },
                    ],
            ...
            steps: [
                {
                    ...
                    body: `**Confirm the SDK language.** DB-branching replay is available for **TypeScript, Python, and Ruby**. If the project is **Go**, stop and tell the user Go has no replay, so this doesn't apply.
...
Do **not** ask the user to set any \`BITFAB_NEON_*\` or \`NEON_API_KEY\` environment variables, those are Bitfab-side server config, not customer config. The customer only pastes their source Postgres URL in the dashboard.`,
                    next: "db-branching/await-connection",
                },
                {
                    id: "await-connection",
                    kind: "branch",
...
If the active flag is **false** for a freshly captured trace, either the source database isn't connected (re-check the dashboard Database section, step {{step:db-branching/connect-db}}) or the SDK predates always-on capture (upgrade with \`{{cmd}}update\`).
