...
(\`{{pluginRoot}}\` resolves to the plugin directory; \`<planId>\` is the id returned by {{tool:saveTracePlan}}.) The script navigates Studio to the trace plan page and stays alive until the user leaves that page{{#codex}} (up to 30 minutes){{/codex}}. Studio offers one button there: **Close** (leave the plan exactly as you drafted it) or **Save** (leave, saving the capture and mock toggles they made). **Nothing on that page is an abort:** Close, Update, and closing the window all mean "keep going", and none of them is a request for another round of plan edits.
...
**Polling (mandatory, see the Blocking-process rule at the top of this skill):** keep polling the live exec session for its terminal line; the user's Close/Update arrives as stdout on the already-running process, not as a chat message.{{/codex}}
  ...
  - **Nothing yet, and they typed a change instead:** close the open plan with \`{{command:openStudio}} "/studio"\`, stop the background process, then rebuild the tree and start this step over from {{tool:saveTracePlan}}. The superseded plan needs no cleanup; it expires on its own.
  - The script emits JSONL to stdout. If it emits \`{"event":"window-open-requested","url":"..."}\`, immediately surface the URL in a normal chat message, e.g. \`Opening Studio: <url>. Click it if a window doesn't appear\`, before continuing to read. (This event means the open was *requested*, not that a window is confirmed on screen; the link is the reliable fallback when nothing surfaces.) \`{"event":"session-ready","sessionId":"<uuid>"}\` appears once the Studio session is established (on a logged-out run, an \`{"event":"auth-required",...}\` then \`{"event":"authenticated",...}\` line precede it while the user signs in, keep waiting for \`session-ready\`). On exit, parse the final JSON line:
    - \`{"event":"confirmed","planId":"<uuid>"}\`, the user clicked **Close** or **Save**. Continue instrumenting, this is not a request to revise the plan. The \`planId\` may differ from the original if a mid-session \`save_trace_plan\` call created a new plan (the script auto-tracks the latest plan via \`tracePlan:created\` events). Call {{tool:getTracePlan}} with the returned \`planId\` to read the authoritative \`capturedNodeIds\` for step {{step:instrument/write-instrumentation}}. If it differs from your initial recommendation (they clicked **Save**), prune \`[auto]\` lines whose ancestor manual span was uncaptured, and drop manual \`●\` wraps that aren't in the set. Every node should already be classified; if a confirmed captured node somehow lacks \`analysis\`, classify it now with the same decision procedure before instrumenting, never wrap a captured span without a mock decision (the UI renders an unclassified captured node as plain "runs live", which would silently let a side effect fire).
...
1. **Post the modified plan.** Do NOT open Studio here, rendering (step 2) and the browser-or-continue ask (step 3) come next. Which tool you call depends on whether step {{step:modify/reconstruct-current-plan}} found a prior plan:
   ...
   - **No prior plan (the code-reading fallback in step {{step:modify/reconstruct-current-plan}}): call {{tool:saveTracePlan}}** with \`{ language, tree, capturedNodeIds, traceFunctionKey }\` (and \`stats\` if you have one), exactly as Instrument does. Persisting the key lets the next Modify cycle bootstrap from this plan.
...
If **zero** valid candidates remain, skip to step {{step:analyze-repo/report}} and say so.`,
                ...
                },
                {
                    ...
                    kind: "reference",
                    refs: ["reference"],
                    toolCalls: ["read", "saveTracePlan", "getTracePlan"],
...
For every candidate call {{tool:saveTracePlan}} with \`{ language, tree, capturedNodeIds, traceFunctionKey, source: "analyze_repo" }\`. **Always pass \`source: "analyze_repo"\`** here so these auto-drafted, unconfirmed plans are marked distinctly from interactively-confirmed ones (an interactive run omits \`source\`, which defaults to \`interactive\`):
