 ...
 */
export function detectLegacyInstall(platform) {
    if (!platform.legacyMarketplaceKey) {
        return false;
    }
    try {
        ...
        const data = JSON.parse(content);
        const entries = data.plugins?.[platform.legacyMarketplaceKey];
 ...
 */
export function legacyMigrationMessage(platform) {
    const legacyMarketplace = platform.legacyMarketplaceKey.split("@")[1];
    return [
        `[Bitfab] You're running from the old "${legacyMarketplace}" marketplace. Migrate to the official marketplace:`,
        ``,
