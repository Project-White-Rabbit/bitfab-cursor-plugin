...
import { extractSkillFromPrompt } from "../hooks/skillDetection.js";
import { ampTranscriptPath, arm, armedAtMs, clearAgentRunId, disarm, ensureSessionsDir, isArmed, isBitfabMcpToolName, listArmedSessionIds, localOffsetPath, readAgentRunId, readArmedMetadata, } from "./arming.js";
import { appendChatSession, getChatSessionState, } from "./client.js";
import { readTranscriptDelta } from "./transcriptReader.js";
function agentRunIdFromEnv() {
    const value = process.env.BITFAB_AGENT_RUN_ID;
    return value && value.trim() !== "" ? value : undefined;
}
function configFilePath() {
    return path.join(os.homedir(), ".config", "bitfab", "config.json");
}
export function isLocalCaptureDisabled() {
 ...
 * agent.
 */
export async function captureFromHook(hookType, input, agent = "claude-code", pluginVersion) {
    ...
    // SubagentStop fires with the MAIN session's session_id but the subagent's
    // own transcript_path. Doing anything here is harmful: flushing would read the
    // subagent transcript under the main session's offset state (corrupting it),
    // and the terminal cleanup below would disarm the still-running main session,
    // dropping every turn after the first subagent completes. The main transcript
    // is already captured by PreToolUse / UserPromptSubmit / Stop, so SubagentStop
    // has nothing to contribute. Ignore it entirely.
    if (hookType === "SubagentStop") {
        return { status: "skipped_subagent_stop" };
    }
    if (hookType === "PreToolUse" &&
        input.tool_name &&
        isBitfabMcpToolName(input.tool_name)) {
        armSession(input, agent, pluginVersion);
    }
    if (hookType === "UserPromptSubmit" &&
        input.prompt &&
        extractSkillFromPrompt(input.prompt) !== null) {
        armSession(input, agent, pluginVersion);
    }
    if (hookType === "SessionStart") {
        pruneDeadSessions(sessionId);
    }
    // A session ends on SessionEnd (the user actually exits the session) or, as a
    // backstop, the server-side idle timeout. Stop fires at the end of every
    // assistant turn, so it must NOT be treated as terminal: doing so marks the
    // session ended (and triggers the summary) the moment the user stops
    // interacting, even mid-conversation. Stop only flushes captured turns.
    ...
    const result = await flushDelta({
        ...
        agentRunId: agentRunIdFromEnv() ??
            readAgentRunId(sessionId) ??
            readArmedMetadata(sessionId)?.agentRunId,
...
export function armSession(input, agent, pluginVersion) {
...
async function flushDelta(args) {
    ...
    const stat = fs.statSync(args.transcriptPath);
    if (state.offset > stat.size) {
        const remote = await getChatSessionState(args.sessionId);
        const sequenceOffset = remote?.exists
            ? (remote.nextSequence ?? remote.turnCount)
            : 0;
    ...
    if (!result.ok &&
        result.reason === "offset_conflict" &&
        result.lastTranscriptOffset > stat.size) {
...
function writeOffsetState(offsetFile, state) {
    fs.writeFileSync(offsetFile, JSON.stringify(state));
}
...
// A new session starting is a convenient moment to prune local bookkeeping for
// other sessions whose transcript no longer exists. Once Claude Code has removed
// the transcript, that session can never resume or be re-captured, so its marker
// and offset files are dead weight.
//
// We deliberately do NOT end, flush, or otherwise touch sessions whose
// transcript still exists. A live parallel session must never be marked ended by
// a sibling starting up (doing so fired its summary minutes in, mid-work). The
// server-side idle backstop (IDLE_CHAT_SESSION_TIMEOUT_MS) is the single
// authority on when an abandoned session is finalized and summarized; the client
// only ends a session it owns, on its own SessionEnd.
function pruneDeadSessions(currentSessionId) {
    for (const sessionId of listArmedSessionIds()) {
        ...
        if (!fs.existsSync(meta.transcriptPath)) {
            cleanupSession(sessionId);
        }
    }
}
const AMP_SESSION_RETENTION_MS = 7 * 24 * 60 * 60 * 1000;
...
function isStaleAmpSession(sessionId, transcriptPath) {
    ...
    return (lastActivityMs !== null &&
        Date.now() - lastActivityMs > AMP_SESSION_RETENTION_MS);
}
