...
describe("McpProxy", () => {
    ...
    describe("toolCall", () => {
        it("returns dev server error when serviceUrl is dsm:not-started sentinel", async () => {
            const proxy = new McpProxy(testPlatform, "9.9.9");
            const result = await proxy.toolCall({ serviceUrl: "dsm:not-started", apiKey: "test-key" }, "list_trace_functions", {});
            expect(result).toEqual(errorResult("No Bitfab dev server running in this worktree. Start one with /bitfab-dev:server (or `dsm up -p bitfab`)."));
        });
        ...
        it("sends correct headers", async () => {
            ...
            expect(initReq.headers["x-bitfab-client-name"]).toBe("test");
            expect(initReq.headers["x-bitfab-client-display-name"]).toBe("Test");
            expect(initReq.headers["x-bitfab-client-version"]).toBe("9.9.9");
            const resolved = Intl.DateTimeFormat().resolvedOptions();
            expect(initReq.headers["x-bitfab-client-timezone"]).toBe(resolved.timeZone);
            expect(initReq.headers["x-bitfab-client-locale"]).toBe(resolved.locale);
            expect(initReq.headers["x-bitfab-client-os"]).toBe(os.platform());
            expect(initReq.headers["x-bitfab-client-arch"]).toBe(os.arch());
            expect(initReq.headers["x-bitfab-client-node-version"]).toBe(process.version);
            expect(initReq.headers["x-bitfab-install-id"]).toBe("test-install-id");
        });
