...
function readTsFromYarn(cwd) {
    ...
    const match = content.match(/^(?:"?(?:@bitfab\/sdk|bitfab)@[^",:\n]+"?(?:,\s*"?(?:@bitfab\/sdk|bitfab)@[^",:\n]+"?)*):\s*\n(?:[^\n]*\n)*?\s+version\s+"([^"]+)"/m);
    return match ? { version: match[1], path: p } : null;
}
