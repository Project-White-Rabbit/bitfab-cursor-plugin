...
export function createMcpServer(platform, getConfig, pluginVersion) {
    ...
    server.registerTool("save_agent_labels", {
        description: 'Save a pass/fail label authored by you on one or more traces (`labelSource="agent"`, `approvedAt=null`). Use this AFTER you have read the traces with read_traces and decided yourself whether each one looks like a pass or a fail. The labels are real — they show up in the labeling UI as the pre-selected verdict for the human reviewer to approve or change. They start unapproved; once a human approves one in the UI, `approvedAt` is set and the label becomes part of the validated dataset (queryable via `search_traces` with `validated: true`). The annotation is your reasoning, shown to the human reviewer. This is purely a persistence call; the platform does not re-judge the traces.',
        inputSchema: {
            labels: z
                ...
                annotation: z
                    .string()
                    .min(1)
                    .describe("Your reasoning for the verdict — shown to the human reviewer in the labeling UI. Required."),
            }))
                .min(1)
                .max(50))
                .describe("One label per trace (1-50)"),
        },
    }, async ({ labels }) => proxyToolCall("save_agent_labels", { labels }));
    return { server, proxy };
}
