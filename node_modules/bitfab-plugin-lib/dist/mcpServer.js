...
// node unclassified (LLM calls included). Keep in sync with bitfab-web.
...
export function createMcpServer(platform, getConfig, pluginVersion) {
    ...
    server.registerTool("save_experiment_group", {
        ...
        inputSchema: {
            ...
            name: z
                ...
                .max(120)
                .optional()
                .describe("Human-readable group name. Optional when creating and required when editing."),
        },
    }, async ({ experimentGroupId, testRunIds, name }) => {
        ...
        if (experimentGroupId !== undefined && name === undefined) {
            return {
                content: [
                    {
                        type: "text",
                        text: "Error saving experiment group: name is required when editing an experiment group.",
                    },
                ],
                isError: true,
            };
    ...
    }, async ({ testRunId }) => proxyToolCall("get_experiment", { testRunId }));
    ...
    server.registerTool("create_trace_plan", {
        ...
        description: "Post a tracing instrumentation plan and get back a URL the user opens to confirm or adjust. Send the full call tree (root + children + framework spans + ~10 surrounding callers above the root and ~10 surrounding callees below each leaf as `pure` context nodes), the initial recommended captured node ids, replay `analysis` (classification + kind) on every node, and any sample input/output stored on the relevant nodes. Pass `traceFunctionKey` so future Modify cycles for this same key can bootstrap from this plan via get_trace_plan. Returns a URL the user opens in the browser; after they confirm or abort there, call get_trace_plan to read the final captured set and the per-node replay/mock decisions, then act on them.",
        inputSchema: {
            language: z
            ...
            capturedNodeIds: z
                ...
                .describe("Initial recommended captured set. Must reference ids present in tree.nodes. Must form a connected sub-tree rooted at one of the nodes (selecting any descendant implies its ancestors). Surrounding `pure` context nodes are not captured by default."),
            stats: z
    ...
    }, async ({ language, tree, capturedNodeIds, stats, traceFunctionKey, source, agentRunId, }) => proxyToolCall("create_trace_plan", {
        language,
        tree,
        capturedNodeIds,
        stats,
    ...
    server.registerTool("confirm_trace_plan", {
        ...
        description: "Confirm a trace plan WITHOUT the browser, for the inline 'Continue' path where the user reviewed the ASCII plan in chat and chose not to open Studio. Persists the plan as the latest *confirmed* plan for its traceFunctionKey (so later get_trace_plan by key, setup view, and setup modify can find it) and returns the final captured set plus per-node replay/mock decisions, exactly like get_trace_plan. Studio's Confirm button does this for the browser path; call this only for Continue. Pass the recommended capturedNodeIds; omit mockOnReplayByNodeId to accept the analysis-derived replay decisions (only pass it to override specific nodes the way a Studio toggle would).",
        inputSchema: {
            planId: z
                .string()
