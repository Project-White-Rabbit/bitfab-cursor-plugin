...
function createMcpServer(platform, getConfig, pluginVersion) {
    ...
    server.registerTool("search_traces", {
        ...
        inputSchema: {
            ...
            labelSource: z
                .enum(["human", "agent", "human_or_approved_agent"])
                .optional()
                .describe('Filter by label source: "human" for manual human labels, "agent" for any agent labels (approved or not), "human_or_approved_agent" for validated labels — human labels OR agent labels a human has approved (use this when you want trustworthy/reviewed labels).'),
            labelResult: z
                .boolean()
                .optional()
    ...
    server.registerTool("save_agent_labels", {
        description: 'Save your own pass/fail verdict on one or more traces as an agent label (labelSource="agent"). Use this AFTER you have read the traces with read_traces and decided yourself whether each one looks like a pass or a fail — the labeling UI surfaces your verdict to the human reviewer as a one-click suggestion. The annotation is your own reasoning shown to the human. This is purely a persistence call; the platform does not re-judge the traces.',
        inputSchema: {
            labels: z
                .array(z.object({
