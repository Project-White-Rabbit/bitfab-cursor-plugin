...
export function createMcpServer(platform, getConfig, pluginVersion) {
    ...
    server.registerTool("read_traces", {
        description: "Read one or more traces by ID. Includes inputs, outputs, status, trace-level duration (ms), token usage (in/out/cached/total), and model (when available). Use these to compare latency and cost between a trace and its replay. Returns full span details (input, output, reasoning, context, errors) plus per-span duration, tokens, time-to-first-token, and model when available.",
        inputSchema: {
            traceIds: z
            ...
            scope: z
                ...
                .optional()
                .default("full")
                .describe('Level of span detail. Always returns full detail: complete input, output, reasoning, content, and errors plus per-span duration, tokens, time-to-first-token, and model when available. "summary" is accepted as a deprecated alias (older plugins) and also returns full detail.'),
        },
