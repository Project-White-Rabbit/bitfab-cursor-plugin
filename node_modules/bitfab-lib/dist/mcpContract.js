    ...
    inputSchema: {
        ...
        assertions: z
            ...
            assertion: z
                .string()
                .min(1)
                .describe("The single thing that must hold, in one sentence, stated so a reader who has never seen this trace could check it, e.g. 'The itinerary returned lands before 9am local time'. One claim per assertion: if you are about to write 'and', write two assertions instead, so each can pass or fail on its own."),
            humanNote: z
                .string()
                .max(10_000)
                .optional()
...
export const saveTracePlan = {
    ...
    inputSchema: {
        ...
        capture: z
            .preprocess(parseJsonString, z.array(z.string()))
            .optional()
            .describe("TARGETED update mode. Node ids to add to the existing captured set, each one pulling in every span above it so the captured set stays one connected sub-tree. Requires `planId`; cannot be combined with `tree` / `capturedNodeIds`."),
        uncapture: z
            .preprocess(parseJsonString, z.array(z.string()))
            .optional()
            .describe("TARGETED update mode. Node ids to remove from the existing captured set, each one also dropping every span beneath it. Requires `planId`; cannot be combined with `tree` / `capturedNodeIds`."),
...
export const getSimPlan = {
    ...
    inputSchema: {
        ...
        limit: z
            ...
            .optional()
            .describe("Recent traces to overlay (1-100). Defaults to 20."),
    },
};
