...
export const updateTracePlan = {
    ...
    inputSchema: {
        ...
        capture: z
            .preprocess(parseJsonString, z.array(z.string()))
            .optional()
            .describe("TARGETED mode. Node ids to add to the captured set. Must reference ids present in the plan's tree. Cannot be combined with `tree` / `capturedNodeIds`."),
        uncapture: z
            .preprocess(parseJsonString, z.array(z.string()))
            .optional()
        ...
        mockOnReplayByNodeId: z
            .preprocess(parseJsonString, z.record(z.string(), z.boolean()))
            .optional()
            .describe("Per-node overrides of the replay/mock decision (the equivalent of a Studio toggle): true serves the span's recorded output on replay, false re-runs it live. Only the listed nodes change. Valid in either mode. The replay entry point cannot be mocked, and neither can a framework-observed (`kind: 'auto'`) span, since replay cannot short-circuit one."),
        tree: z
            .preprocess(parseJsonString, tracePlanTreeShape)
            .optional()
            .describe("STRUCTURAL mode. The full replacement TracePlanTree, same shape and the same per-node `analysis` requirements as create_trace_plan. Send this only when the plan's nodes themselves change (code moved, a call was added or removed); to merely capture or mock differently use `capture` / `uncapture` / `mockOnReplayByNodeId`. Must be sent together with `capturedNodeIds`. `rootId` cannot be changed; to move the replay entry point, change which nodes are captured instead, an uncaptured tree root is honored and the top-most captured span becomes the entry."),
        capturedNodeIds: z
            .preprocess(parseJsonString, z.array(z.string()))
            .optional()
            .describe("STRUCTURAL mode. The absolute captured set for the replacement `tree`, restated in full. Must reference ids present in that tree and form a single connected sub-tree with one entry point. Required with `tree`, and invalid without it."),
        traceFunctionKey: z
            .string()
            .min(1)
            .optional()
            .describe("Optional replacement trace function key, for when the key the plan was created with was wrong or is being renamed."),
        stats: z
            .preprocess(parseJsonString, z.unknown())
            .optional()
            .describe("Optional sample-run stats: { durationMs?, tokens?, cost? }"),
