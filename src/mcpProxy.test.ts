...
describe("McpProxy", () => {
  ...
  const config: McpConfig = { serviceUrl: "", apiKey: "test-key" }

  function configWithUrl(): McpConfig {
  ...
  describe("toolCall", () => {
    it("returns auth error when no API key", async () => {
      ...
      const result = await proxy.toolCall(
        { serviceUrl: baseUrl, apiKey: null },
    ...
    it("returns error on network failure", async () => {
      const proxy = new McpProxy()
      const result = await proxy.toolCall(
        { serviceUrl: "http://127.0.0.1:1", apiKey: "key" },
        "tool",
        {},
      )
  ...
  describe("ensureSession", () => {
    it("throws on failed initialize", async () => {
      ...
      await expect(proxy.ensureSession(configWithUrl())).rejects.toThrow(
        "Initialize failed (503)",
      )
    })
  })
})
